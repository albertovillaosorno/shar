...
# Boundary-Contract:
# - Owns:
#   - Read-only intake of the release-index-bound world-material sidecar.
# - Must-Not:
...
# - Summary:
#   - Bound world-material construction sidecar reader.
# - Description:
...
_ARTIFACT_ID = "world-materials"
_FILENAME = "world-materials.json"
_MAX_BYTES = 512 * 1024 * 1024


def read_bound_world_material_document(
    staging_root: Path,
    bundle: ValidatedPlanBundle,
) -> JsonObject:
    """Read the release-index-bound world-material sidecar."""
...
def _bound_summary(bundle: ValidatedPlanBundle) -> SemanticArtifactSummary:
    ...
    if len(matches) != 1 or matches[0].filename != _FILENAME:
        fail_protocol(
            "plan bundle does not bind one canonical world-material sidecar"
        )
    return matches[0]
...
def _require_unlinked_directory(path: Path) -> None:
    try:
        ...
        fail_protocol("world-material staging root is missing", cause=error)
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISDIR(metadata.st_mode):
        fail_protocol(
            "world-material staging root is redirected or not a directory"
        )
...
def _read_bound_bytes(
    ...
    try:
        ...
        fail_protocol("world-material sidecar is missing", cause=error)
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        fail_protocol("world-material sidecar is not a regular unlinked file")
    if metadata.st_size > _MAX_BYTES or metadata.st_size != summary.byte_count:
        fail_protocol("world-material sidecar byte count is stale")
    try:
        data = path.read_bytes()
    except OSError as error:
        fail_protocol("failed to read world-material sidecar", cause=error)
    if len(data) != metadata.st_size:
        fail_protocol("world-material sidecar size changed during read")
    if hashlib.sha256(data).hexdigest() != summary.revision:
        fail_protocol("world-material sidecar revision is stale")
    return data
...
def _parse_document(data: bytes) -> JsonObject:
    try:
        ...
        fail_protocol(
            "world-material sidecar is not canonical JSON", cause=error
        )
    return require_json_object(value, context="world-material sidecar")
