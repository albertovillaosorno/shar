...
impl WriteFile {
    ...
    pub fn bytes(
        ...
        require_explicit_path(path, "write file")?;
        if !has_file_destination(path) {
            return Err(path_error(
                io::ErrorKind::InvalidInput,
                "write file",
...
impl InspectPath {
    ...
    pub fn canonicalize(
        ...
        require_explicit_path(path, "canonicalize path")?;
        inspector.canonicalize(path)
    }
}

fn validate_collected_files(
    root: &Path,
...
impl CollectStrictRegularFiles {
    /// Collects sorted regular files while rejecting redirects and special
    /// entries.
    ///
    /// # Errors
    ///
