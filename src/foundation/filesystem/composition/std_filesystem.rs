...
use crate::io_context::{invalid_input, with_path};
...
fn reject_links_in_parents(path: &Path) -> io::Result<()> {
    for prefix in path.ancestors().skip(1) {
        reject_existing_link(prefix)?;
    }
    Ok(())
}
...
impl FileReader for StdFilesystem {
    fn read_bytes(&self, path: &Path) -> io::Result<Vec<u8>> {
        ...
        match fs::read(path) {
            Ok(bytes) => Ok(bytes),
            Err(source) => Err(with_path("read file", path, source)),
        }
    }
}
...
impl PathInspector for StdFilesystem {
    ...
    fn canonicalize(&self, path: &Path) -> io::Result<PathBuf> {
        match fs::canonicalize(path) {
            ...
            Err(source) => Err(with_path("canonicalize path", path, source)),
        }
    }
}
