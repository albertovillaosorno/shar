...
fn write_raw_os_error(
    ...
    let provider_source = source.get_ref().unwrap_or(source);
    let Some(raw_os_error) = raw_os_error::find(provider_source) else {
        return Ok(());
    };
    write!(formatter, " [OS error code: {raw_os_error}]")
}
...
impl core::fmt::Display for OutputError {
    fn fmt(
        ...
        write!(formatter, " [I/O error kind: {primary_kind}]")?;
        write_raw_os_error(formatter, primary_source)?;
            ...
            && let Some(secondary_chunk_index) = self.secondary_chunk_index
        {
            ...
            write!(formatter, " [I/O error kind: {secondary_kind}]")?;
            write_raw_os_error(formatter, secondary_source)?;
        }
        let command_status_name = status_name(self.status);
        write!(formatter, " (command status: {command_status_name}")?;
