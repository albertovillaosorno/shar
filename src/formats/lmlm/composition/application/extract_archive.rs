...
// Boundary-Contract:
// - Owns:
//   - Extract archive application service.
// - Must-Not:
...
impl std::error::Error for ExtractArchiveError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            Self::Read { source, .. } | Self::Materialize { source, .. } => {
                Some(source)
            },
...
impl ExtractArchive {
    ...
    pub fn execute(
        ...
        let archive = source.read_archive(input).map_err(|read_error| {
            ExtractArchiveError::Read {
                path: input.to_path_buf(),
                source: read_error,
            }
        })?;
        let entries = parse(&archive).map_err(|parse_error| {
            ExtractArchiveError::Parse {
                path: input.to_path_buf(),
                source: parse_error,
            }
        })?;
        sink.materialize(&archive, &entries, output_root).map_err(
