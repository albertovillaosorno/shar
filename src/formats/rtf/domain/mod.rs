...
//   - Domain domain module.
// - Description:
//   - Implements the declared domain module responsibility for rtf.
// - Usage:
...
impl<'a> Parser<'a> {
    ...
    fn current_ignore(&self) -> bool {
        *self.ignore.last().unwrap_or(&false)
    }

    /// Reads the current group's selected font id.
    fn current_font(&self) -> Option<i32> {
        self.font.last().copied().flatten()
    }

    /// Updates the current group's selected font id.
    fn set_font(&mut self, font_id: i32) {
        if let Some(top) = self.font.last_mut() {
            *top = Some(font_id);
        }
    }

    /// Records and selects the document's default font.
    fn set_default_font(&mut self, font_id: i32) {
    ...
    fn decode_ansi_byte(&self, byte: u8) -> char {
        let uses_symbol_charset = self
            .current_font()
            .is_some_and(|font_id| self.symbol_fonts.contains(&font_id));
    ...
    fn current_unicode_fallback(&self) -> usize {
        self.unicode_fallback.last().copied().unwrap_or(1)
    }

    /// Updates the current group's Unicode fallback width.
    fn set_unicode_fallback(&mut self, count: usize) {
        if let Some(top) = self.unicode_fallback.last_mut() {
            *top = count;
        }
    }
    ...
    fn emit_unicode(&mut self, value: i32) {
    ...
    fn set_ignore(&mut self) {
        self.flush_pending_surrogate();
        if let Some(top) = self.ignore.last_mut() {
            *top = true;
        }
    }
    ...
    fn advance(&mut self, amount: usize) {
        self.pos = self.pos.saturating_add(amount).min(self.bytes.len());
    }
    ...
    fn run(mut self) -> String {
        while let Some(current) = self.bytes.get(self.pos).copied() {
            match current {
                b'{' => {
                    ...
                    self.ignore.push(inherited_ignore);
                    self.unicode_fallback.push(inherited_fallback);
                    self.font.push(inherited_font);
                    self.advance(1);
                },
                b'}' => {
                    if self.ignore.pop().is_none() {
                        self.ignore.push(false);
                    }
                    if self.ignore.is_empty() {
                        self.ignore.push(false);
                    }
                    if self.unicode_fallback.pop().is_none() {
                        self.unicode_fallback.push(1);
                    }
                    if self.unicode_fallback.is_empty() {
                        self.unicode_fallback.push(1);
                    }
                    if self.font.pop().is_none() {
                        self.font.push(None);
                    }
    ...
    fn control(&mut self) {
        self.advance(1);
        let Some(symbol) = self.bytes.get(self.pos).copied() else {
            return;
        };
    ...
    /// this converter.
    fn apply_word(&mut self, word: &str, param: Option<i32>) {
        match word {
            ...
            "endash" => self.emit('–'),
            "bullet" => self.emit('•'),
            "emspace" => self.emit('\u{2003}'),
            "enspace" => self.emit('\u{2002}'),
            "qmspace" => self.emit('\u{2005}'),
            ...
            "fcharset" => {
                ...
                {
                    let _inserted = self.symbol_fonts.insert(font_id);
                }
            },
...
#[cfg(test)]
#[path = "../../../../tests/formats/rtf/unit/domain/tests.rs"]
mod tests;
