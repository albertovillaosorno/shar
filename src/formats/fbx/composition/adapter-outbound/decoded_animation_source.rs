...
// Boundary-Contract:
// - Owns:
//   - Decoded animation source outbound adapter.
// - Must-Not:
...
pub fn load_animation_clips(
    ...
    paths
        .iter()
        .map(|path| load_clip(path, bones, &rest_transforms))
        .collect()
}
...
fn decoded_groups(
    ...
    for list in group_lists {
        ...
        if list.group_count != actual {
            return Err(DecodedAnimationError::InvalidGroupCount {
        ...
        for group in list.groups {
            ...
            let actual_channels = group.channels.len();
            if group.channel_count != actual_channels {
                return Err(DecodedAnimationError::InvalidChannelCount {
                    group: identity,
            ...
            for channel in &group.channels {
                let actual_keys = channel.frames.len();
                if channel.key_count != actual_keys {
                    return Err(DecodedAnimationError::InvalidKeyCount {
                        group: identity,
...
fn sample_track(
    ...
    for frame in 0..frame_count {
        let translation_value = translation
            .map_or(Ok(rest.translation), |channel| {
                sample_translation(channel, frame)
            })?;
        let rotation_value = rotation
            .map_or(Ok(rest.rotation_wxyz), |channel| {
                sample_rotation(channel, frame)
            })?;
        samples.push(LocalTransformSample {
            translation: translation_value,
            rotation_wxyz: rotation_value,
        });
    }
    Ok(BoneAnimationTrack {
...
fn unique_channel<'group>(
    ...
    if channels.next().is_some() {
        return Err(DecodedAnimationError::DuplicateChannel {
...
fn sample_translation(
    ...
    match channel.kind.as_str() {
        "vector1" => sample_vector1(channel, frame),
        "vector2" => sample_vector2(channel, frame),
        "vector3" => sample_vector3(channel, frame),
        kind => Err(DecodedAnimationError::UnsupportedChannelKind {
            parameter: channel.param.clone(),
            kind: kind.to_owned(),
        }),
    }
...
fn sample_rotation(
    ...
    match channel.kind.as_str() {
        ...
        "quaternion" => {
            ensure_key_series(&channel.frames, channel.values.len())?;
            let values = channel
                .values
                .iter()
                .map(|value| {
                    ...
                    normalize([value[0], value[1], value[2], value[3]])
                        .map_err(DecodedAnimationError::Quaternion)
                })
                .collect::<Result<Vec<_>, _>>()?;
            sample_quaternion_series(
...
fn bracket(
    frames: &[u16],
    frame: usize,
) -> Result<(usize, usize, f64), DecodedAnimationError> {
    ...
    if frame >= last_frame {
        return Ok((last_index, last_index, 0.));
    }
    for (start, pair) in frames.windows(2).enumerate() {
        ...
        if frame >= start_frame && frame <= end_frame {
            ...
            let elapsed = frame
                .checked_sub(start_frame)
                .ok_or(DecodedAnimationError::InvalidKeySeries)?;
            return Ok((start, start + 1, elapsed as f64 / span as f64));
        }
    }
...
fn sample_scalar_series(
    ...
    let (start, end, amount) = bracket(frames, frame)?;
...
fn sample_quaternion_series(
    ...
    let end_value = *values
        .get(end)
        .ok_or(DecodedAnimationError::InvalidKeySeries)?;
    Ok(slerp(start_value, end_value, amount))
}
...
fn frame_count(value: f64) -> Result<usize, DecodedAnimationError> {
    if !value.is_finite() || value < 1. || value.fract().abs() > 0f64 {
...
#[path = "../../../../../tests/formats/fbx/unit/adapter-outbound/decoded_animation_source/loose_tests.rs"]
mod loose_tests;
