...
// Boundary-Contract:
// - Owns:
//   - Binary character input outbound adapter.
// - Must-Not:
...
pub(super) fn bone_transforms(
    ...
    let mut transforms: Vec<BoneTransform> =
        Vec::with_capacity(character.bones.len());
    for bone in &character.bones {
        let local = widen(&bone.rest_matrix);
        let local_parts = decompose(&local).map_err(|error| {
            CharacterInputError::UnsupportedRestMatrix {
                bone: bone.id.clone(),
                error,
            }
        })?;
        let global_bind = match &bone.parent_id {
            Some(parent) => {
                ...
                multiply(&local, &parent_transform.global_bind)
            },
            None => local,
        };
        transforms.push(BoneTransform { local_parts, global_bind });
    }
    Ok(transforms)
}
...
pub(super) fn effective_material_semantics(
    ...
    binding
        .semantics
        .merge(MaterialSemantics::from_identities(mesh_name, None))
}
...
pub(super) fn material_variant_key(
    ...
    let signature = semantics.suffix().unwrap_or_else(|| "opaque".to_owned());
...
> {
    ...
    for binding in materials {
        if source_bindings
            ...
            .is_some()
        {
            return Err(CharacterInputError::DuplicateMaterialBinding {
                material: binding.material_name.clone(),
            });
        }
        if let Some(file_name) = &binding.texture_file_name
            && !is_portable_texture_file_name(file_name)
        {
            return Err(CharacterInputError::InvalidTextureFileName {
                file_name: file_name.clone(),
            });
        }
        if !used_shaders.contains(binding.material_name.as_str()) {
            return Err(CharacterInputError::UnusedMaterialBinding {
                material: binding.material_name.clone(),
            });
        }
    }
