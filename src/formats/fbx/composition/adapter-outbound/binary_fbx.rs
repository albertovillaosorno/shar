...
impl BinaryNode {
    ...
    pub(super) fn branch(name: impl Into<String>, children: Vec<Self>) -> Self {
        Self::new(name, Vec::new(), children)
    }
    ...
    fn encoded_len(&self) -> Result<usize, BinaryFbxError> {
        ...
        if u8::try_from(name_len).is_err() {
            return Err(BinaryFbxError::NodeNameTooLong {
        ...
        let children_len =
            self.children.iter().try_fold(0_usize, |total, child| {
                checked_add(total, child.encoded_len()?, "children")
            })?;
        ...
        .try_fold(0_usize, |total, value| {
            checked_add(total, value, "node size")
        })
    }

    /// Return the complete encoded property-list size.
    fn properties_len(&self) -> Result<usize, BinaryFbxError> {
        self.properties.iter().try_fold(0_usize, |total, property| {
            checked_add(total, property.encoded_len()?, "property list")
        })
    }

    /// Append this node using absolute file offsets.
    fn append_to(&self, output: &mut Vec<u8>) -> Result<(), BinaryFbxError> {
        let start = output.len();
        let node_len = self.encoded_len()?;
        let end = checked_add(start, node_len, "node end offset")?;
        append_u64(output, end, "node end offset")?;
        append_u64(output, self.properties.len(), "property count")?;
        append_u64(output, self.properties_len()?, "property list length")?;
        let name_len =
            u8::try_from(self.name.len()).map_err(|_conversion_error| {
        ...
        if output.len() != end {
            return Err(BinaryFbxError::LengthMismatch {
...
impl BinaryProperty {
    ...
    fn encoded_len(&self) -> Result<usize, BinaryFbxError> {
        let payload = match self {
            ...
            Self::Bytes(value) => checked_add(4, value.len(), "raw property")?,
            Self::String(value) => {
                checked_add(4, value.len(), "string property")?
            },
            Self::I32Array(values) => array_property_len(values.len(), 4)?,
            Self::I64Array(values) => array_property_len(values.len(), 8)?,
            Self::F32Array(values) => array_property_len(values.len(), 4)?,
            Self::F64Array(values) => array_property_len(values.len(), 8)?,
        };
        checked_add(1, payload, "typed property")
    }
    ...
    fn append_to(&self, output: &mut Vec<u8>) -> Result<(), BinaryFbxError> {
        match self {
            ...
            Self::Bytes(value) => {
                output.push(b'R');
                append_u32(output, value.len(), "raw property length")?;
                output.extend_from_slice(value);
            },
            Self::String(value) => {
                output.push(b'S');
                append_u32(output, value.len(), "string property length")?;
                output.extend_from_slice(value.as_bytes());
            },
            Self::I32Array(values) => {
                output.push(b'i');
                append_array_header(output, values.len(), 4)?;
                for value in values {
                    output.extend_from_slice(&value.to_le_bytes());
                }
            },
            Self::I64Array(values) => {
                output.push(b'l');
                append_array_header(output, values.len(), 8)?;
                for value in values {
                    output.extend_from_slice(&value.to_le_bytes());
                }
            },
...
pub(super) fn encode_binary_document(
    ...
    let body_len = nodes.iter().try_fold(0_usize, |total, node| {
        checked_add(total, node.encoded_len()?, "document body")
    })?;
    ...
    .into_iter()
    .try_fold(0_usize, |total, value| {
        checked_add(total, value, "document capacity")
    })?;
    ...
    let padding = if remainder == 0 {
        ...
        16_usize.checked_sub(remainder).ok_or(
            BinaryFbxError::LengthOverflow {
                context: "footer padding",
            },
        )?
    };
    output.resize(checked_add(output.len(), padding, "footer alignment")?, 0);
    output.extend_from_slice(&FBX_VERSION.to_le_bytes());
...
fn array_property_len(
    ...
    let payload = element_count
        ...
        .ok_or(BinaryFbxError::LengthOverflow { context: "array payload" })?;
    checked_add(12, payload, "array property")
}
...
fn append_array_header(
    ...
    let payload = element_count
        .checked_mul(element_width)
        .ok_or(BinaryFbxError::LengthOverflow { context: "array payload" })?;
    append_u32(output, element_count, "array element count")?;
    output.extend_from_slice(&0_u32.to_le_bytes());
    append_u32(output, payload, "array payload length")
}
...
fn append_u32(
    ...
    let narrowed = u32::try_from(value).map_err(|_conversion_error| {
        BinaryFbxError::ValueExceedsU32 { context, value }
    })?;
...
fn append_u64(
    ...
    let narrowed = u64::try_from(value).map_err(|_conversion_error| {
        BinaryFbxError::ValueExceedsU64 { context, value }
    })?;
    output.extend_from_slice(&narrowed.to_le_bytes());
    Ok(())
}
...
fn checked_add(
    ...
    left.checked_add(right)
        .ok_or(BinaryFbxError::LengthOverflow { context })
}
