...
// Boundary-Contract:
// - Owns:
//   - Binary structural guide writer outbound adapter.
// - Must-Not:
...
pub fn write_binary_structural_guide_fbx(
    ...
    let bytes = encode_binary_document(&nodes).map_err(|error| {
        StructuralGuideFbxError::Encoding(format!("{error:?}"))
    })?;
    persist(path, &bytes)?;
    Ok(summary)
}
...
fn validate_mesh(
    ...
    if !mesh.normals.is_empty() && mesh.normals.len() != mesh.positions.len() {
        return Err(StructuralGuideFbxError::ChannelLengthMismatch {
            channel: "normal",
            positions: mesh.positions.len(),
            values: mesh.normals.len(),
        });
    }
    for (name, values) in [
        (STRUCTURAL_GUIDE_UV_NAMES[0], mesh.atlas_uvs.len()),
        (STRUCTURAL_GUIDE_UV_NAMES[1], mesh.source_uvs.len()),
        (STRUCTURAL_GUIDE_UV_NAMES[2], mesh.atlas_offsets.len()),
        (STRUCTURAL_GUIDE_UV_NAMES[3], mesh.atlas_scales.len()),
    ] {
        if values != mesh.positions.len() {
            return Err(StructuralGuideFbxError::ChannelLengthMismatch {
                channel: name,
                positions: mesh.positions.len(),
                values,
    ...
    validate_finite3("position", &mesh.positions)?;
    validate_finite3("normal", &mesh.normals)?;
    validate_finite2(STRUCTURAL_GUIDE_UV_NAMES[0], &mesh.atlas_uvs)?;
    validate_finite2(STRUCTURAL_GUIDE_UV_NAMES[1], &mesh.source_uvs)?;
    validate_finite2(STRUCTURAL_GUIDE_UV_NAMES[2], &mesh.atlas_offsets)?;
    validate_finite2(STRUCTURAL_GUIDE_UV_NAMES[3], &mesh.atlas_scales)?;
    for (vertex, normal) in mesh.normals.iter().enumerate() {
        let length_squared =
            normal.iter().map(|value| value * value).sum::<f32>();
        if !(0.999..=1.001).contains(&length_squared) {
            return Err(StructuralGuideFbxError::InvalidNormal { vertex });
        }
    }
    for (ordinal, triangle) in mesh.triangles.iter().enumerate() {
        let [first, second, third] = *triangle;
        if first == second || first == third || second == third {
            return Err(StructuralGuideFbxError::DegenerateTriangle {
                triangle: ordinal,
            });
        }
        for index in triangle {
            if usize::try_from(*index)
                .map_or(true, |value| value >= mesh.positions.len())
            {
                return Err(StructuralGuideFbxError::IndexOutOfBounds {
                    index: *index,
                    positions: mesh.positions.len(),
                });
            }
    ...
    let Some(first_position) = mesh.positions.first().copied() else {
        return Err(StructuralGuideFbxError::EmptyMesh);
    };
    let mut bounds_min = first_position;
    let mut bounds_max = first_position;
    for position in mesh.positions.iter().skip(1) {
        for ((minimum, maximum), value) in bounds_min
            .iter_mut()
            .zip(bounds_max.iter_mut())
            .zip(position)
        {
...
fn validate_finite3(
    ...
    for (element, value) in values.iter().enumerate() {
        if let Some(axis) =
            value.iter().position(|component| !component.is_finite())
        {
            return Err(StructuralGuideFbxError::NonFiniteValue {
                channel,
                element,
                axis,
            });
...
fn persist(path: &Path, bytes: &[u8]) -> Result<(), StructuralGuideFbxError> {
    let parent = path.parent().ok_or_else(|| {
        StructuralGuideFbxError::MissingParent(path.display().to_string())
    })?;
    ...
    let mut file = OpenOptions::new()
        ...
        if source.kind() == ErrorKind::AlreadyExists {
            StructuralGuideFbxError::OutputExists(path.display().to_string())
        } else {
            StructuralGuideFbxError::Write {
    ...
    file.write_all(bytes)
        .map_err(|source| StructuralGuideFbxError::Write {
            path: path.display().to_string(),
            source: source.to_string(),
        })
}
