...
// Boundary-Contract:
// - Owns:
//   - Semantic character texture outbound adapter.
// - Must-Not:
...
pub fn prepare_semantic_character(
    ...
    if request.skin_paths.is_empty() {
    ...
    .map_err(|error| {
        SemanticTextureArtifactError::Character(format!("{error:?}"))
    })?;
    let animations = load_requested_animations(request, &character.bones)?;
    let body_source = decode_image("body-texture", &request.body_texture_path)?;
    let body_mode = request.body_texture_mode().map_err(|error| {
        SemanticTextureArtifactError::Request(format!("{error:?}"))
    })?;
    let recipe = request.body_recipe().map_err(|error| {
        SemanticTextureArtifactError::Request(format!("{error:?}"))
    })?;
    let body = match body_mode {
        BodyTextureMode::PreserveSource => {
            preserve_body_texture(&character, &body_source, &recipe.groups)?
        },
        BodyTextureMode::SemanticAtlas => {
            plan_body_texture(&character, &body_source, &recipe).map_err(
                |error| {
                    SemanticTextureArtifactError::Body(format!("{error:?}"))
                },
            )?
        },
    };
    let eye = prepare_eye(&character, request)?;
    let package = package::build(request, &body.remapped_character)?;
...
fn load_requested_animations(
    ...
    let paths = request
        ...
        .map(std::path::PathBuf::as_path)
        .collect::<Vec<_>>();
    let animations = load_animation_clips(&paths, bones).map_err(|error| {
        SemanticTextureArtifactError::Animation(format!("{error:?}"))
    })?;
    let mut names = BTreeSet::new();
    for animation in &animations {
        if !names.insert(animation.name.clone()) {
            return Err(SemanticTextureArtifactError::DuplicateAnimationClip(
...
> {
    let (eye_group_request, eye_frame_paths) =
        match (request.eye_group, request.eye_frame_paths.as_ref()) {
            (None, None) => return Ok(None),
            (Some(group_request), Some(frame_paths)) => {
                (group_request, frame_paths)
            },
            (None, Some(_)) | (Some(_), None) => {
                return Err(SemanticTextureArtifactError::Request(format!(
                    "{:?}",
    ...
    let eye_group = group(character, eye_group_address)?;
    let eye_source_images = eye_frame_paths
        .iter()
        .enumerate()
        .map(|(index, path)| decode_image(&format!("eye-frame-{index}"), path))
        .collect::<Result<Vec<_>, _>>()?;
    let eye_sources: [_; 4] =
        eye_source_images.try_into().map_err(|_frames: Vec<_>| {
            SemanticTextureArtifactError::EyeFrameCount
        })?;
    analyze_eye_frames(eye_group, &eye_sources, request.eye_output_size)
        .map(Some)
        .map_err(|error| {
            SemanticTextureArtifactError::Eye(format!("{error:?}"))
        })
}
...
fn preserve_body_texture(
    ...
    for address in groups {
        let part =
            character.parts.get(address.part_index).ok_or_else(|| {
                SemanticTextureArtifactError::Body(format!(
                    "missing body part: {address:?}"
                ))
            })?;
        let group =
            part.mesh.groups.get(address.group_index).ok_or_else(|| {
                SemanticTextureArtifactError::Body(format!(
                    "missing body group: {address:?}"
                ))
            })?;
        if group.uvs.len() != group.positions.len() {
            return Err(SemanticTextureArtifactError::Body(format!(
                "body group has incomplete UVs: {address:?}"
            )));
        }
...
fn decode_image(
    ...
    let bytes = read_bytes(path).map_err(|error| {
        SemanticTextureArtifactError::Read {
            role: role.to_owned(),
            message: error.to_string(),
        }
    })?;
    decode_png_bytes(&bytes).map_err(|error| {
