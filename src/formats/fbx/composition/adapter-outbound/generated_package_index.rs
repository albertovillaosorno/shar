...
// Boundary-Contract:
// - Owns:
//   - Generated package index outbound adapter.
// - Must-Not:
...
impl GeneratedPackageCatalog {
    ...
    pub fn from_jsonl(text: &str) -> Result<Self, PackageIndexAdapterError> {
        let jsonl_text = text.strip_prefix('\u{feff}').unwrap_or(text);
        let mut rows = BTreeMap::new();
        for (line_index, line) in jsonl_text.lines().enumerate() {
            if line.trim().is_empty() {
                continue;
            }
            let row: IndexedPackageRow =
            ...
            validate_indexed_row(&row, line_number)?;
            if rows.insert(row.package_id.clone(), row).is_some() {
                return Err(PackageIndexAdapterError::DuplicatePackageId {
                    line: line_index.saturating_add(1),
                });
            }
        }
...
fn validate_canonical_whitespace(
    ...
    for (field, value) in [
        ...
        ("package_category", row.package_category.as_str()),
    ] {
        if value != value.trim() || value.chars().any(char::is_control) {
            return Err(PackageIndexAdapterError::NonCanonicalWhitespace {
                line,
    ...
    for member in &row.members {
        for (field, value) in [
            ("member.id", member.id.as_str()),
            ("member.role", member.role.as_str()),
        ] {
            if value != value.trim() || value.chars().any(char::is_control) {
                return Err(PackageIndexAdapterError::NonCanonicalWhitespace {
                    line,
...
fn validate_members(
    ...
    if row.members.iter().any(|member| member.id.trim().is_empty()) {
        return Err(PackageIndexAdapterError::BlankMemberId { line });
    }
    if row
        .members
        .iter()
        .any(|member| member.role.trim().is_empty())
    {
        return Err(PackageIndexAdapterError::BlankMemberRole { line });
    }
    validate_canonical_whitespace(row, line)?;
    if let Some(member) = row
        .members
        .iter()
        .find(|member| !is_safe_member_id(&member.id))
    {
        return Err(PackageIndexAdapterError::InvalidMemberId {
            line,
            id: member.id.clone(),
        });
    }
    ...
    {
        return Err(PackageIndexAdapterError::UnknownMemberRole {
            line,
    ...
    for member in &row.members {
        if !member_ids.insert(member.id.as_str()) {
            return Err(PackageIndexAdapterError::DuplicateMemberId {
                line,
                id: member.id.clone(),
            });
        }
...
fn validate_indexed_row(
    ...
    if row.package_id.trim().is_empty() {
        return Err(PackageIndexAdapterError::BlankPackageId { line });
    }
    if row.package_category.trim().is_empty() {
        return Err(PackageIndexAdapterError::BlankPackageCategory { line });
    }
    validate_members(row, line)?;
    Ok(())
}
...
impl PackageIndexReader for GeneratedPackageCatalog {
    ...
    fn require_model_package(
        ...
        {
            return Err(PackageIndexAdapterError::InvalidPackageSelector(
                package_id.to_owned(),
            ));
        }
        let row = self.rows.get(package_id).ok_or_else(|| {
            PackageIndexAdapterError::MissingPackage(package_id.to_owned())
        })?;
...
fn model_family(
    ...
    match row.package_category.as_str() {
        ...
        category => Err(PackageIndexAdapterError::NotFbxEligible {
...
fn classify_members(row: &IndexedPackageRow) -> MemberBuckets {
    ...
    for member in &row.members {
        match member.role.as_str() {
            "model" => buckets.models.push(member.id.clone()),
            "material" => buckets.materials.push(member.id.clone()),
            "texture" => buckets.textures.push(member.id.clone()),
            "animation" => buckets.animations.push(member.id.clone()),
            _ => {},
        }
    }
    buckets.models.sort();
...
struct IndexedMember {
    ...
    _kind: Option<String>,
    #[serde(default, rename = "source_chunk_kind")]
