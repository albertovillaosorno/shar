...
pub(super) fn build(
    ...
    let eye_group = request.eye_group.map(Into::into);
    ...
    for address in &body_groups {
        bind_group(character, *address, "body-atlas.png", &mut bindings)?;
    }
    if let Some(address) = eye_group {
        bind_group(character, address, "eye.png", &mut bindings)?;
    }
    let extra_textures = assemble_extra_materials(request, &mut bindings)?;
    assemble_untextured_materials(request, &mut bindings)?;
    validate_group_coverage(character, &body_groups, eye_group, &bindings)?;
    let materials = bindings
        .into_iter()
        .map(|(material, texture)| {
            MaterialBinding::new(material, texture).map_err(|error| {
    ...
    Ok(PackageBindings {
        materials,
        extra_textures: extra_textures.into_values().collect(),
    })
}
...
> {
    ...
    for extra in &request.extra_materials {
        ...
        let binding = MaterialBinding::new(
            extra.material_name.clone(),
            Some(extra.output_file_name.clone()),
        )
        .map_err(|error| {
        ...
        let png = encode_png_bytes(&image).map_err(|error| {
            package_error(format!(
...
fn assemble_untextured_materials(
    ...
    for material_name in &request.untextured_materials {
        let binding = MaterialBinding::new(material_name.clone(), None)
            .map_err(|error| {
                package_error(format!("invalid untextured material: {error:?}"))
            })?;
        if bindings
            .insert(binding.material_name.clone(), None)
            .is_some()
        {
            return Err(package_error(format!(
...
fn bind_group(
    ...
    let shader = group_shader(character, address)?;
    let selected = Some(texture.to_owned());
    if let Some(existing) = bindings.insert(shader.clone(), selected.clone())
        && existing != selected
    {
        return Err(package_error(format!(
            "shader {shader} maps to conflicting texture policies"
        )));
    }
    Ok(())
}
...
fn validate_group_coverage(
    ...
    let body_shaders = body_groups
        .iter()
        .map(|address| group_shader(character, *address))
    ...
    for (part_index, part) in character.parts.iter().enumerate() {
        for (group_index, group) in part.mesh.groups.iter().enumerate() {
            let address = GroupAddress { part_index, group_index };
            ...
            if eye_shader.as_ref().is_some_and(|shader| {
            ...
            let _inserted = used.insert(group.shader.clone());
        }
    }
    if let Some(unused) = bindings.keys().find(|shader| !used.contains(*shader))
    {
        return Err(package_error(format!(
