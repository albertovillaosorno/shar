...
impl SemanticTextureRequest {
    ...
    pub fn body_texture_mode(&self) -> Result<BodyTextureMode, RequestError> {
        match self.body_texture_mode.as_str() {
    ...
    /// Validate explicit extra texture output identities.
    ///
    /// Exact repeated file names remain valid because package assembly compares
    /// their normalized PNG bytes before deduplicating them.
    ///
    /// # Errors
    ///
    /// Returns an error when one extra claims a generated texture name or two
    /// differently-cased names alias on a portable case-insensitive filesystem.
    pub fn validate_extra_texture_outputs(&self) -> Result<(), RequestError> {
        let mut outputs = BTreeMap::<String, String>::new();
        for extra in &self.extra_materials {
            let identity = extra.output_file_name.to_ascii_lowercase();
            if is_generated_texture_identity(&identity) {
                return Err(RequestError::ReservedExtraTextureOutput(
                    extra.output_file_name.clone(),
                ));
            }
            if let Some(existing) = outputs.get(&identity) {
                if existing != &extra.output_file_name {
                    return Err(RequestError::CaseAliasedExtraTextureOutput {
                        first: existing.clone(),
                        second: extra.output_file_name.clone(),
                    });
                }
            } else {
                let _previous =
                    outputs.insert(identity, extra.output_file_name.clone());
            }
        }
        Ok(())
    }
    ...
    pub fn body_recipe(&self) -> Result<BodySemanticRecipe, RequestError> {
        ...
        for request in &self.color_overrides {
            ...
            if overrides.insert(color, region).is_some() {
                return Err(RequestError::DuplicateColorOverride(color));
            }
        }
        BodySemanticRecipe::new(
            self.body_groups.iter().copied().map(Into::into).collect(),
            overrides,
...
pub enum RequestError {
    ...
    /// One extra material claimed a texture identity generated by the package.
    ReservedExtraTextureOutput(String),
    /// Two differently-cased extra names alias to one portable file identity.
    CaseAliasedExtraTextureOutput {
        /// First explicit output spelling.
        first: String,
        /// Later spelling with the same ASCII case-folded identity.
        second: String,
    },
}
...
/// Return whether one case-folded texture name is generated by the package.
fn is_generated_texture_identity(identity: &str) -> bool {
    matches!(
        identity,
        "body-atlas.png" | "eye.png" | "eye-pupil.png" | "eye-lids.png"
    )
}

/// Convert one channel tuple into the domain color value.
const fn rgba(channels: [u8; 4]) -> Rgba8 {
    Rgba8::new(channels[0], channels[1], channels[2], channels[3])
}
