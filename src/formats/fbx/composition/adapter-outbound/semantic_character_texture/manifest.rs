...
pub(super) fn render(
    ...
    let selected_uvs = selected_uvs(request, body)?;
    let assignments = assignments(body);
    let charts = charts(body);
    let eyes = eyes(request, eye, eye_profile_sha256)?;
...
fn eyes(
    ...
    match (request.eye_group, eye, eye_profile_sha256) {
        (None, None, None) => Ok(Value::Null),
        (Some(group), Some(plan), Some(profile)) => Ok(json!({
            "eye_group": {
