...
// Boundary-Contract:
// - Owns:
//   - Decoded rigid prop source outbound adapter.
// - Must-Not:
...
#![expect(
    ...
    reason = "Finite f64 transform results are range-checked immediately \
              after               the required FBX f32 narrowing."
)]
...
use super::decoded_skin_source::{
    ...
    mark_transparent_mesh, rigid_group_influences, source_effect_bindings,
    source_prop_bindings,
};
...
pub fn load_selected_rigid_prop_asset(
    ...
    let composite_effect_bindings =
        source_effect_bindings(0, &composite.effects)?;
    ...
    let source_provenance =
        ...
        .and_then(|provenance| {
            provenance.with_composite_effect_bindings(composite_effect_bindings)
        })
        .and_then(|provenance| {
    ...
    let retained_bones = bones
        ...
        .filter_map(|(index, bone)| {
            retained_indices.contains(&index).then_some(bone)
        })
        .collect();
...
/// Mutable assembly state for instanced rigid component bindings.
#[derive(Default)]
struct InstancedRigidState {
    /// Per-source occurrence counts used in stable instance names.
    occurrences: BTreeMap<String, usize>,
    /// Source component identities with at least one authored binding.
    bound_names: BTreeSet<String>,
    /// Skeleton joint ordinals required by selected components.
    selected_joints: BTreeSet<usize>,
    /// Fully baked character-compatible rigid parts.
...
pub fn load_instanced_rigid_prop_asset_with_billboards(
    ...
    let mut state = InstancedRigidState::default();
    for binding in bindings {
        append_instanced_binding(
            binding,
            &meshes,
            &bones,
            &global_rest,
...
fn append_supplemental_bindings(
    bindings: &mut Vec<CompositePropBinding>,
    supplemental: &[SupplementalRigidPropBinding],
    bones: &[Bone],
) {
    ...
    for binding in supplemental {
        if let Some(joint) =
            bone_indices.get(binding.joint_id.as_str()).copied()
        {
...
/// Append one available authored component binding to mutable assembly state.
fn append_instanced_binding(
    binding: CompositePropBinding,
    meshes: &BTreeMap<String, MeshAsset>,
    bones: &[Bone],
    global_rest: &[[f64; 16]],
    state: &mut InstancedRigidState,
    ...
    let ordinal = state.occurrences.entry(mesh_name.clone()).or_insert(0);
    let mut mesh = source_mesh.clone();
    if binding.translucent {
        mark_transparent_mesh(&mut mesh);
    }
    let joint_matrix = global_rest.get(joint).ok_or_else(|| {
        SkinSourceError::Prop(format!(
            "instanced rigid prop {mesh_name} has no global rest \
                         matrix for joint {joint}"
        ))
    })?;
    bake_rigid_mesh(&mut mesh, joint_matrix)?;
    mesh.name =
        format!("{}__joint_{joint:02}__instance_{:02}", mesh.name, *ordinal);
    *ordinal = ordinal.saturating_add(1);
    let bone_id =
        bones
            ...
            .ok_or_else(|| {
                SkinSourceError::Prop(format!(
                    "instanced rigid prop {mesh_name} references missing \
                         joint {joint}"
                ))
            })?;
    let _selected = state.selected_joints.insert(joint);
    let _bound = state.bound_names.insert(mesh_name);
    state.parts.push(SkinnedPart {
        group_influences: rigid_group_influences(&mesh, &bone_id)?,
        mesh,
    });
    Ok(())
}

/// Reject selected source meshes that have no composite or supplemental
/// binding.
fn ensure_required_bindings(
    required_meshes: &BTreeSet<String>,
    bound_names: &BTreeSet<String>,
) -> Result<(), SkinSourceError> {
    ...
    Err(SkinSourceError::Prop(format!(
...
fn load_rigid_component_map(
    mesh_paths: &[&Path],
    billboard_paths: &[&Path],
) -> Result<(BTreeMap<String, MeshAsset>, BTreeSet<String>), SkinSourceError> {
    let mut meshes = load_rigid_mesh_map(mesh_paths)?;
    let required_meshes = meshes.keys().cloned().collect::<BTreeSet<_>>();
    for path in billboard_paths {
        let requested_id = mesh_member_id(path)?.trim_end_matches('_');
        let mesh =
            read_billboard_quad_group(path, requested_id).map_err(|error| {
                SkinSourceError::Prop(format!(
        ...
        if meshes.insert(mesh_name.clone(), mesh).is_some() {
            return Err(SkinSourceError::Prop(format!(
                "duplicate instanced rigid component: {mesh_name}"
            )));
        }
    }
    Ok((meshes, required_meshes))
}
...
fn load_selected_parts(
    ...
    for mesh_path in mesh_paths {
        let mut mesh = read_selected_rigid_mesh(mesh_path)?;
        if !selected_names.insert(mesh.name.clone()) {
            return Err(SkinSourceError::Prop(format!(
...
fn global_rest_matrices(
    ...
    for bone in bones {
        ...
        let matrix = match bone.parent_id.as_deref() {
            Some(parent) if parent != bone.id => {
                let parent_index =
                    ordinals.get(parent).copied().ok_or_else(|| {
                        SkinSourceError::Prop(format!(
                ...
                let parent_matrix =
                    global.get(parent_index).ok_or_else(|| {
                        SkinSourceError::Prop(format!(
...
fn transform_position(
    ...
    narrow_vector(
        [
            z.mul_add(
                matrix[8],
                y.mul_add(matrix[4], x.mul_add(matrix[0], matrix[12])),
            ),
            z.mul_add(
                matrix[9],
                y.mul_add(matrix[5], x.mul_add(matrix[1], matrix[13])),
            ),
            z.mul_add(
                matrix[10],
                y.mul_add(matrix[6], x.mul_add(matrix[2], matrix[14])),
            ),
        ],
        mesh,
        "position",
...
fn transform_normal(
    ...
    let [x, y, z] = value.map(f64::from);
    let source_length = z.mul_add(z, y.mul_add(y, x * x)).sqrt();
    ...
    narrow_vector(transformed, mesh, "normal")
}
...
fn retained_bone_indices(
    ...
    let by_name = bones
        ...
        .map(|(index, bone)| (bone.id.as_str(), index))
        .collect::<BTreeMap<_, _>>();
    let mut pending = selected_joints.iter().copied().collect::<Vec<_>>();
    let mut retained = BTreeSet::new();
    while let Some(index) = pending.pop() {
        if !retained.insert(index) {
            continue;
        }
        let bone = bones.get(index).ok_or_else(|| {
            SkinSourceError::Prop(format!(
                "selected rigid prop references missing joint \
                             {index}"
            ))
        })?;
        if let Some(parent_id) = bone.parent_id.as_deref() {
            let parent_index =
                by_name.get(parent_id).copied().ok_or_else(|| {
                    SkinSourceError::Prop(format!(
