...
// Boundary-Contract:
// - Owns:
//   - Binary animation outbound adapter.
// - Must-Not:
...
pub(super) fn build_animation_plan(
    ...
    let mut take_nodes = vec![string_node("Current", &active_stack_name)];
    let mut counts = BinaryAnimationCounts::default();
    let mut max_stop_time = 0_i64;
    for (clip_ordinal, clip) in clips.iter().enumerate() {
        let clip_ids = clip_ids(clip_ordinal)?;
        let final_frame = clip
            .frame_count
            .checked_sub(1)
            .ok_or(BinaryAnimationError::TimeOverflow)?;
        let stop_time = frame_time(final_frame, clip.frame_rate)?;
        max_stop_time = max_stop_time.max(stop_time);
        objects.push(animation_stack_node(clip_ids.stack, clip, stop_time)?);
        objects.push(animation_layer_node(clip_ids.layer)?);
        connections.push(object_connection(clip_ids.layer, clip_ids.stack)?);
        counts.stacks += 1;
        counts.layers += 1;
        for (track_ordinal, track) in clip.tracks.iter().enumerate() {
            let bone_ordinal =
                *bone_ordinals.get(track.bone_id.as_str()).ok_or_else(
                    || BinaryAnimationError::UnknownBone(track.bone_id.clone()),
                )?;
            let bone_model = bone_ids(bone_ordinal)
                .map_err(BinaryAnimationError::Identity)?
                .model;
            let ids = track_ids(clip_ordinal, track_ordinal)?;
            let times = key_times(clip)?;
            let translations = translation_values(clip, track_ordinal)?;
            let rotations = rotation_eulers(clip, track_ordinal)?;
            ...
            connections
                .push(object_connection(ids.translation, clip_ids.layer)?);
            connections.push(object_connection(ids.rotation, clip_ids.layer)?);
        }
        take_nodes.push(take_node(clip, stop_time));
    }
    Ok(BinaryAnimationPlan {
        counts,
        objects,
        connections,
        takes: BinaryNode::branch("Takes", take_nodes),
        max_stop_time,
        frame_rate,
        active_stack_name,
    })
...
fn validate_unique_clip_names(
    ...
    for clip in clips {
        if !names.insert(clip.name.as_str()) {
            return Err(BinaryAnimationError::DuplicateClipName(
                clip.name.clone(),
            ));
        }
    }
...
fn shared_frame_rate(
    ...
    if clips
        .iter()
        .any(|clip| !frame_rates_match(clip.frame_rate, first.frame_rate))
    {
...
pub(super) fn key_times(
    ...
    (0..clip.frame_count)
        .map(|frame| frame_time(frame, clip.frame_rate))
        .collect()
}
...
pub(super) fn rotation_eulers(
    ...
    let track = clip
        ...
        .get(track_ordinal)
        .ok_or(BinaryAnimationError::IdOverflow)?;
    let mut rotations = Vec::with_capacity(track.samples.len());
    for sample in &track.samples {
        let matrix = to_row_matrix(sample.rotation_wxyz, sample.translation);
        let mut rotation = decompose(&matrix)
            .map_err(BinaryAnimationError::Matrix)?
            .rotation_degrees;
        if let Some(previous) = rotations.last().copied() {
            unwrap_euler(&mut rotation, previous);
        }
...
fn unwrap_euler(current: &mut [f64; 3], previous: [f64; 3]) {
    for axis in 0..3 {
        while current[axis] - previous[axis] > 180. {
            current[axis] -= 360.;
        }
        while current[axis] - previous[axis] < -180. {
            current[axis] += 360.;
...
fn append_transform_animation(
    ...
    let defaults = values
        ...
        .copied()
        .ok_or(BinaryAnimationError::TimeOverflow)?;
    objects.push(animation_curve_node(node_id, node_name, defaults)?);
    connections.push(property_connection(
        node_id,
        bone_model,
        target_property,
    )?);
    for (axis, property) in ["d|X", "d|Y", "d|Z"].iter().enumerate() {
        let axis_values = values
            .iter()
            .map(|value| checked_f32(value[axis]))
            .collect::<Result<Vec<_>, _>>()?;
        objects.push(animation_curve_node_values(
            curve_ids[axis],
            defaults[axis],
            times,
            axis_values,
...
fn animation_layer_node(id: u64) -> Result<BinaryNode, BinaryAnimationError> {
    Ok(BinaryNode::new(
        ...
        vec![
            id_property(id)?,
            name_class("BaseLayer", "AnimLayer"),
            string(""),
        ],
        Vec::new(),
    ))
...
fn animation_curve_node(
    ...
    Ok(BinaryNode::new(
        ...
        vec![
            id_property(id)?,
            name_class(name, "AnimCurveNode"),
            string(""),
        ],
        vec![BinaryNode::branch("Properties70", vec![
            number_property("d|X", defaults[0]),
            number_property("d|Y", defaults[1]),
            number_property("d|Z", defaults[2]),
        ])],
    ))
}
...
fn animation_curve_node_values(
    ...
    let key_attribute_data = [0., 0., 9.419_963e-30, 0.].repeat(key_count);
    Ok(BinaryNode::new(
        "AnimationCurve",
        vec![id_property(id)?, name_class("", "AnimCurve"), string("")],
        vec![
            BinaryNode::leaf("Default", vec![BinaryProperty::F64(
                default_value,
            )]),
            BinaryNode::leaf("KeyVer", vec![BinaryProperty::I32(
                ANIMATION_KEY_VERSION,
            )]),
            BinaryNode::leaf("KeyTime", vec![BinaryProperty::I64Array(
                times.to_vec(),
            )]),
            BinaryNode::leaf("KeyValueFloat", vec![BinaryProperty::F32Array(
                values,
            )]),
            BinaryNode::leaf("KeyAttrFlags", vec![BinaryProperty::I32Array(
                key_flags,
            )]),
            BinaryNode::leaf("KeyAttrDataFloat", vec![
                BinaryProperty::F32Array(key_attribute_data),
            ]),
            BinaryNode::leaf("KeyAttrRefCount", vec![
                BinaryProperty::I32Array(key_ref_counts),
            ]),
        ],
    ))
}
...
fn take_node(clip: &AnimationClip, stop_time: i64) -> BinaryNode {
    BinaryNode::new("Take", vec![string(&clip.name)], vec![
        string_node("FileName", &format!("{}.tak", clip.name)),
        BinaryNode::leaf("LocalTime", vec![
            BinaryProperty::I64(0),
            BinaryProperty::I64(stop_time),
        ]),
        BinaryNode::leaf("ReferenceTime", vec![
            BinaryProperty::I64(0),
            BinaryProperty::I64(stop_time),
        ]),
    ])
...
fn number_property(name: &str, value: f64) -> BinaryNode {
...
fn name_class(name: &str, class_name: &str) -> BinaryProperty {
...
#[path = "../../../../../tests/formats/fbx/unit/adapter-outbound/binary_animation/loose_tests.rs"]
mod loose_tests;
