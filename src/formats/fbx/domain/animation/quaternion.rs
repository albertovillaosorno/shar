...
pub fn decode_signed_i16_wxyz(values: [u16; 4]) -> Result<[f64; 4], Error> {
    normalize([
        f64::from(i16::from_le_bytes(values[0].to_le_bytes())) / 32_767.,
        f64::from(i16::from_le_bytes(values[1].to_le_bytes())) / 32_767.,
        f64::from(i16::from_le_bytes(values[2].to_le_bytes())) / 32_767.,
        f64::from(i16::from_le_bytes(values[3].to_le_bytes())) / 32_767.,
    ])
}
...
#[must_use]
pub fn slerp(start: [f64; 4], mut end: [f64; 4], amount: f64) -> [f64; 4] {
    let mut dot = start
        .iter()
        .zip(end.iter())
    ...
    let start_weight = ((1. - amount) * theta).sin() / sin_theta;
...
#[must_use]
pub fn to_row_matrix(quaternion: [f64; 4], translation: [f64; 3]) -> [f64; 16] {
...
pub fn from_row_matrix(matrix: &[f64; 16]) -> Result<[f64; 4], Error> {
    ...
    let quaternion = if trace > 0. {
        let scale = (trace + 1.).sqrt() * 2.;
        [
            0.25 * scale,
        ...
        let scale = (1. + matrix[0] - matrix[5] - matrix[10]).sqrt() * 2.;
        [
            (matrix[6] - matrix[9]) / scale,
            0.25 * scale,
            (matrix[1] + matrix[4]) / scale,
        ...
        let scale = (1. + matrix[5] - matrix[0] - matrix[10]).sqrt() * 2.;
        [
            (matrix[8] - matrix[2]) / scale,
            (matrix[1] + matrix[4]) / scale,
            0.25 * scale,
        ...
        let scale = (1. + matrix[10] - matrix[0] - matrix[5]).sqrt() * 2.;
        [
            (matrix[1] - matrix[4]) / scale,
            (matrix[2] + matrix[8]) / scale,
            (matrix[6] + matrix[9]) / scale,
