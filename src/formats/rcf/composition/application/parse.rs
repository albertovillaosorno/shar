...
fn parse_catalog_fields(bytes: &[u8]) -> Result<CatalogFields, ArchiveError> {
    let entry_count =
        usize::try_from(read_u32(bytes, 0)?).map_err(|source| {
            ArchiveError::invalid_archive(format!(
                "entry count does not fit usize: {source}"
            ))
        })?;
    Ok(CatalogFields {
        entry_count,
        name_table_offset: read_u32(bytes, U32_BYTES)?,
        first_file_offset: read_u32(bytes, U32_BYTES.saturating_mul(2))?,
    })
}
...
fn read_catalog_layout(
    ...
    if archive_size < FILE_INFO_BYTES {
        return Err(ArchiveError::invalid_archive(format!(
            "archive is too small: {archive_size} bytes"
        )));
    }

    let file_info = reader.read_exact_range(0, FILE_INFO_BYTES)?;
    validate_magic(&file_info)?;
    validate_version(&file_info)?;
    validate_completion(&file_info)?;
    validate_endianness(&file_info)?;

    let alignment = read_u32(&file_info, FILE_INFO_ALIGNMENT_OFFSET)?;
    let header_start =
        u64::from(read_u32(&file_info, FILE_INFO_HEADER_START_OFFSET)?);
    if header_start < FILE_INFO_BYTES {
        return Err(ArchiveError::invalid_archive(
            "catalog header overlaps file info",
        ));
    }
    ...
    let catalog_payload =
        reader.read_exact_range(header_start, CATALOG_HEADER_BYTES)?;
    let catalog = parse_catalog_fields(&catalog_payload)?;
    let Some(index_bytes) = catalog.entry_count.checked_mul(INDEX_RECORD_BYTES)
    else {
        return Err(ArchiveError::invalid_archive(
            "index table byte count overflow",
        ));
    };
    let index_length = usize_to_u64(index_bytes, "index byte count")?;
    ...
    validate_index_end(index_end, archive_size, catalog.name_table_offset)?;
...
impl ArchiveParser {
    ...
    pub fn from_reader(
        ...
        let layout = read_catalog_layout(reader, archive_size)?;
        let CatalogLayout {
            alignment,
            catalog,
            index_start,
        ...
        let index_payload =
            reader.read_exact_range(index_start, index_length)?;
        let records =
            parse_records(&index_payload, catalog.entry_count, archive_size)?;
        validate_hash_index_order(&records)?;
        let mut records_by_offset = records.clone();
        records_by_offset
            .sort_by_key(|record| (record.offset, record.length, record.hash));
        ...
        validate_records_after_metadata(&records, metadata_end)?;
        ...
        let entries = build_entries(records, names)?;
...
fn usize_to_u64(value: usize, context: &str) -> Result<u64, ArchiveError> {
    u64::try_from(value).map_err(|source| {
        ArchiveError::invalid_archive(format!(
            "{context} does not fit u64: {source}"
        ))
    })
}
...
fn validate_magic(bytes: &[u8]) -> Result<(), ArchiveError> {
    ...
    let is_terminated = bytes.get(MAGIC.len()).copied() == Some(0);
...
fn validate_version(bytes: &[u8]) -> Result<(), ArchiveError> {
    let major =
        read_u8(bytes, FILE_INFO_VERSION_MAJOR_OFFSET, "major version")?;
    let minor =
        read_u8(bytes, FILE_INFO_VERSION_MINOR_OFFSET, "minor version")?;
    if major != FORMAT_VERSION_MAJOR || minor != FORMAT_VERSION_MINOR {
        return Err(ArchiveError::invalid_archive(format!(
            "unsupported RCF version {major}.{minor}"
        )));
    }
    Ok(())
}
...
fn validate_completion(bytes: &[u8]) -> Result<(), ArchiveError> {
    let valid = read_u8(bytes, FILE_INFO_VALID_OFFSET, "valid flag")?;
    if valid != 1 {
        return Err(ArchiveError::invalid_archive(format!(
            "unsupported RCF valid flag: {valid}"
        )));
    }
    Ok(())
}
...
fn validate_endianness(bytes: &[u8]) -> Result<(), ArchiveError> {
    let big_endian =
        read_u8(bytes, FILE_INFO_BIG_ENDIAN_OFFSET, "big-endian flag")?;
    if big_endian != 0 {
        return Err(ArchiveError::invalid_archive(format!(
            "unsupported big-endian RCF flag: {big_endian}"
        )));
    }
    Ok(())
}
...
fn read_u8(
    ...
    bytes.get(offset).copied().ok_or_else(|| {
        ArchiveError::invalid_archive(format!(
            "{field} byte is outside the file info"
        ))
    })
}
...
fn read_u32(bytes: &[u8], offset: usize) -> Result<u32, ArchiveError> {
    ...
    let slice = bytes.get(offset..end).ok_or_else(|| {
        ArchiveError::invalid_archive(format!(
            "u32 read exceeds buffer at offset {offset}"
        ))
    })?;
    Ok(u32::from_le_bytes(slice.try_into().map_err(|source| {
        ArchiveError::invalid_archive(format!(
...
fn validate_index_end(
    ...
    if index_end > u64::from(name_table_offset) {
        return Err(ArchiveError::invalid_archive(format!(
            concat!("index end 0x{:x} exceeds ", "name table 0x{:x}"),
...
fn parse_records(
    ...
    for index in 0..entry_count {
        let offset =
            index.checked_mul(INDEX_RECORD_BYTES).ok_or_else(|| {
                ArchiveError::invalid_archive("record offset overflow")
            })?;
        let hash = read_u32(bytes, offset)?;
        let payload_offset = u64::from(read_u32(
            bytes,
            offset.checked_add(U32_BYTES).ok_or_else(|| {
                ArchiveError::invalid_archive("record offset overflow")
            })?,
        )?);
        let length = u64::from(read_u32(
            bytes,
            offset.checked_add(U32_BYTES.saturating_mul(2)).ok_or_else(
                || ArchiveError::invalid_archive("record offset overflow"),
            )?,
        )?);
        let payload_end =
            payload_offset.checked_add(length).ok_or_else(|| {
                ArchiveError::invalid_archive(format!(
                    "entry {index} payload range overflows"
                ))
            })?;
        if payload_end > archive_size {
            return Err(ArchiveError::invalid_archive(format!(
                "entry {index} range exceeds archive size"
            )));
        }
...
fn validate_records_do_not_overlap(
    ...
    for record in records {
        ...
        previous_end =
            record.offset.checked_add(record.length).ok_or_else(|| {
...
fn validate_records_after_metadata(
    ...
    if records.iter().any(|record| record.offset < metadata_end) {
        return Err(ArchiveError::invalid_archive(
            "payload overlaps archive metadata",
        ));
    }
    Ok(())
}
...
fn validate_first_file_hint(
    ...
    if !is_aligned {
        return Err(ArchiveError::invalid_archive(
            "first file hint is not aligned",
        ));
    }
    Ok(())
}
...
fn read_name_count(
    ...
    let count_bytes =
        reader.read_exact_range(name_table_offset, U32_BYTES_U64)?;
    usize::try_from(read_u32(&count_bytes, 0)?).map_err(|source| {
        ArchiveError::invalid_archive(format!(
            "name table count does not fit usize: {source}"
        ))
    })
}
...
fn parse_detailed_name_record(
    ...
    let name_length_bytes =
        reader.read_exact_range(cursor, NAME_LENGTH_BYTES)?;
    let name_len = usize::try_from(read_u32(&name_length_bytes, 0)?).map_err(
        |source| {
            ArchiveError::invalid_archive(format!(
                "entry {index} name length invalid: {source}"
            ))
        },
    )?;
    let name_start =
        cursor.checked_add(NAME_LENGTH_BYTES).ok_or_else(|| {
            ArchiveError::invalid_archive("name record cursor overflow")
        })?;
    let name_len_u64 = u64::try_from(name_len).map_err(|source| {
        ArchiveError::invalid_archive(format!(
            "name length does not fit u64: {source}"
        ))
    })?;
    let name_end = name_start.checked_add(name_len_u64).ok_or_else(|| {
        ArchiveError::invalid_archive("name range end overflow")
    })?;
    if name_len == 0 || name_end > archive_size {
        return Err(ArchiveError::invalid_archive(format!(
            "entry {index} name range is not valid"
        )));
    }
    let raw_name = reader.read_exact_range(name_start, name_len_u64)?;
    let modification_time_bytes =
        reader.read_exact_range(name_end, MODIFICATION_TIME_BYTES)?;
    let modification_time = read_u32(&modification_time_bytes, 0)?;
    let next_offset = name_end
        .checked_add(MODIFICATION_TIME_BYTES)
        .ok_or_else(|| {
            ArchiveError::invalid_archive("name record cursor overflow")
        })?;
    Ok(DetailedNameRecord {
        modification_time,
        name: decode_name(&raw_name, index)?,
        next_offset,
    })
}
...
fn parse_names(
    ...
    archive_size: u64,
) -> Result<(Vec<(u32, String)>, u64), ArchiveError> {
    ...
    let actual_count = read_name_count(reader, name_table_offset)?;
    ...
    for index in 0..expected_count {
        let record =
            parse_detailed_name_record(reader, cursor, index, archive_size)?;
        cursor = record.next_offset;
        names.push((record.modification_time, record.name));
    }
    Ok((names, cursor))
}
...
fn decode_name(raw_name: &[u8], index: usize) -> Result<String, ArchiveError> {
    let trimmed = raw_name.strip_suffix(&[0]).ok_or_else(|| {
        ArchiveError::invalid_archive("entry name is not NUL-terminated")
    })?;
    if trimmed.is_empty() {
        return Err(ArchiveError::invalid_archive(format!(
            "entry {index} decoded to an empty name"
        )));
    }
    if trimmed.contains(&0) {
        return Err(ArchiveError::invalid_archive(format!(
            "entry {index} name contains an embedded NUL"
        )));
    }
    let decoded = std::str::from_utf8(trimmed).map_err(|source| {
        ArchiveError::invalid_archive(format!(
            "entry {index} name is not UTF-8: {source}"
        ))
    })?;
    if decoded.chars().any(char::is_control) {
        return Err(ArchiveError::invalid_archive(format!(
            "entry {index} name contains a control character"
        )));
    }
    if contains_unsafe_unicode_path_control(decoded) {
        return Err(ArchiveError::invalid_archive(format!(
            "entry {index} name contains a Unicode path control"
        )));
    }
    Ok(decoded.to_owned())
}
...
fn name_hash32(text: &str) -> u32 {
    ...
    for item in text.bytes() {
        ...
        value = value.wrapping_mul(31).wrapping_add(u32::from(adjusted));
    }
    value
}
...
fn build_entries(
    ...
    for record in records {
        if table.insert(record.hash, record).is_some() {
            return Err(ArchiveError::invalid_archive(
                "repeated archive lookup value",
            ));
        }
    }
    ...
    for (modification_time, stored_name) in names {
        ...
        let record = table.remove(&lookup).ok_or_else(|| {
            ArchiveError::invalid_archive(format!(
                "missing archive lookup for {stored_name}"
            ))
        })?;
        ...
        if !output_identities.insert(output_identity) {
            return Err(ArchiveError::invalid_archive(format!(
                "duplicate output path: {output_name}"
            )));
        }
...
#[cfg(test)]
#[path = "../../../../../tests/formats/rcf/unit/application/parse/tests.rs"]
mod tests;
