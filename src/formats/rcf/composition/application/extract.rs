...
impl ExtractionReport {
    ...
    pub const fn overhead_bytes(&self) -> u64 {
        self.archive_bytes.saturating_sub(self.extracted_bytes)
    }
...
impl Extractor {
    ...
    pub fn extract(
        ...
        sink.prepare_archive(&archive_stem, &archive.entries)?;
        for entry in &archive.entries {
            let payload =
                reader.read_exact_range(entry.offset, entry.length)?;
            let output_path =
                sink.write_entry(&archive_stem, &entry.name, &payload)?;
            observer.entry_extracted(&entry_record(entry), &output_path)?;
        }
        Ok(ExtractionReport {
            archive_stem,
