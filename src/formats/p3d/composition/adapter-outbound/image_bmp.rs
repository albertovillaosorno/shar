...
pub(super) fn looks_like_bmp(payload: &[u8]) -> bool {
    ...
    let dib_size_supported =
        matches!(header.dib_size, 12 | 40 | 52 | 56 | 64 | 108 | 124);
    dib_size_supported
        && bmp_dimensions_are_supported(payload, header.dib_size)
        && bmp_plane_count_is_supported(payload, header.dib_size)
        && bmp_pixel_depth_is_supported(payload, header.dib_size)
        && bmp_compression_is_supported(payload, header.dib_size)
        && bmp_image_size_is_supported(payload, &header)
...
fn bmp_image_size_is_supported(payload: &[u8], header: &BmpHeader) -> bool {
    if header.dib_size == 12 {
        return true;
    }
    let Some(image_size_u32) = read_u32_at(payload, 34) else {
        return false;
    };
    let Ok(image_size) = usize::try_from(image_size_u32) else {
        return false;
    };
    let Some(compression) = read_u32_at(payload, 30) else {
        return false;
    };
    if matches!(compression, 1_u32 | 2 | 4 | 5) && image_size == 0 {
        return false;
    }
    let available_size_result =
        header.file_size.checked_sub(header.pixel_offset);
...
fn bmp_compression_is_supported(payload: &[u8], dib_size: u32) -> bool {
    if dib_size == 12 {
        return true;
    }
    let Some(compression) = read_u32_at(payload, 30) else {
    ...
    let Some(pixel_depth) = read_u16_at(payload, 28) else {
        return false;
    };
    let Some(height) = read_i32_at(payload, 22) else {
        return false;
    };
    if height < 0_i32 && !matches!(compression, 0_u32 | 3) {
        return false;
    }
    match compression {
        0 | 4 | 5 => true,
        1 => pixel_depth == 8,
        2 => pixel_depth == 4,
        3 => matches!(pixel_depth, 16 | 32),
        _ => false,
    }
}

/// Returns whether one BMP DIB header uses a supported pixel depth.
fn bmp_pixel_depth_is_supported(payload: &[u8], dib_size: u32) -> bool {
    let depth_offset = if dib_size == 12 {
        24
    } else {
        28
    };
    let Some(pixel_depth) = read_u16_at(payload, depth_offset) else {
        return false;
    };
    matches!(pixel_depth, 1 | 4 | 8 | 16 | 24 | 32)
}

/// Returns whether one BMP DIB header declares exactly one color plane.
fn bmp_plane_count_is_supported(payload: &[u8], dib_size: u32) -> bool {
    let plane_offset = if dib_size == 12 {
        22
    } else {
        26
    };
    let Some(plane_count) = read_u16_at(payload, plane_offset) else {
        return false;
    };
    plane_count == 1
}
...
fn bmp_dimensions_are_supported(payload: &[u8], dib_size: u32) -> bool {
    if dib_size == 12 {
        let Some(width) = read_u16_at(payload, 18) else {
            return false;
        };
        let Some(height) = read_u16_at(payload, 20) else {
            return false;
        };
        return width > 0 && height > 0;
    }
    let Some(width) = read_i32_at(payload, 18) else {
        return false;
    };
    let Some(height) = read_i32_at(payload, 22) else {
        return false;
...
fn parse_bmp_header(payload: &[u8]) -> Option<BmpHeader> {
    ...
    let file_size_u32 = read_u32_at(payload, 2)?;
    let pixel_offset_u32 = read_u32_at(payload, 10)?;
    let dib_size = read_u32_at(payload, 14)?;
...
fn read_u16_at(payload: &[u8], offset: usize) -> Option<u16> {
