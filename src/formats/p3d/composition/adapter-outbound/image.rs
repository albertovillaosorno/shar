...
const PNG_IHDR_LENGTH_BYTES: [u8; 4] = [0, 0, 0, 13];
/// Four-byte zero value rejected for PNG width and height.
const PNG_ZERO_DIMENSION_BYTES: [u8; 4] = [0, 0, 0, 0];
...
fn looks_like_png(payload: &[u8]) -> bool {
    let has_ihdr_length = has_bytes_at(payload, 8, &PNG_IHDR_LENGTH_BYTES);
    let has_ihdr_type = has_bytes_at(payload, 12, b"IHDR");
    let has_width = !has_bytes_at(payload, 16, &PNG_ZERO_DIMENSION_BYTES);
    let has_height = !has_bytes_at(payload, 20, &PNG_ZERO_DIMENSION_BYTES);
    payload.len() >= 33
...
fn looks_like_dds(payload: &[u8]) -> bool {
    payload.len() >= 128
        ...
        && has_bytes_at(payload, 4, b"|\x00\x00\x00")
        && has_bytes_at(payload, 76, b" \x00\x00\x00")
        && !has_bytes_at(payload, 12, b"\x00\x00\x00\x00")
        && !has_bytes_at(payload, 16, b"\x00\x00\x00\x00")
}

/// Returns whether one exact byte sequence begins at a checked payload offset.
fn has_bytes_at(payload: &[u8], offset: usize, expected: &[u8]) -> bool {
...
fn looks_like_tga(payload: &[u8]) -> bool {
    payload.len() > 18
        && payload.get(2).is_some_and(|image_type| {
            matches!(*image_type, 1 | 2 | 3 | 9 | 10 | 11)
        })
        && !has_bytes_at(payload, 12, b"\x00\x00")
        && !has_bytes_at(payload, 14, b"\x00\x00")
...
fn tga_color_map_flag_is_supported(payload: &[u8]) -> bool {
    ...
    match *image_type {
        1 | 9 => {
            *color_map_type == 1
                && !has_bytes_at(payload, 5, b"\x00\x00")
...
fn tga_palette_depth_is_supported(payload: &[u8]) -> bool {
    payload
        .get(7)
        .is_some_and(|entry_depth| matches!(*entry_depth, 15 | 16 | 24 | 32))
}
...
fn tga_pixel_depth_is_supported(payload: &[u8]) -> bool {
    ...
    match *image_type {
        1 | 9 => matches!(*pixel_depth, 8 | 16),
        2 | 10 => matches!(*pixel_depth, 15 | 16 | 24 | 32),
        3 | 11 => matches!(*pixel_depth, 8 | 16),
        _ => false,
    }
}
