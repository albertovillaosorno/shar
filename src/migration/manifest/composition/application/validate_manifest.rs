...
// Boundary-Contract:
// - Owns:
//   - Validate manifest application service.
// - Must-Not:
...
type Requirement = (DirCount, Coordinate);
...
fn validate_record(
    ...
    let expected_kind =
        classify_manifest_bucket(&record.dir, &record.extension);
    ...
    let key = (record.dir.clone(), record.extension.clone());
...
fn parse_manifest(manifest: &str) -> Result<Vec<Requirement>, String> {
    ...
    for (line_offset, line) in lines.enumerate() {
        ...
        let key =
            validate_record(&record, line_number, &mut seen, &mut previous)?;
        requirements.push((record, key));
    }
...
fn compare_requirements(
    ...
    for (record, key) in requirements {
        ...
        if have < record.min_count {
            let location = if record.dir.is_empty() {
                "<root>".to_owned()
            } else {
                record.dir.clone()
            };
