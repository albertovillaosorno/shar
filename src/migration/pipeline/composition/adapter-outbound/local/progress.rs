...
impl ProgressState {
    ...
    const fn verbosity(&self) -> Verbosity {
        match self {
            Self::Installed { verbosity, .. } => *verbosity,
        }
    }
...
pub(in crate::adapters) fn install(
    ...
    let log = log_file.map(RunLog::open).transpose()?.map(Mutex::new);
    STATE
        .set(ProgressState::Installed { verbosity, log })
...
impl StageProgress {
    ...
    #[must_use]
    pub(super) fn begin(stage: impl Into<String>, total: usize) -> Self {
        Self::start(stage.into(), Some(total))
    }
    ...
    pub(super) fn begin_unknown(stage: impl Into<String>) -> Self {
        Self::start(stage.into(), None)
    }

    /// Construct and announce one stage reporter.
    fn start(stage: String, total: Option<usize>) -> Self {
        let now = Instant::now();
        if STATE.get().is_some() {
            match total {
                Some(item_total) => {
                    terminal::line(&format!(
                        "[{stage}] starting: {item_total} items"
                    ));
                },
                None => {
                    terminal::line(&format!(
                        "[{stage}] starting: discovering items"
                    ));
                },
            }
            append_event(&format!(
        ...
        drop(update_current_progress(&stage, Some(0), total, None));
        Self {
            stage,
            total,
            done: 0,
