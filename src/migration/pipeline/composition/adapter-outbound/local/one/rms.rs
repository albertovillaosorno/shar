...
/// Bytes to json.
fn bytes_to_json(bytes: &[u8], source: &str) -> String {
    let words = le_u32_words(bytes);
    let symbols = ascii_strings(bytes);
    let byte_hex = hex_bytes(bytes);
    let offset_refs = offset_references_json(&words, bytes, &symbols);
    let symbol_json = symbols_json(&symbols);
    let schema_terms = filtered_symbols_json(&symbols, SymbolClass::SchemaTerm);
    let named_assets = filtered_symbols_json(&symbols, SymbolClass::NamedAsset);
    let regions = filtered_symbols_json(&symbols, SymbolClass::Region);
    let placeholders =
        filtered_symbols_json(&symbols, SymbolClass::Placeholder);
    let rsd_refs = filtered_symbols_json(&symbols, SymbolClass::RsdReference);
    format!(
        concat!(
...
"##,
        ...
        json_escape(source),
        json_escape(symbols.first().map_or("", |symbol| symbol.value.as_str())),
        schema_classes_json(),
        bytes.len(),
...
fn filtered_symbols_json(symbols: &[Symbol], class: SymbolClass) -> String {
    symbols
        .iter()
        .filter(|symbol| symbol_matches(symbol, class))
        .map(symbol_json)
        .collect::<Vec<_>>()
        .join(",")
}
...
/// Symbol matches.
fn symbol_matches(symbol: &Symbol, class: SymbolClass) -> bool {
    let value = symbol.value.as_str();
    match class {
        SymbolClass::SchemaTerm => is_radmusic_schema_term(value),
        SymbolClass::NamedAsset => {
            !is_radmusic_schema_term(value)
                && !is_placeholder(value)
                && !value.to_ascii_lowercase().ends_with(".rsd")
        },
        SymbolClass::Region => value.to_ascii_lowercase().ends_with("_region"),
...
fn le_u32_words(bytes: &[u8]) -> Vec<u32> {
    ...
    while cursor.saturating_add(4) <= bytes.len() {
        ...
        let word_bytes = chunk.try_into().unwrap_or([0_u8; 4]);
            ...
            .is_some_and(|byte| byte.is_ascii_graphic() || *byte == b' ')
        {
            ...
            if cursor.saturating_sub(start) >= 4 {
                let value = String::from_utf8_lossy(
                    bytes.get(start..cursor).unwrap_or_default(),
                )
                .into_owned();
                strings.push(Symbol { offset: start, value });
            }
        }
        cursor = cursor.saturating_add(1);
    }
...
fn hex_bytes(bytes: &[u8]) -> String {
    ...
    for byte in bytes {
        output.push(char::from(
            *HEX.get(usize::from(byte >> 4_u8)).unwrap_or(&b'0'),
        ));
        output.push(char::from(
            *HEX.get(usize::from(byte & 15)).unwrap_or(&b'0'),
        ));
    }
    output
}
...
#[path = "../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/one/rms/tests.rs"]
mod tests;
