...
impl RcfProgressObserver {
    ...
    fn begin(archive_ordinal: usize) -> Self {
        Self {
            progress: StageProgress::begin_unknown(format!(
                "rcf archive {archive_ordinal} entries"
            )),
        }
    }
...
fn verify_manifest(
    ...
    let mut progress = StageProgress::begin("manifest rules", rules.len());
    for rule in &rules {
        check_cancellation()?;
        progress.advance(&format!("{}/.{}", rule.dir, rule.extension,));
        let actual = counts
            .get(&(rule.dir.clone(), rule.extension.clone()))
            .copied()
            .unwrap_or_default();
    ...
    fs::write(&report_path, out).map_err(io_error(&report_path))?;
    if failures > 0 {
        return Err(PipelineError::new(format!(
...
fn convert_readme(
    ...
    write_bytes(&output, markdown.as_bytes())?;
    Ok(StageReport {
        name: "readme",
        files: 1,
...
fn extract_rcf(
    ...
    if extracted_root.join("scripts").exists()
        && (extracted_root.join("dialog").exists()
            || extracted_root.join("art").exists())
    {
    ...
    let archives = files_with_extension(game_root, "rcf")?;
    let mut progress = StageProgress::begin("rcf archives", archives.len());
    ...
    for (index, archive) in archives.into_iter().enumerate() {
        ...
        let report = Extractor::extract(&source, &mut sink, &mut observer)
            .map_err(|error| {
...
fn convert_rsd(
    ...
    let game_inputs = files_with_extension(game_root, "rsd")?;
    let extracted_inputs = files_with_extension(extracted_root, "rsd")?;
    let mut progress = StageProgress::begin(
        "rsd audio",
        game_inputs.len().saturating_add(extracted_inputs.len()),
    );
    let mut files = 0usize;
    let mut bytes = 0u64;
    for input in game_inputs {
        check_cancellation()?;
        progress.advance(&progress_item(game_root, &input));
        let relative = input.strip_prefix(game_root).map_err(|_error| {
            PipelineError::new("failed to relativize game RSD")
        })?;
        let output = extracted_root.join(relative).with_extension("wav");
        let written = convert_one_rsd(&input, &output)?;
        files = files.saturating_add(1);
        bytes = bytes.saturating_add(written);
    }
    for input in extracted_inputs {
        check_cancellation()?;
        progress.advance(&progress_item(extracted_root, &input));
        if !input.exists() {
            continue;
        }
...
/// Convert one rsd.
fn convert_one_rsd(input: &Path, output: &Path) -> PipelineOutcome<u64> {
    ...
    let audio = RsdAudio::parse(&bytes).map_err(|error| {
        PipelineError::new(format!("{}: {error}", input.display()))
    })?;
    let wav = audio.to_wav().map_err(|error| {
        PipelineError::new(format!("{}: {error}", input.display()))
    })?;
    let wav_bytes = wav.to_bytes().map_err(|error| {
        PipelineError::new(format!("{}: {error}", input.display()))
    })?;
    write_bytes(output, &wav_bytes)?;
...
fn export_movie_with_ffmpeg(
    ...
    for index in 0..audio_tracks {
        let output_wav = plan
            .movie_directory
            .join(format!("audio_track_{:02}.wav", index.saturating_add(1)));
...
fn movie_outputs_complete(plan: &UnrealHapPackagePlan) -> bool {
    ...
    required
        ...
        && local_file_len(&plan.movie_directory.join("audio_track_01.wav"))
...
fn parse_frame_rate_fraction(value: &str) -> f64 {
    let Some((numerator_text, denominator_text)) = value.split_once('/') else {
        return value.parse::<f64>().unwrap_or_default();
    };
    let numerator = numerator_text.parse::<f64>().unwrap_or_default();
    let denominator = denominator_text.parse::<f64>().unwrap_or(1f64);
    if denominator == 0. {
        0.
    } else {
...
"
    ...
    write_bytes(&output.join("timing.tsv"), timing.as_bytes())
}
...
fn extract_movies(
    ...
    let report =
        auditor
            .audit_roots(&roots, extracted_root)
            .map_err(|error| {
                PipelineError::new(format!("movie audit failed: {error}"))
            })?;
    let selected = select_logical_movies(game_root, &report.records)?;
    let stale_user_root = extracted_root.join("movies").join("user");
    ...
    let mut progress = StageProgress::begin("movies", selected.len());
    let mut files = 0usize;
    let mut bytes = 0u64;
    for (logical_relative, record) in selected {
        ...
        .map_err(|error| {
            PipelineError::new(format!(
                "movie plan failed for {movie_stem}: {error}"
            ))
        })?;
        write_movie_package_plan(game_root, record, &logical_relative, &plan)?;
        if is_movie_decodable_by_ffmpeg(record.kind) {
            export_movie_with_ffmpeg(record, &plan)?;
...
fn write_movie_package_plan(
    ...
    let legacy_audio_dir = plan.movie_directory.join("audio");
    ...
    let legacy_mezzanine = plan.movie_directory.join("mezzanine.mov");
...
"
    ...
    write_bytes(&plan.timing_manifest_path, timing.as_bytes())?;
    Ok(())
}
...
fn select_logical_movies<'a>(
    ...
    for record in records {
        let logical = logical_movie_relative(game_root, &record.source_path)?;
        let replace = selected.get(&logical).is_none_or(|existing| {
            movie_priority(record) > movie_priority(existing)
        });
        if replace {
            let _previous_record = selected.insert(logical, record);
        }
    }
    Ok(selected)
}
...
fn logical_movie_relative(
    ...
    if let [movies, user, name] = parts.as_slice()
        && movies.as_os_str().to_str() == Some("movies")
        && user.as_os_str().to_str() == Some("user")
    {
...
/// Movie priority.
fn movie_priority(record: &MovieRecord) -> (u8, u64) {
    let user_supersede = record
        .relative_path
        .components()
        .any(|component| component.as_os_str().to_str() == Some("user"));
...
fn extract_p3d(
    ...
    let game_inputs = files_with_extension(game_root, "p3d")?;
    let extracted_inputs = files_with_extension(extracted_root, "p3d")?;
    let mut progress = StageProgress::begin(
        "p3d packages",
        game_inputs.len().saturating_add(extracted_inputs.len()),
    );
    ...
    for input in game_inputs {
        ...
        progress.advance(&progress_item(game_root, &input));
        let relative = input.strip_prefix(game_root).map_err(|_error| {
            PipelineError::new("failed to relativize game P3D")
        })?;
        let output = extracted_root.join(path_without_extension(relative));
        if p3d_package_complete(&output) {
            files = files.saturating_add(1);
            let input_bytes =
                fs::metadata(&input).map_err(io_error(&input))?.len();
            bytes = bytes.saturating_add(input_bytes);
            continue;
        }
        normalize_p3d_file(&input, &output)?;
        files = files.saturating_add(1);
        let input_bytes = fs::metadata(&input).map_err(io_error(&input))?.len();
        bytes = bytes.saturating_add(input_bytes);
    }
    for input in extracted_inputs {
        check_cancellation()?;
        ...
        if input.file_name().and_then(|value| value.to_str())
            == Some("source.p3d")
        {
    ...
    files
        ...
        .any(|path| {
            validate_generated_text_file(path).is_err()
                || fs::read_to_string(path).map_or(true, |text| {
...
fn component_ledger_files_exist(output: &Path) -> bool {
    ...
    if extract_json_string_field(header, "schema").as_deref()
        != Some("p3d.package.v1")
    {
        return false;
    }
    let component_lines = lines.collect::<Vec<_>>();
    let Some(component_count) =
        extract_json_usize_field(header, "component_count")
    else {
        return false;
    };
    ...
    for line in component_lines {
        ...
        let Ok(resolved) = resolve_under(&components, relative) else {
            return false;
        };
...
fn extract_json_usize_field(line: &str, key: &str) -> Option<usize> {
    let value = find_unique_top_level_json_value(line, key)?.trim();
...
fn extract_json_string_field(line: &str, key: &str) -> Option<String> {
    let value = find_unique_top_level_json_value(line, key)?;
    let (decoded, cursor) = parse_component_json_string(value, 0)?;
...
fn find_unique_top_level_json_value<'a>(
    ...
    let mut found = None;
    loop {
        cursor = skip_json_whitespace(trimmed_line, cursor);
        if bytes.get(cursor) == Some(&b'}') {
            return (cursor == bytes.len().saturating_sub(1))
                .then_some(found)?;
        }
        let (field, next) = parse_component_json_string(trimmed_line, cursor)?;
        cursor = skip_json_whitespace(trimmed_line, next);
        if bytes.get(cursor) != Some(&b':') {
            return None;
        }
        let value_start =
            skip_json_whitespace(trimmed_line, cursor.saturating_add(1));
        let value_end = skip_component_json_value(trimmed_line, value_start)?;
        if field == key {
            if found.is_some() {
                return None;
            }
        ...
        cursor = skip_json_whitespace(line, value_end);
...
fn skip_component_json_value(line: &str, start: usize) -> Option<usize> {
    if line.as_bytes().get(start) == Some(&b'"') {
        return parse_component_json_string(line, start).map(|(_, next)| next);
    }
    let bytes = line.as_bytes();
    let mut cursor = start;
    let container_value = matches!(bytes.get(start), Some(b'{' | b'['));
...
fn json_primitive_is_valid(value: &str) -> bool {
    matches!(value, "true" | "false" | "null") || json_number_is_valid(value)
}
...
fn has_raw_component_output(components: &Path) -> bool {
    ...
    files.iter().any(|path| is_raw_component_path(path))
}

/// Is raw component path.
fn is_raw_component_path(path: &Path) -> bool {
    let has_raw_segment = path.components().any(|component| {
        component
            .as_os_str()
            .to_str()
...
/// Normalize p3d file.
fn normalize_p3d_file(input: &Path, output: &Path) -> PipelineOutcome<()> {
    ...
    p3d::write_lossless_package(input, output).map_err(|error| {
        PipelineError::new(format!("{}: {error}", input.display()))
    })?;
...
fn is_p3d_diff_path(path: &Path) -> bool {
    path.components().any(|component| {
        component
            .as_os_str()
            .to_str()
            .is_some_and(|value| value.eq_ignore_ascii_case("P3D_Diffs"))
    })
}
...
fn assert_normalized(extracted_root: &Path) -> PipelineOutcome<StageReport> {
    ...
    let mut progress =
        StageProgress::begin("normalized output audit", extracted_files.len());
    let mut bad = Vec::new();
    for file in &extracted_files {
        check_cancellation()?;
        progress.advance(&progress_item(extracted_root, file));
    ...
    if !bad.is_empty() {
        return Err(PipelineError::new(format!(
            ...
            bad.first().map_or_else(
                || Path::new("<none>").display().to_string(),
...
fn write_pipeline_report(
    ...
    fs::write(&path, jsonl).map_err(io_error(&path))
}
...
fn progress_item(root: &Path, path: &Path) -> String {
...
fn relative_manifest_path_text(
    ...
    let relative = if input.starts_with(game_root) {
        input.strip_prefix(game_root).map_err(|_error| {
            PipelineError::new("failed to relativize game path")
        })?
    } else {
        input.strip_prefix(extracted_root).map_err(|_error| {
...
fn normalize_sound_scripts(
    ...
    let spt_inputs = files_with_extension(extracted_root, "spt")?;
    let rms_inputs = files_with_extension(extracted_root, "rms")?;
    let mut progress = StageProgress::begin(
        "sound scripts",
        spt_inputs.len().saturating_add(rms_inputs.len()),
    );
    let mut files = 0usize;
    let mut bytes = 0u64;
