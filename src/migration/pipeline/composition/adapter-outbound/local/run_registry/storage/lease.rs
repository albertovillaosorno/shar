...
impl RegistryStorage {
    ...
    pub(in super::super) fn request_cancel(
        ...
        match OpenOptions::new().write(true).create_new(true).open(&path) {
            Ok(mut file) => file.write_all(b"cancel\n").map_err(|error| {
    ...
    pub(in super::super) fn cancellation_requested(
        ...
        self.run_dir(run_id).join(CANCEL_FILE).is_file()
    }
    ...
    pub(in super::super) fn acquire_exclusive(
        ...
        validate_run_id(run_id)?;
        self.prepare()?;
        let path = self.root.join(EXCLUSIVE_LOCK);
        ...
        Err(String::from(
            "pipeline exclusive lease could not be recovered",
        ))
    }
    ...
    fn remove_abandoned_exclusive(
        ...
        if self.run_dir(owner.trim()).is_dir() {
            return Ok(false);
        }
        ...
        let age_ms = u64::try_from(age.as_millis()).map_err(|error| {
            format!("pipeline exclusive lease age overflowed: {error}")
        })?;
    ...
    pub(in super::super) fn release_exclusive(
        ...
        let owner = match fs::read_to_string(&path) {
            ...
            Err(error) => {
                return Err(format!(
                    "pipeline exclusive lease read failed: {error}"
                ));
            },
        };
        ...
        fs::remove_file(&path).map_err(|error| {
            format!("pipeline exclusive lease release failed: {error}")
        })
    }
}
