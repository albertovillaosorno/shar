...
impl RunSnapshot {
    ...
    pub(in super::super) fn render(&self, now_unix_ms: u64) -> String {
        let elapsed_seconds = now_unix_ms
            .saturating_sub(self.started_unix_ms)
            .checked_div(1_000)
            .unwrap_or_default();
        let progress = match (self.done, self.total) {
        ...
        let eta = self
            .eta_seconds
            .map_or_else(|| String::from("unknown"), format_duration);
        let label = self
            .label
            .as_deref()
            .map_or_else(|| String::from("-"), safe_text);
        let item = self
            .item
            .as_deref()
            .map_or_else(|| String::from("-"), safe_text);
...
fn format_duration(seconds: u64) -> String {
    let hours = seconds.checked_div(3_600).unwrap_or_default();
    let minutes = seconds
        .checked_rem(3_600)
        .unwrap_or_default()
        .checked_div(60)
        .unwrap_or_default();
    let remainder = seconds.checked_rem(60).unwrap_or_default();
    if hours > 0 {
