...
impl RunSnapshot {
    ...
    pub(in super::super) fn parse(bytes: &[u8]) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            mode,
            state,
            started_unix_ms: integer_field(&value, "started_unix_ms")?,
            heartbeat_unix_ms: integer_field(&value, "heartbeat_unix_ms")?,
            stage_started_unix_ms: integer_field(
                &value,
                "stage_started_unix_ms",
            )?,
            stage: string_field(&value, "stage")?.to_owned(),
            done: optional_integer_field(&value, "done")?,
            total: optional_integer_field(&value, "total")?,
            item: optional_string_field(&value, "item")?,
            eta_seconds: optional_integer_field(&value, "eta_seconds")?,
        })
    }
