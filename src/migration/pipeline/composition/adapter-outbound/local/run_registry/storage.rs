...
#![expect(
    ...
    reason = "Single-directory creation is required to detect collisions and preserve atomic run ownership."
)]
...
impl RegistryStorage {
    ...
    pub(super) fn active_runs(
        ...
        let entries = fs::read_dir(self.runs_root()).map_err(|error| {
            format!("pipeline runtime enumeration failed: {error}")
        })?;
        for entry_result in entries {
            let entry = entry_result.map_err(|error| {
                format!("pipeline runtime entry read failed: {error}")
            })?;
            let file_type = entry.file_type().map_err(|error| {
                format!("pipeline runtime entry metadata failed: {error}")
            })?;
            if !file_type.is_dir() {
                continue;
            }
            ...
            if snapshot.is_stale(now_unix_ms, STALE_AFTER_MS) {
    ...
    pub(super) fn create_run(
        ...
        fs::rename(&pending, &run_dir).map_err(|error| {
            drop(fs::remove_dir_all(&pending));
    ...
    pub(super) fn remove_run(&self, run_id: &str) -> Result<(), String> {
        validate_run_id(run_id)?;
        match fs::remove_dir_all(self.run_dir(run_id)) {
            Ok(()) => Ok(()),
            Err(error) if error.kind() == ErrorKind::NotFound => Ok(()),
            Err(error) => Err(format!(
                "pipeline run cleanup failed for {run_id}: {error}"
            )),
        }
    }
    ...
    fn read_snapshot(run_dir: &Path) -> Result<RunSnapshot, String> {
        ...
        for attempt in 0_u8..STATE_READ_ATTEMPTS {
            match fs::read(&path) {
                ...
                Err(error) => {
                    return Err(format!(
                        "pipeline run state read failed: {error}"
                    ));
                },
            }
        }
