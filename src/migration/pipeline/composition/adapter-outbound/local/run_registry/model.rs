...
impl RunSnapshot {
    ...
    pub(super) const fn heartbeat(&mut self, now_unix_ms: u64) {
        self.heartbeat_unix_ms = now_unix_ms;
    }

    /// Mark a cooperative cancellation request.
    pub(super) const fn request_cancellation(&mut self, now_unix_ms: u64) {
...
fn estimate_eta_seconds(
    ...
    u64::try_from(remaining_ms).ok()?.checked_div(1_000)
}
