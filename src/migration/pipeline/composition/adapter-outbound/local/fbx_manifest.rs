...
impl PhaseThreeFbxManifest {
    ...
    pub(super) fn from_plan(plan: &FbxModelPlan) -> Self {
        Self {
            ...
            scene_ids: plan.scene_ids.clone(),
            locator_ids: plan.locator_ids.clone(),
            camera_ids: plan.camera_ids.clone(),
            animation_ids: plan.animation_ids.clone(),
            texture_ids: plan.texture_ids.clone(),
            material_ids: plan.material_ids.clone(),
            physics_ids: plan.physics_ids.clone(),
        }
    }
    ...
    pub(super) fn to_json(&self) -> String {
        ...
        push_array_field(&mut json, "scene_ids", &self.scene_ids, true);
        push_array_field(&mut json, "locator_ids", &self.locator_ids, true);
        push_array_field(&mut json, "camera_ids", &self.camera_ids, true);
        push_array_field(&mut json, "animation_ids", &self.animation_ids, true);
        push_array_field(&mut json, "texture_ids", &self.texture_ids, true);
        push_array_field(&mut json, "material_ids", &self.material_ids, true);
        push_array_field(&mut json, "physics_ids", &self.physics_ids, false);
        json.push_str("}\n");
        json
    }
    ...
    pub(super) fn write_to(
        ...
        local_write_text(&path, &self.to_json(), true).map_err(|error| {
            PipelineError::new(format!("failed to write FBX manifest: {error}"))
        })?;
        Ok(path)
    }
}
...
fn file_len(path: &Path) -> Result<u64, PipelineError> {
    local_file_len(path).map_err(|error| {
        PipelineError::new(format!("failed to stat manifest: {error}"))
    })
}
...
#[path = "../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/fbx_manifest/tests.rs"]
mod tests;
