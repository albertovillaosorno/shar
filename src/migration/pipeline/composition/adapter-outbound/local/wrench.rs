...
pub(super) fn export_wrench(
    ...
    ensure_missing(output_dir, "Wrench output")?;
    let staging = staging_path(output_dir)?;
    ensure_missing(&staging, "Wrench staging")?;
    create_dir_all(&staging).map_err(|error| {
        PipelineError::new(format!("Wrench staging failed: {error}"))
    })?;
    let result = build_wrench(index_path, &staging, base_root).and_then(
        |(files, bytes, summary)| {
