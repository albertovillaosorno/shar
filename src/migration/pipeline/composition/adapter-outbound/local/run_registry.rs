...
impl RunRegistry {
    ...
    pub(in crate::adapters) fn active_lines(
        ...
        self.storage
            .active_runs(now)
            .map(|runs| runs.iter().map(|run| run.render(now)).collect())
    }
    ...
    pub(in crate::adapters) fn request_cancel(
        ...
        if target != "all" {
            self.storage.request_cancel(target)?;
            return Ok(vec![target.to_owned()]);
        }
        let now = RegistryStorage::now_unix_ms()?;
        let runs = self.storage.active_runs(now)?;
        let mut requested = Vec::with_capacity(runs.len());
        for run in runs {
            self.storage.request_cancel(run.run_id())?;
            requested.push(run.run_id().to_owned());
        }
        Ok(requested)
    }
    ...
    pub(in crate::adapters) fn start(
        ...
        let exclusive = self.acquire_start_lease(now, &run_id, mode)?;
        ...
        if let Err(error) = self.storage.create_run(&snapshot) {
            self.rollback_start(&run_id, exclusive, false);
            return Err(RunStartError::failure(error));
        }
        let control = RunControl::new(self.storage.clone(), snapshot);
        if let Err(error) = install_current_control(Arc::clone(&control)) {
            self.rollback_start(&run_id, exclusive, false);
            return Err(RunStartError::failure(error));
        }
        let supervisor =
            RunSupervisor::spawn(Arc::clone(&control)).map_err(|error| {
                self.rollback_start(&run_id, exclusive, true);
    ...
    fn acquire_start_lease(
        ...
        if mode == RunMode::Exclusive && !active.is_empty() {
            return Err(RunStartError::conflict(&active, now_unix_ms));
        }
        ...
        self.storage
            ...
            .map_err(RunStartError::failure)?;
        let raced = match self.storage.active_runs(now_unix_ms) {
            Ok(raced) => raced,
            Err(error) => {
                drop(self.storage.release_exclusive(run_id));
                return Err(RunStartError::failure(error));
            },
        };
        if raced.is_empty() {
            ...
            drop(self.storage.release_exclusive(run_id));
            Err(RunStartError::conflict(&raced, now_unix_ms))
        }
    }
    ...
    fn rollback_start(
        ...
        drop(self.storage.remove_run(run_id));
        if exclusive {
            drop(self.storage.release_exclusive(run_id));
        }
    }
...
impl RunGuard {
    ...
    fn cleanup(&mut self) -> Result<(), String> {
        ...
        if let Err(error) = self.supervisor.shutdown(&self.control) {
            failures.push(error);
        }
        if let Some(error) = self.control.take_background_error() {
            failures.push(error);
        }
        clear_current_control(&self.run_id);
        if let Err(error) = self.storage.remove_run(&self.run_id) {
            failures.push(error);
        }
        if self.exclusive
            && let Err(error) = self.storage.release_exclusive(&self.run_id)
        {
            failures.push(error);
        }
...
fn new_run_id(now_unix_ms: u64) -> String {
    ...
    format!("run-{now_unix_ms}-{}-{nonce}", std::process::id())
}
