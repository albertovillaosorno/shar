...
pub(super) fn export_world_props(
    ...
    for (ordinal, candidate) in candidates.iter().enumerate() {
        ...
        let variants = groups.entry(name).or_default();
        if let Some(existing) = variants
            .iter_mut()
            .find(|value| value.prepared.signature == prepared.signature)
        {
            existing.aliases.push(PropAlias::from(candidate));
        } else {
            variants.push(WorldVariant {
    ...
    for (name, variants) in groups {
        output.push(publish_group(&name, variants, output_root)?);
    }
...
fn publish_group(
    ...
    let mut canonical = iterator.next().ok_or_else(|| {
        PipelineError::new("world prop variant group is empty")
    })?;
    let canonical_structure = canonical.structural_sha256.clone();
    let canonical_rig = canonical.rig_sha256.clone();
    ...
    for variant in iterator {
            ...
            && variant.rig_sha256 == canonical_rig
        {
            ...
            omitted.push(OmittedWorldVariant {
                semantic_sha256: variant.prepared.signature,
                visual_sha256: variant.visual_sha256,
                structural_sha256: variant.structural_sha256,
                route: variant.prepared.route,
                source_count: variant.aliases.len(),
            });
            canonical.aliases.extend(variant.aliases);
        }
    }
    let merged_count = compatible.len();
    merge_compatible(&mut canonical, compatible);
    write_world_prop(name, canonical, omitted, merged_count, output_root)
}
...
fn first_package_id(variant: &WorldVariant) -> &str {
    variant
        .aliases
        .first()
        .map_or("", |alias| alias.package_id.as_str())
}

/// Order same-name variants by completeness and stable provenance.
fn compare_variants(left: &WorldVariant, right: &WorldVariant) -> Ordering {
    richness(right)
        .cmp(&richness(left))
        .then_with(|| first_package_id(left).cmp(first_package_id(right)))
        .then_with(|| left.prepared.signature.cmp(&right.prepared.signature))
}

/// Return one deterministic completeness score for canonical selection.
fn richness(variant: &WorldVariant) -> (bool, usize, usize, usize) {
    let (groups, bones) = match &variant.prepared.geometry {
        PreparedGeometry::Static(meshes) => {
            (meshes.iter().map(|mesh| mesh.groups.len()).sum(), 0)
        },
        PreparedGeometry::RigidAnimated { asset, .. } => (
            asset.parts.iter().map(|part| part.mesh.groups.len()).sum(),
            asset.bones.len(),
        ),
    };
    (
        variant.prepared.route == PropRoute::RigidAnimated,
        groups,
        bones,
        variant.aliases.len(),
    )
...
fn merge_compatible(canonical: &mut WorldVariant, variants: Vec<WorldVariant>) {
    let mut textures: BTreeMap<String, PreparedTexture> = canonical
        .prepared
        .textures
        .drain(..)
        .map(|texture| (texture.file_name.clone(), texture))
        .collect();
    let mut clips = take_clips(&mut canonical.prepared);
    let mut clip_keys =
        clips.iter().map(animation_key).collect::<BTreeSet<_>>();
    for mut variant in variants {
        canonical.aliases.append(&mut variant.aliases);
        for texture in std::mem::take(&mut variant.prepared.textures) {
            let _published_texture =
                textures.entry(texture.file_name.clone()).or_insert(texture);
        }
    ...
    if let PreparedGeometry::RigidAnimated { animations, .. } =
        &mut canonical.prepared.geometry
    {
        *animations = clips;
    }
    canonical.prepared.textures = textures.into_values().collect();
    canonical.prepared.signature = prepared_signature(
        canonical.prepared.route,
        &canonical.prepared.geometry,
        &canonical.prepared.materials,
        &canonical.prepared.textures,
    );
    canonical.aliases.sort_by(|left, right| {
        (&left.package_id, &left.container_key)
            .cmp(&(&right.package_id, &right.container_key))
    });
}
...
fn take_clips(prepared: &mut PreparedProp) -> Vec<AnimationClip> {
    match &mut prepared.geometry {
        ...
        PreparedGeometry::RigidAnimated { animations, .. } => {
...
fn publish_world_textures(
    ...
    for texture in &prepared.textures {
        fs::write(texture_dir.join(&texture.file_name), &texture.bytes)
            .map_err(|error| {
                PipelineError::new(format!(
                    "world prop texture write failed: {error}"
                ))
            })?;
...
fn write_world_prop(
    ...
    let textures = publish_world_textures(&variant.prepared, &texture_dir)?;
    let path = directory.join(format!("{name}.fbx"));
    let summary = match &variant.prepared.geometry {
        PreparedGeometry::Static(meshes) => write_binary_model_fbx(
            name,
            meshes,
            &variant.prepared.materials,
            &path,
        ),
    ...
    .map_err(|error| {
        PipelineError::new(format!("world prop FBX write failed: {error:?}"))
    })?;
    let bytes = fs::read(&path).map_err(|error| {
        PipelineError::new(format!("world prop FBX read-back failed: {error}"))
    })?;
    Ok(ExportedWorldProp {
        ...
        fbx_bytes: u64::try_from(bytes.len()).map_err(|error| {
            PipelineError::new(format!(
                "world prop FBX size overflowed: {error}"
            ))
        })?,
...
fn portable_name(value: &str) -> Result<String, PipelineError> {
    ...
    Ok(name
        .strip_suffix("-phonestop")
        .map_or_else(|| name.clone(), |level| format!("{level}-phone-booth")))
}

#[cfg(test)]
// jig-ignore-next-line: exact syntax is indivisible
#[path = "../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/prop_catalog/world_export/tests.rs"]
mod tests;
