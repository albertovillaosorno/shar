...
pub(super) enum VertexColorBake {
    ...
    Uniform { rgba8: [u8; 4], sample_count: u64 },
    /// One deterministic average across every use of one source texture.
    SourceAverage { rgba8: [u8; 4], sample_count: u64 },
}
...
impl VertexColorBake {
    ...
    pub(super) const fn rgba8(&self) -> [u8; 4] {
        match self {
            Self::Uniform { rgba8, .. } | Self::SourceAverage { rgba8, .. } => {
                *rgba8
            },
        }
    }
    ...
    pub(super) const fn is_approximate(&self) -> bool {
        matches!(self, Self::SourceAverage { .. })
    }
    ...
    pub(super) const fn sample_count(&self) -> u64 {
        match self {
            Self::Uniform { sample_count, .. }
            | Self::SourceAverage { sample_count, .. } => *sample_count,
        }
    }
