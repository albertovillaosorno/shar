...
#![expect(
    ...
    reason = "Atlas accumulation uses bounded channel sums and exact integer rounding by construction."
)]
...
impl TintAggregate {
    fn add(
        ...
        self.approximated |= self.first.is_some_and(|first| first != color);
        let _first = self.first.get_or_insert(color);
        ...
        self.samples =
            self.samples.checked_add(occurrences).ok_or_else(|| {
    ...
    fn finish(self) -> Result<VertexColorBake, PipelineError> {
        ...
        let rgba8 = self.sums.map(|sum| {
            let rounded = sum.saturating_add(self.samples / 2) / self.samples;
            u8::try_from(rounded).unwrap_or(u8::MAX)
        });
        Ok(if self.approximated {
...
pub(super) fn build(
    ...
    for (surface, vertex_bake) in surfaces {
        let material =
            content.materials.get(&surface.material).ok_or_else(|| {
                PipelineError::new(format!(
                    "structural-guide material binding is missing: {}",
                    surface.material
                ))
            })?;
        let source = source_pixels(content, material)?;
        let presentation_tint =
            combine_tints(material.base_color_rgba8, vertex_bake.rgba8());
        let entry = sources.entry(source.sha256.clone()).or_insert_with(|| {
            SourceAggregate {
                source: source.clone(),
        ...
        validate_same_source(&entry.source, &source)?;
        entry.tint.add(
            presentation_tint,
    ...
    let png_bytes = encode_rgb_png_bytes(ATLAS_SIZE, ATLAS_SIZE, &pixels)
        .map_err(|error| {
            PipelineError::new(format!(
                "structural-guide atlas PNG failed: {error:?}"
            ))
        })?;
    let assignments = assignments(&variants)?;
    let mut entries = variants
        .into_iter()
        .map(|variant| AtlasLayoutEntry {
    ...
    entries.sort_by(|left, right| {
        left.source_sha256
            ...
            .then_with(|| left.variant_sha256.cmp(&right.variant_sha256))
    });
    Ok(AtlasBuild {
        png_bytes,
        layout: AtlasLayout {
...
fn discover_surfaces(
    ...
    for mesh in &content.meshes {
        for group in &mesh.groups {
            let aggregate = surfaces.entry(surface_key(group)).or_default();
            if group.colors.is_empty() {
                aggregate.add(
                    [u8::MAX; 4],
                    u64::try_from(group.positions.len()).map_err(|error| {
                ...
                for color in &group.colors {
                    aggregate.add(quantize_rgba(*color)?, 1, false)?;
                }
            }
        }
    }
    ...
    surfaces
        .into_iter()
        .map(|(key, aggregate)| Ok((key, aggregate.finish()?)))
        .collect()
}
...
fn build_variant(
    ...
    let presentation_bake = source.tint.finish()?;
    ...
    let mut identity =
        Vec::with_capacity(pixels.len().saturating_mul(3).saturating_add(160));
    identity.extend_from_slice(source.source.sha256.as_bytes());
    presentation_bake.append_identity(&mut identity);
    identity.extend(pixels.iter().flat_map(|pixel| pixel.iter()).copied());
    Ok(AtlasVariant {
...
fn assignments(
    ...
    for variant in variants {
        let common = AtlasAssignment {
            ...
            scale: [
                atlas_component(variant.width.saturating_sub(1))?,
                atlas_component(variant.height.saturating_sub(1))?,
            ],
            repeat: 0.,
        ...
        for key in &variant.surface_keys {
            ...
            if assignments.insert(key.clone(), assignment).is_some() {
                return Err(PipelineError::new(
...
fn source_pixels(
    ...
    if let Some(file_name) = material.texture_file_name.as_deref() {
        let texture = content.textures.get(file_name).ok_or_else(|| {
            PipelineError::new(format!(
        ...
        let image = decode_png_bytes(&texture.bytes).map_err(|error| {
            PipelineError::new(format!(
    ...
    let source_identity = [b"swatch:".as_slice(), color.as_slice()].concat();
    Ok(SourcePixels {
        name: format!(
...
fn combine_tints(material: [u8; 4], vertex: [u8; 4]) -> [u8; 4] {
    [
        multiply_channel(material[0], vertex[0]),
        multiply_channel(material[1], vertex[1]),
        multiply_channel(material[2], vertex[2]),
        multiply_channel(material[3], vertex[3]),
    ]
}
...
fn wrap_mode(surfaces: &[SurfaceKey]) -> &'static str {
    let repeat = surfaces.iter().any(|surface| surface.repeat);
    let clamp = surfaces.iter().any(|surface| !surface.repeat);
    match (repeat, clamp) {
...
fn quantize_color(value: f32) -> Result<u8, PipelineError> {
    ...
    Ok((value * 255.).round() as u8)
}

#[cfg(test)]
// jig-ignore-next-line: exact syntax is indivisible
#[path = "../../../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/prop_catalog/world_level/structural_guide/atlas/tests.rs"]
mod tests;
