...
const OBJECT_JOIN_DISTANCE: f32 = 1.;
...
pub(super) fn split_distant_islands(
    ...
    clusters
        ...
        .enumerate()
        .map(|(ordinal, cluster)| {
            rebuild_mesh(&mesh.name, ordinal, &cluster, &components)
        })
        .collect()
}
...
fn rebuild_mesh(
    ...
    .map_err(|error| {
        PipelineError::new(format!(
            "world island mesh rebuild failed: {error:?}"
        ))
    })
}
...
fn group_components(
    ...
    triangle_clusters(&position_sets)?
        .into_iter()
        .map(|triangles| extract_component(group, &triangles))
        .collect()
}
    ...
    group
        ...
        .map(|triangle| {
            triangle
                ...
                .map(|vertex_index| {
                    let index =
                        usize::try_from(*vertex_index).map_err(|error| {
                            PipelineError::new(format!(
...
fn triangle_clusters(
    ...
    while let Some(seed) = remaining.pop_first() {
        ...
        while let Some(current) = queue.pop_front() {
            ...
            let current_set = position_sets.get(current).ok_or_else(|| {
                PipelineError::new("world triangle position set is missing")
            })?;
            let connected = remaining
                ...
                .filter(|candidate| {
                    position_sets
                        .get(*candidate)
                        .is_some_and(|other| !current_set.is_disjoint(other))
                })
                .collect::<Vec<_>>();
...
fn extract_component(
    ...
    let used_vertices = collect_used_vertices(source, triangle_indices)?;
    let compact = compact_attributes(source, &used_vertices)?;
    let indices = remapped_indices(source, triangle_indices, &compact.remap)?;
    let group = rebuild_group(source, compact, &indices)?;
    let (low, high) = group_bounds(&group)?;
    Ok(GroupComponent { group, low, high })
}
...
fn collect_used_vertices(
    ...
    for triangle_index in triangle_indices {
        let triangle =
            source.triangles.get(*triangle_index).ok_or_else(|| {
...
fn compact_attributes(
    ...
    for (compact_index, source_vertex) in
        used_vertices.iter().copied().enumerate()
    {
        let target = u32::try_from(compact_index).map_err(|error| {
            PipelineError::new(format!(
                "world island compact index overflowed: {error}"
            ))
        })?;
        let source_index = usize::try_from(source_vertex).map_err(|error| {
            PipelineError::new(format!(
                "world island source index overflowed: {error}"
            ))
        })?;
        let _previous = compact.remap.insert(source_vertex, target);
        compact.positions.push(copy_required(
            &source.positions,
            source_index,
            "position",
        )?);
        copy_optional(&source.uvs, source_index, &mut compact.uvs, "UV")?;
        copy_optional(
            &source.normals,
            source_index,
            &mut compact.normals,
...
fn copy_optional<T: Copy>(
    ...
    output.push(copy_required(source, index, label)?);
    Ok(())
}
...
fn remapped_indices(
    ...
    let capacity = triangle_indices.len().checked_mul(3).ok_or_else(|| {
    ...
    for triangle_index in triangle_indices {
        ...
        for source_vertex in triangle {
            indices.push(*remap.get(source_vertex).ok_or_else(|| {
                PipelineError::new("world island vertex remap is missing")
            })?);
        }
    }
    Ok(indices)
...
fn rebuild_group(
    ...
    .map_err(|error| {
        PipelineError::new(format!(
            "world island primitive rebuild failed: {error:?}"
        ))
    })?;
    if !normals.is_empty() {
        group = group.with_normals(normals).map_err(|error| {
            PipelineError::new(format!(
    ...
    if !colors.is_empty() {
        group = group.with_colors(colors).map_err(|error| {
            PipelineError::new(format!(
                "world island color rebuild failed: {error:?}"
            ))
        })?;
    }
    Ok(group)
    ...
    while let Some(seed) = remaining.pop_first() {
        ...
        while let Some(current) = queue.pop_front() {
            ...
            let current_component =
                components.get(current).ok_or_else(|| {
                    PipelineError::new(
            ...
            let connected = remaining
                ...
                .copied()
                .filter(|candidate| {
                    components.get(*candidate).is_some_and(|other| {
                        component_gap(current_component, other)
...
fn component_gap(left: &GroupComponent, right: &GroupComponent) -> f32 {
    ...
    [
        axis_gap(left_low_x, left_high_x, right_low_x, right_high_x),
        axis_gap(left_low_y, left_high_y, right_low_y, right_high_y),
        axis_gap(left_low_z, left_high_z, right_low_z, right_high_z),
    ]
    .into_iter()
    .fold(0f32, f32::max)
}
...
fn group_bounds(
    ...
    let first =
        group.positions.first().copied().ok_or_else(|| {
            PipelineError::new("world island has no positions")
        })?;
    let mut low = first;
    let mut high = first;
    for position in &group.positions {
        for ((low_axis, high_axis), position_axis) in
            low.iter_mut().zip(high.iter_mut()).zip(position.iter())
        {
