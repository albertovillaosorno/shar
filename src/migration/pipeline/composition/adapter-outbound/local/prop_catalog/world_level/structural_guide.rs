...
// Boundary-Contract:
// - Owns:
//   - Structural guide outbound adapter.
// - Must-Not:
...
#![expect(
    ...
    reason = "PNG validation checks fixed header length and chunk bounds before exact field access."
)]
...
pub(in crate::adapters::driven::local) fn export(
    ...
    super::super::ensure_missing(output_dir, "structural-guide output")?;
    let staging = super::super::staging_path(output_dir)?;
    super::super::ensure_missing(&staging, "structural-guide staging")?;
    fs::create_dir_all(&staging).map_err(|error| {
        PipelineError::new(format!("structural-guide staging failed: {error}"))
    })?;
    let result = build(index_path, game_root, coordinate_root, &staging)
        .and_then(|report| {
            fs::rename(&staging, output_dir).map_err(|error| {
                PipelineError::new(format!(
                    "structural-guide publication failed: {error}"
                ))
            })?;
            Ok(report)
        });
...
fn build(
    ...
    let (guide_mesh, counts) = mesh::build(&content, &atlas)?;
    let texture_dir = staging.join("textures");
    fs::create_dir_all(&texture_dir).map_err(|error| {
        PipelineError::new(format!(
            "structural-guide texture staging failed: {error}"
        ))
    })?;
    let atlas_path = staging.join(ATLAS_FILE);
    write_new(&atlas_path, &atlas.png_bytes)?;
    let mut layout_bytes = serde_json::to_vec_pretty(&atlas.layout)
        .map_err(|error| PipelineError::new(error.to_string()))?;
    layout_bytes.push(b'\n');
    let layout_path = staging.join(LAYOUT_FILE);
    write_new(&layout_path, &layout_bytes)?;
    let fbx_path = staging.join(FBX_FILE);
    let summary = write_binary_structural_guide_fbx(&guide_mesh, &fbx_path)
        .map_err(|error| {
            PipelineError::new(format!("guide FBX failed: {error:?}"))
        })?;
    let fbx_bytes = fs::read(&fbx_path).map_err(|error| {
        PipelineError::new(format!("structural-guide FBX read failed: {error}"))
    })?;
...
fn validate_publication(
    ...
    if files != 4 {
        return Err(PipelineError::new(format!(
            "structural-guide file count changed: {files}"
        )));
    }
    validate_fbx(fbx)?;
    ...
    for path in [FBX_FILE, MANIFEST_FILE, ATLAS_FILE, LAYOUT_FILE] {
...
fn validate_fbx(bytes: &[u8]) -> Result<(), PipelineError> {
    ...
    if version != 7_700 {
        return Err(PipelineError::new(format!(
            "structural-guide FBX version changed: {version}"
        )));
    }
    for marker in [
        ...
        if !contains_bytes(bytes, marker.as_bytes()) {
            return Err(PipelineError::new(format!(
                "structural-guide FBX marker is missing: {marker}"
            )));
        }
    }
    if contains_bytes(bytes, b"AnimationStack")
        || contains_bytes(bytes, b"NodeAttribute")
        || contains_bytes(bytes, b"Deformer")
        || contains_bytes(bytes, b"\x89PNG\r\n\x1a\n")
    {
...
fn validate_atlas(bytes: &[u8]) -> Result<(), PipelineError> {
    ...
    let width = u32::from_be_bytes(bytes[16..20].try_into().map_err(
        |error: std::array::TryFromSliceError| {
            PipelineError::new(error.to_string())
        },
    )?);
    let height = u32::from_be_bytes(bytes[20..24].try_into().map_err(
        |error: std::array::TryFromSliceError| {
            PipelineError::new(error.to_string())
        },
    )?);
...
fn validate_layout(value: &serde_json::Value) -> Result<(), PipelineError> {
    let width = value.get("atlasWidth").and_then(serde_json::Value::as_u64);
    let height = value.get("atlasHeight").and_then(serde_json::Value::as_u64);
    let padding = value
        .get("paddingPixels")
    ...
    for entry in entries {
        ...
        if let Some((previous_source, previous_variant)) = previous
            && (source, variant) < (previous_source, previous_variant)
        {
            return Err(PipelineError::new(
                "atlas entries are not ordered by source SHA-256",
            ));
        }
        previous = Some((source, variant));
    }
    Ok(())
}
...
fn validate_manifest(value: &serde_json::Value) -> Result<(), PipelineError> {
    ...
    let normal_layer_included = value
        ...
        .and_then(serde_json::Value::as_bool);
    let normal_contract_changed =
        match (groups_without_normals, normal_layer_included) {
    ...
    if contract_changed {
        return Err(PipelineError::new(
            "structural-guide manifest contract changed",
        ));
    }
    for pointer in [
        "/hashes/fbxSha256",
...
#[cfg(test)]
// jig-ignore-next-line: exact syntax is indivisible
#[path = "../../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/prop_catalog/world_level/structural_guide/tests.rs"]
mod tests;
