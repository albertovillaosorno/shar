...
pub(super) fn collect_lights(
    ...
    for component in components {
        let light_identity =
            identity(&component.value, &component.relative_path);
        let Some(extras) =
            component.value.get("extras").and_then(Value::as_array)
        else {
            continue;
        };
        for (ordinal, extra) in extras.iter().enumerate() {
            let Some(kind) = extra.get("kind").and_then(Value::as_str) else {
                continue;
            };
            ...
            match kind {
                "position" => {
                    let source = vector3(value, "light position")?;
                    let moved = movement
                        .transform_point(source)
                        .map_err(movement_error(&light_identity))?;
                    records.push(WorldMovedCoordinateRecord::point(
                ...
                "direction" => {
                    let source = vector3(value, "light direction")?;
                    let moved = movement
                        .transform_direction(source)
                        .map_err(movement_error(&light_identity))?;
                    records.push(WorldMovedCoordinateRecord::direction(
                        component.relative_path.clone(),
