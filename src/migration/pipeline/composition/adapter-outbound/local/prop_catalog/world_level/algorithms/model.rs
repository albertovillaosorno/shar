...
impl FbxFingerprint {
    ...
    pub(super) fn from_meshes(
        ...
        for mesh in meshes {
            result.groups =
                add_count(result.groups, mesh.groups.len(), "group")?;
            for group in &mesh.groups {
                ...
                result.uvs = add_count(result.uvs, group.uvs.len(), "UV")?;
                result.normals =
                    add_count(result.normals, group.normals.len(), "normal")?;
                result.colors =
                    add_count(result.colors, group.colors.len(), "color")?;
            }
        }
        Ok(result)
    }
...
fn count(value: usize, subject: &str) -> Result<u64, PipelineError> {
    u64::try_from(value).map_err(|error| {
        PipelineError::new(format!(
            "FBX repair {subject} count overflowed: {error}"
        ))
    })
}
...
fn add_count(
    ...
    current.checked_add(count(value, subject)?).ok_or_else(|| {
        PipelineError::new(format!("FBX repair {subject} total overflowed"))
    })
}
