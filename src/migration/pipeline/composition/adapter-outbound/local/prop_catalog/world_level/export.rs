...
pub(super) fn export_world_collection(
...
) -> Result<(ExportedWorldCollection, MasterContent), PipelineError> {
    ...
    for directory in [&texture_root, &review_root] {
        fs::create_dir_all(directory).map_err(|error| {
            PipelineError::new(format!(
                "world package output directory failed: {error}"
            ))
        })?;
    }
    ...
    for package in packages {
        ...
        let mut record = package_content.packages.pop().ok_or_else(|| {
        ...
        if !used_file_names.insert(stem.clone()) {
            return Err(PipelineError::new(format!(
                "world package FBX identity repeats: {stem}"
            )));
        }
        ...
        if record.interior {
            ...
            pending_interiors.push(PendingInterior {
                identity,
                level,
        ...
        if let Some(artifact) = record.world_fbx.as_ref() {
            aggregate_semantics.add(artifact.surface_semantics);
            append_world_fbx_to_guide(&package_content, &mut guide_content)?;
        }
        records.push(record);
    }
    ...
    let textures = publish_textures(&all_textures, &texture_root)?;
    if has_review_fbx {
        fs::create_dir_all(&review_texture_root).map_err(|error| {
            PipelineError::new(format!(
                "world review texture directory failed: {error}"
            ))
        })?;
        let _review_records =
            publish_textures(&all_textures, &review_texture_root)?;
    }
    Ok((
...
> {
    ...
    for package in pending {
        groups.entry(package.identity).or_default().push(package);
    }
    ...
    for (identity, mut packages) in groups {
        packages.sort_by(|left, right| {
            left.level.cmp(&right.level).then_with(|| {
                left.record.package_id.cmp(&right.record.package_id)
            })
        });
        let source_package_ids = packages
            .iter()
            .map(|package| package.record.package_id.clone())
            .collect::<Vec<_>>();
        ...
        append_world_fbx_to_guide(&base, guide_content)?;
        let halloween_fbx = if identity.halloween_overlay {
            ...
            append_world_fbx_to_guide(&overlay, guide_content)?;
            Some(artifact)
        } else {
        ...
        interiors.push(WorldInteriorRecord {
...
fn publish_nested_content_textures(
    ...
    if parent.components().count() < 2 || content.textures.is_empty() {
        return Ok(());
    }
    let texture_root = output_root.join(parent).join("textures");
    fs::create_dir_all(&texture_root).map_err(|error| {
        PipelineError::new(format!(
            "nested world texture directory failed: {error}"
        ))
    })?;
    let _records = publish_textures(&content.textures, &texture_root)?;
    Ok(())
}
...
fn write_content_fbx(
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|error| {
            PipelineError::new(format!(
                "world FBX parent directory failed: {error}"
            ))
        })?;
    }
...
fn append_package(
    ...
    package_content.packages.push(WorldPackageRecord {
    ...
    {
        let package_record =
            package_content.packages.get_mut(package_index).ok_or_else(
                || PipelineError::new("world package record is missing"),
            )?;
        package_record.excluded_collision_meshes = collisions.meshes.len();
    ...
    if package_scratch.is_dir() {
        fs::remove_dir_all(&package_scratch).map_err(|error| {
            PipelineError::new(format!(
                "world package material cleanup failed: {error}"
            ))
        })?;
    }
    Ok(())
...
fn load_analysis_meshes(
    ...
    package_root: &Path,
) -> Result<(Vec<MeshAsset>, usize), PipelineError> {
    let recovered = sources
        .iter()
        .map(|source| {
            read_mesh_for_analysis(package_root, &source.member_id).map_err(
                |error| {
                    PipelineError::new(format!(
    ...
    let discarded =
        recovered
            .iter()
            .try_fold(0_usize, |total, (_mesh, count)| {
...
fn append_source_mesh(
    ...
    for (ordinal, matrix) in matrices.iter().enumerate() {
...
fn append_direct_or_omit(
    ...
    if !is_direct_world_mesh(source) {
        bake_mesh(
            ...
            scene_name(
                &package.package_id,
                &source.member_id,
                &role_marked_suffix("review-definition", source, false),
            )?,
        )?;
        let profile = shape_profile(&mesh)?;
        content.review.push(ReviewMesh { mesh, profile });
        increment_package_count(
            &mut content.packages,
            package_index,
    ...
    let used_reference =
        coordinates.apply_direct_reference(source, &mut mesh)?;
...
fn place_review_gallery(
    ...
    if content.review.is_empty() {
        return Ok(0);
    }
    content
        .review
        .sort_by(|left, right| left.mesh.name.cmp(&right.mesh.name));
    let mut representatives = Vec::<ShapeProfile>::new();
    let mut assignments = Vec::with_capacity(content.review.len());
    for item in &content.review {
        let group = if let Some(group) =
            representatives.iter().position(|representative| {
                shape_similarity(item.profile, *representative) >= 0.5_f32
            }) {
            group
        } else {
            representatives.push(item.profile);
            representatives.len().checked_sub(1).ok_or_else(|| {
                PipelineError::new("world review group underflowed")
            })?
        };
        assignments.push(group);
    }
    let world_high = content.meshes.iter().map(mesh_bounds).fold(
        [0f32, 0f32, 0f32],
        |high, (_low, item_high)| {
            let [high_x, high_y, high_z] = high;
            let [item_x, item_y, item_z] = item_high;
            [high_x.max(item_x), high_y.max(item_y), high_z.max(item_z)]
        },
    );
    let maximum_extent = representatives
        .iter()
        .flat_map(|profile| {
            let [x, _y, z] = profile.extents;
            [x, z]
        })
        .fold(10f32, f32::max);
    let cell = maximum_extent.mul_add(1f32, 20f32);
    let columns = square_columns(representatives.len());
    let review = std::mem::take(&mut content.review);
    for (mut item, group) in review.into_iter().zip(assignments) {
        let column = group
            .checked_rem(columns)
        ...
        let column_offset = review_grid_index(column)?.mul_add(1f32, 1f32);
        let row_offset = review_grid_index(row)?.mul_add(1f32, 1f32);
        let [world_x, _world_y, world_z] = world_high;
        let target = [
            cell.mul_add(column_offset, world_x),
            0f32,
            cell.mul_add(row_offset, world_z),
        ];
        let (low, high) = mesh_bounds(&item.mesh);
        let [low_x, low_y, low_z] = low;
        let [high_x, _high_y, high_z] = high;
        let centre = [low_x.midpoint(high_x), low_y, low_z.midpoint(high_z)];
        let final_name = portable_asset_name(
            &format!("{}-review-group-{group:04}", item.mesh.name),
            120,
            "world review mesh has no portable identity",
        )?;
        let [target_x, target_y, target_z] = target;
        let [centre_x, centre_y, centre_z] = centre;
        bake_mesh(
            &mut item.mesh,
            &translation([
                target_x - centre_x,
...
fn review_grid_index(value: usize) -> Result<f32, PipelineError> {
    u16::try_from(value).map(f32::from).map_err(|error| {
        PipelineError::new(format!(
            "world review grid index overflowed: {error}"
        ))
    })
}
...
fn shape_profile(mesh: &MeshAsset) -> Result<ShapeProfile, PipelineError> {
    let vertices = mesh.groups.iter().map(|group| group.positions.len()).sum();
    let triangles = mesh.groups.iter().map(|group| group.triangles.len()).sum();
    ...
    Ok(ShapeProfile {
        ...
        triangles,
        extents: [
            (high_x - low_x).abs().max(0.001_f32),
            (high_y - low_y).abs().max(0.001_f32),
            (high_z - low_z).abs().max(0.001_f32),
        ],
    })
}
...
fn shape_similarity(left: ShapeProfile, right: ShapeProfile) -> f32 {
    let components = [
        count_ratio(left.vertices, right.vertices),
        count_ratio(left.triangles, right.triangles),
        value_ratio(left.extents[0], right.extents[0]),
        value_ratio(left.extents[1], right.extents[1]),
        value_ratio(left.extents[2], right.extents[2]),
    ];
    components.into_iter().sum::<f32>() / 5f32
}

/// Return one symmetric ratio in the inclusive zero-to-one range.
fn count_ratio(left: usize, right: usize) -> f32 {
    value_ratio(review_count_f32(left), review_count_f32(right))
}
...
fn value_ratio(left: f32, right: f32) -> f32 {
    left.min(right) / left.max(right).max(f32::EPSILON)
}
...
fn world_object_semantic_counts(meshes: &[MeshAsset]) -> (usize, usize, usize) {
    ...
    for mesh in meshes {
        ...
        if mesh.name.contains("__interactable") {
            interactable = interactable.saturating_add(geometries);
        }
    }
    (independent, breakable, interactable)
}
...
fn role_marked_suffix(
    ...
    object_role(source).suffix().map_or_else(
        || {
...
fn world_surface_semantics(
    ...
    for group in meshes.iter().flat_map(|mesh| &mesh.groups) {
...
fn retain_used_presentation(content: &mut MasterContent) {
    ...
    let used_textures = content
        .materials
        .values()
        .filter_map(|binding| binding.texture_file_name.clone())
        .collect::<BTreeSet<_>>();
    content
        .textures
...
fn merge_textures(
    ...
    for texture in textures {
        match target.get(&texture.file_name) {
            ...
            None => {
                let _previous =
                    target.insert(texture.file_name.clone(), texture);
            },
        }
    }
    Ok(())
...
fn publish_textures(
    ...
    for texture in textures.values() {
        fs::write(output.join(&texture.file_name), &texture.bytes).map_err(
            |error| {
                PipelineError::new(format!(
                    "world package texture write failed: {error}"
                ))
            },
        )?;
        records.push(TextureRecord {
            file_name: texture.file_name.clone(),
            bytes: u64::try_from(texture.bytes.len()).map_err(|error| {
                PipelineError::new(format!(
...
#[path = "../../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/prop_catalog/world_level/export/tests.rs"]
mod tests;
