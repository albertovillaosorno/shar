...
pub(super) fn load_intersect_meshes(
    ...
    let reference = reference_root
        .map_or_else(|| Ok(BTreeMap::new()), intersect_documents)?;
    ...
    for (stem, canonical_document) in canonical {
        let reference_positions = reference
            .get(&stem)
            .filter(|document| topology_matches(&canonical_document, document))
            .map(|document| document.positions.clone());
        reference_coordinate_meshes = reference_coordinate_meshes
            .checked_add(usize::from(reference_positions.is_some()))
            .ok_or_else(|| {
                PipelineError::new("world collision reference count overflowed")
            })?;
        ...
        discarded_triangles =
            discarded_triangles.checked_add(discarded).ok_or_else(|| {
                PipelineError::new("world collision discard count overflowed")
            })?;
        meshes.push(mesh);
    }
    Ok(CollisionBatch {
...
fn intersect_documents(
    ...
    let root = package_root.join("components").join("srr_intersect_dsg");
    if !root.is_dir() {
    ...
    for path in paths {
        ...
        let document = parse_document(&value, &path)?;
        ...
        let stem = path
            ...
            .to_ascii_lowercase();
        if documents.insert(stem.clone(), document).is_some() {
            return Err(PipelineError::new(format!(
                "world collision stem is ambiguous: {stem}"
            )));
        }
    }
    Ok(documents)
...
fn parse_document(
    ...
    let schema = value
        ...
        .to_owned();
    let num_indices = json_usize(value, "num_indices", path)?;
    let indices = value
        .get("indices")
    ...
    let num_positions = json_usize(value, "num_positions", path)?;
    let positions = value
        .get("positions")
        .and_then(Value::as_array)
        .ok_or_else(|| {
        ...
        .iter()
        .map(|item| json_position(item, path))
        .collect::<Result<Vec<_>, _>>()?;
    Ok(IntersectDocument {
        schema,
        num_indices,
        indices,
        num_positions,
...
fn json_position(
    ...
    for (axis, component) in components.iter().enumerate() {
        let source_number = component.as_f64().ok_or_else(|| {
            PipelineError::new(format!(
                "invalid world collision position component for {}",
                path.display()
            ))
        })?;
        let number = checked_position_component(source_number, path)?;
        let target = position.get_mut(axis).ok_or_else(|| {
            PipelineError::new("world collision position axis overflowed")
        })?;
        *target = number;
    }
    Ok(position)
...
fn topology_matches(
    ...
    canonical.indices == reference.indices
        && canonical.positions.len() == reference.positions.len()
}
...
fn validate_document(
    ...
    if document.num_indices != document.indices.len()
        || document.num_positions != document.positions.len()
        || document.indices.is_empty()
        || document.positions.is_empty()
        || !document.indices.len().is_multiple_of(3)
    {
        return Err(PipelineError::new(format!(
...
#[path = "../../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/prop_catalog/world_level/collision/tests.rs"]
mod tests;
