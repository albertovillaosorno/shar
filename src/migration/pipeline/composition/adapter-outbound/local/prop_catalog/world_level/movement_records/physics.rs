        ...
        Value::Object(values) => {
                ...
                .is_some_and(|kind| kind == "obbox")
            {
                ...
                *ordinal = ordinal.checked_add(1).ok_or_else(|| {
                    PipelineError::new("physics movement ordinal overflowed")
                })?;
            }
...
fn collect_oriented_box(
    ...
    let center = vector3(center_value, "physics center")?;
    ...
    for (basis_ordinal, basis_value) in vectors.iter().skip(1).enumerate() {
        let basis = vector3(basis_value, "physics basis")?;
