...
// Boundary-Contract:
// - Owns:
//   - Movement records outbound adapter.
// - Must-Not:
...
pub(super) fn collect_moved_records(
    ...
    collect_locators(
        &components(package_root, "srr_locator")?,
        movement,
        &mut records,
    )?;
    collect_lights(
        &components(package_root, "light")?,
        movement,
        &mut records,
    )?;
    collect_physics(
        &components(package_root, "srr_static_phys_dsg")?,
        movement,
        &mut records,
    )?;
...
fn components(
    ...
    let root = package_root.join("components").join(family);
    if !root.is_dir() {
        return Ok(Vec::new());
    }
    let mut paths = fs::read_dir(&root)
        .map_err(|error| {
            PipelineError::new(format!(
                "movement component directory failed: {error}"
            ))
        })?
        .map(|entry| {
            entry
    ...
    paths
        ...
        .map(|path| {
            ...
            Ok(SourceComponent { relative_path, value })
        })
        .collect()
}

/// Return one decoded identity without fixed-width NUL padding.
pub(super) fn identity(value: &Value, fallback: &str) -> String {
    value
        .get("name")
...
pub(super) fn vector3(
    ...
    values.try_into().map_err(|_values: Vec<f32>| {
...
pub(super) fn movement_error(
    ...
    move |error| {
        PipelineError::new(format!(
            "coordinate movement failed for {identity}: {error}"
        ))
    }
}
...
fn json_f32(value: &Value) -> Result<f32, PipelineError> {
    ...
    let parsed = number.to_string().parse::<f32>().map_err(|error| {
        PipelineError::new(format!(
            "coordinate value conversion failed: {error}"
        ))
    })?;
    parsed
        .is_finite()
