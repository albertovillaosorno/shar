...
#![expect(
    ...
    reason = "Atlas dimensions and padded coordinates are validated before deterministic packing arithmetic."
)]
...
pub(super) fn pack(
    ...
    for variant in variants.iter_mut() {
        ...
        if cursor_x.saturating_add(packed_width) > ATLAS_SIZE {
            cursor_x = 0;
            cursor_y = cursor_y.checked_add(row_height).ok_or_else(|| {
        ...
        cursor_x = cursor_x.checked_add(packed_width).ok_or_else(|| {
    ...
    for variant in variants.iter() {
        blit_with_padding(&mut atlas, variant)?;
    }
    Ok(atlas)
}
...
fn blit_with_padding(
    ...
    for y in 0..variant.height {
        for x in 0..variant.width {
            let source = source_pixel(variant, x, y)?;
            set_atlas_pixel(
                atlas,
                variant.useful_x + x,
                variant.useful_y + y,
    ...
    for distance in 1..=ATLAS_PADDING {
        ...
        for y in 0..variant.height {
            set_atlas_pixel(
                ...
                left_x,
                variant.useful_y + y,
                source_pixel(variant, 0, y)?,
            )?;
            set_atlas_pixel(
                atlas,
                right_x,
                variant.useful_y + y,
                source_pixel(variant, variant.width - 1, y)?,
            )?;
        }
        ...
        for x in 0..variant.width {
            set_atlas_pixel(
                ...
                variant.useful_x + x,
                top_y,
                source_pixel(variant, x, 0)?,
            )?;
            set_atlas_pixel(
                atlas,
                variant.useful_x + x,
                bottom_y,
                source_pixel(variant, x, variant.height - 1)?,
            )?;
        }
        for (x, y, source_x, source_y) in [
            (left_x, top_y, 0, 0),
            (right_x, top_y, variant.width - 1, 0),
            (left_x, bottom_y, 0, variant.height - 1),
            (right_x, bottom_y, variant.width - 1, variant.height - 1),
        ] {
            set_atlas_pixel(
                atlas,
                x,
                y,
                source_pixel(variant, source_x, source_y)?,
            )?;
        }
    }
    Ok(())
...
fn set_atlas_pixel(
    ...
    if x >= ATLAS_SIZE || y >= ATLAS_SIZE {
        return Err(PipelineError::new(
            "atlas write exceeded 4096-square bounds",
        ));
    }
    let index =
