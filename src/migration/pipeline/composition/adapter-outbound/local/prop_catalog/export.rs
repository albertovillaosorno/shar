...
struct ExportState {
    ...
    identities: BTreeMap<(PropFamily, String), usize>,
    /// Family-local public name to semantic signature.
    public_names: BTreeMap<(PropFamily, String), String>,
}
...
impl ExportState {
    ...
    fn publish_candidate(
        ...
        let key = (candidate.family, prepared.signature.clone());
        if let Some(existing) = self.identities.get(&key).copied() {
            let asset = self.assets.get_mut(existing).ok_or_else(|| {
                PipelineError::new("prop duplicate index escaped asset list")
            })?;
            asset.aliases.push(PropAlias::from(candidate));
        } else {
            self.publish_unique(candidate, prepared, key, output_root)?;
        }
    ...
    fn publish_unique(
        ...
        identity: (PropFamily, String),
        output_root: &Path,
    ) -> Result<(), PipelineError> {
        let public_name = asset_name(&candidate.owner_name)?;
        let public_key = (candidate.family, public_name.clone());
        if let Some(existing_signature) = self.public_names.get(&public_key)
        ...
        let exported =
            write_unique_prop(candidate, prepared, &public_name, output_root)?;
        let _previous_public_name = self
            .public_names
            .insert(public_key, exported.signature.clone());
        let _previous_identity = self.identities.insert(identity, index);
        self.assets.push(exported);
        Ok(())
    }
    ...
    fn finish(mut self) -> Vec<ExportedProp> {
        for asset in &mut self.assets {
            asset.aliases.sort_by(|left, right| {
                (&left.package_id, &left.owner_name, &left.container_key).cmp(
                    &(
                        &right.package_id,
                        &right.owner_name,
                        &right.container_key,
        ...
        self.assets.sort_by(|left, right| {
            (left.family, &left.asset_id).cmp(&(right.family, &right.asset_id))
        });
        self.assets
    }
}
...
fn write_unique_prop(
    ...
    let relative_dir = asset_relative_dir(candidate.family, asset_id);
    ...
    let texture_records = publish_textures(&prepared, &texture_dir)?;
    ...
    .map_err(|error| {
        PipelineError::new(format!("prop FBX write failed: {error:?}"))
    })?;
    ...
    Ok(ExportedProp {
        ...
        fbx_bytes: u64::try_from(fbx_bytes.len()).map_err(|error| {
            PipelineError::new(format!(
                "prop FBX byte count overflowed: {error}"
            ))
        })?,
...
fn asset_relative_dir(family: PropFamily, asset_id: &str) -> String {
    if family == PropFamily::Cards && asset_id == "phone-icon" {
        return asset_id.to_owned();
    }
    format!("{}/{asset_id}", family.as_str())
}
...
#[path = "../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/prop_catalog/export/tests.rs"]
mod tests;
