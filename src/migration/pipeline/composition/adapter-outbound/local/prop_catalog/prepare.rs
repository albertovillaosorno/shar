...
fn prepare_candidate_internal(
    ...
    fs::create_dir_all(&scratch).map_err(|error| {
        PipelineError::new(format!("prop preparation scratch failed: {error}"))
    })?;
...
fn prepare_static(
    ...
    let mut meshes = candidate
        ...
        .iter()
        .map(|member| {
            source.load_mesh(member).map_err(|error| {
    ...
    let (materials, textures) = match authority {
        ...
        None => {
            canonicalize_static_materials(&mut meshes, package_root, scratch)
        },
    }?;
    canonicalize_static_meshes(&mut meshes);
    let geometry = PreparedGeometry::Static(meshes);
    let signature =
        prepared_signature(PropRoute::Static, &geometry, &materials, &textures);
    Ok(PreparedProp {
...
fn prepare_animated(
    ...
    let composite = required_component(
        candidate.composite_id.as_deref(),
        package_root,
        "composite_drawable",
    )?;
    let animation = required_component(
        candidate.animation_id.as_deref(),
        package_root,
        "animation",
    )?;
    let mesh_paths = candidate
        .mesh_ids
        .iter()
        .map(|member| component_path(package_root, "mesh", member))
        .collect::<Vec<_>>();
    ...
    .map_err(|error| {
        PipelineError::new(format!("rigid prop animation failed: {error:?}"))
    })?;
    ...
    let (materials, textures) = match authority {
        ...
        None => {
            canonicalize_animated_materials(&mut asset, package_root, scratch)
        },
    }?;
    canonicalize_animated_asset(&mut asset, &mut animations)?;
    let geometry = PreparedGeometry::RigidAnimated { asset, animations };
...
fn component_path(root: &Path, family: &str, member: &str) -> PathBuf {
...
fn required_component(
    ...
    let required_member = member.ok_or_else(|| {
        PipelineError::new(format!(
            "rigid prop is missing required {family} member"
        ))
    })?;
    Ok(component_path(root, family, required_member))
}
...
pub(super) fn structural_signature(prepared: &PreparedProp) -> String {
    let meshes = match &prepared.geometry {
        PreparedGeometry::Static(value) => value.iter().collect::<Vec<_>>(),
        PreparedGeometry::RigidAnimated { asset, .. } => asset
            .parts
            .iter()
            .map(|part| &part.mesh)
            .collect::<Vec<_>>(),
    ...
    let evidence = meshes
        ...
        .map(|mesh| {
            mesh.groups
                .iter()
                .map(|group| (group.index, &group.positions, &group.triangles))
                .collect::<Vec<_>>()
        })
        .collect::<Vec<_>>();
