...
pub(super) fn write_catalog(
    ...
    let mut bytes = serde_json::to_vec_pretty(&payload).map_err(|error| {
        PipelineError::new(format!(
            "non-world prop catalog JSON failed: {error}"
        ))
    })?;
    bytes.push(b'\n');
    fs::write(root.join("non-world-props.catalog.json"), bytes).map_err(
        |error| {
            PipelineError::new(format!(
                "non-world prop catalog write failed: {error}"
            ))
        },
    )
}
...
pub(super) fn inventory(root: &Path) -> Result<(usize, u64), PipelineError> {
        ...
        for entry in fs::read_dir(&directory).map_err(|error| {
            ...
            if path.is_dir() {
                ...
                files = files.checked_add(1).ok_or_else(|| {
                    PipelineError::new("prop catalog file count overflowed")
                })?;
                bytes = bytes
                    .checked_add(
                        fs::metadata(&path)
                    ...
                    .ok_or_else(|| {
                        PipelineError::new("prop catalog byte total overflowed")
                    })?;
            }
        }
    }
