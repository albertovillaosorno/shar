...
pub(in crate::adapters::driven::local) fn export_world_prop_catalog(
    ...
    super::ensure_missing(output_dir, "world prop catalog output")?;
    let staging = super::staging_path(output_dir)?;
    super::ensure_missing(&staging, "world prop catalog staging")?;
    fs::create_dir_all(&staging).map_err(|error| {
        PipelineError::new(format!("world prop staging failed: {error}"))
    })?;
    let result = build(index_path, game_root, &staging).and_then(
        |(files, bytes, counts)| {
            fs::rename(&staging, output_dir).map_err(|error| {
                PipelineError::new(format!(
                    "world prop publication failed: {error}"
                ))
            })?;
            Ok(StageReport {
                name: STAGE,
...
fn build(
    ...
    game_root: &Path,
    staging: &Path,
) -> Result<(usize, u64, super::world_model::WorldCatalogCounts), PipelineError>
{
    ...
    fs::create_dir_all(&normalized).map_err(|error| {
        PipelineError::new(format!("world prop normalization failed: {error}"))
    })?;
    let source_packages =
    ...
    let counts = world_counts(source_packages, candidates.len(), &assets);
    write_world_catalog(staging, counts, &assets)?;
    fs::remove_dir_all(&work).map_err(|error| {
        PipelineError::new(format!("world prop work cleanup failed: {error}"))
    })?;
