...
pub(super) fn verify_model_scope(
    ...
    if asset.parts.len() != BODY_MESH_MEMBERS.len()
        || asset
            .parts
            .first()
            .is_none_or(|part| part.mesh.name != "wrench7Shape")
    {
...
pub(super) fn verify_summary(
    ...
    {
        return Err(PipelineError::new(format!(
            "Wrench FBX summary changed: {summary:?}"
        )));
    }
    Ok(())
}
    ...
    for texture_name in texture_names {
        ...
        bytes = bytes
            .checked_add(file_len(&texture_dir.join(texture_name)).map_err(
                |error| {
                    PipelineError::new(format!(
                        "Wrench texture inventory failed: {error}"
                    ))
                },
            )?)
            .ok_or_else(|| {
...
#[path = "../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/wrench/artifact/tests.rs"]
mod tests;
