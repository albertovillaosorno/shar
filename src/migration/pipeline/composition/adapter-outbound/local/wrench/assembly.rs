...
pub(super) fn build_wrench(
    ...
    let summary =
        write_binary_character_fbx(&asset, &materials, &animations, &fbx_path)
            .map_err(|error| {
                PipelineError::new(format!("Wrench FBX failed: {error:?}"))
            })?;
...
fn member_path(
    ...
    let member = matches.next().ok_or_else(|| {
        PipelineError::new(format!("Wrench member is missing: {expected}"))
    })?;
    if matches.next().is_some() {
        return Err(PipelineError::new(format!(
            "Wrench member is duplicated: {expected}"
        )));
    }
...
fn verify_animation(
    ...
    if animations.len() != 1
        ...
        || clip.frame_count != 61
        || clip.frame_rate.to_bits() != 30f64.to_bits()
        || !clip.cyclic
    {
        return Err(PipelineError::new(format!(
...
#[path = "../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/wrench/assembly/tests.rs"]
mod tests;
