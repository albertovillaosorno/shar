...
pub(super) fn append_summary(json: &mut JsonObject, text: &str) {
    ...
    for element in &elements {
        ...
        tag_sequence.push(element.tag.clone());
        if element.tag.contains("pure3d") {
            p3d_refs = p3d_refs.saturating_add(1);
        }
        if element.tag.contains("textbible") {
            text_bible_refs = text_bible_refs.saturating_add(1);
        }
        quoted_values.extend(element.attributes.values().cloned());
        let count = counts.entry(element.tag.clone()).or_insert(0);
        *count = count.saturating_add(1);
    }

    json.field("root_tag", &root);
    json.number(
        "element_count",
    ...
    json.map("tag_counts", &counts);
    json.string_array("tag_sequence", &tag_sequence);
    json.string_array("quoted_values", &quoted_values);
    json.raw_json("xml_elements", &xml_elements_json(&elements));
}
...
fn xml_elements(text: &str) -> Vec<XmlElement> {
    ...
    while let Some(start) = cursor.find('<') {
        cursor = cursor.get(start.saturating_add(1)..).unwrap_or_default();
        ...
        let element_body = cursor.split('>').next().unwrap_or_default();
        let ordinal = elements.len().saturating_add(1);
...
fn attributes_from_start_tag(value: &str) -> BTreeMap<String, String> {
    ...
    for part in parts {
        if let Some((name, raw_value)) = part.split_once('=') {
            ...
            let clean = without_closing.trim_matches('"').to_owned();
            if !name.is_empty() && !clean.is_empty() {
                drop(attrs.insert(name.to_ascii_lowercase(), clean));
            }
        }
    }
    attrs
...
fn xml_elements_json(elements: &[XmlElement]) -> String {
    ...
    for (index, element) in elements.iter().enumerate() {
        ...
        out.push_str(&element.ordinal.to_string());
        out.push_str(",\"tag\":");
        out.push_str(&json_string(&element.tag));
        out.push_str(",\"attributes\":{");
        for (attr_index, (key, value)) in element.attributes.iter().enumerate()
        {
            if attr_index > 0 {
                out.push(',');
            }
...
#[path = "../../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/two/stragglers/scrooby/tests.rs"]
mod tests;
