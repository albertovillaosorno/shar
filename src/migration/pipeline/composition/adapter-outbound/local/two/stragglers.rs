...
fn artifact_length(stage: &'static str, length: usize) -> PipelineOutcome<u64> {
...
fn relative_source_path<'source>(
    ...
    source.strip_prefix(game_root).map_err(|_error| {
        PipelineError::new(format!("failed to relativize {}", source.display()))
    })
}
    ...
    for source in collect_files(game_root)? {
        ...
        if JSON_EXTENSIONS.contains(&ext.as_str()) {
            ...
            bytes = StageReport::checked_byte_total(
                "phase-two-stragglers",
                bytes,
                artifact_length("phase-two-stragglers", json.len())?,
            )?;
        } else if ext == "rsd" {
            let relative = relative_source_path(&source, game_root)?;
            ...
            bytes = StageReport::checked_byte_total(
                "phase-two-stragglers",
                bytes,
                artifact_length("phase-two-stragglers", wav.len())?,
            )?;
        }
    }
...
/// Route class.
fn route_class(relative: &Path, ext: &str) -> &'static str {
    let route = relative.to_string_lossy().to_ascii_lowercase();
    if ext == "mfk" || route.contains("mission") {
        "mission"
    } else if ext == "con" || route.contains("car") {
        "vehicle-config"
    } else if matches!(ext, "pag" | "scr" | "prj") || route.contains("scrooby")
    {
        "frontend-ui"
        ...
        "sound-metadata"
    } else if matches!(ext, "txt" | "e" | "f" | "g" | "i" | "s" | "x") {
        "localization"
    } else if ext == "err" {
        "build-artifact"
    } else {
...
/// Rsd to wav.
fn rsd_to_wav(bytes: &[u8], relative: &Path) -> PipelineOutcome<Vec<u8>> {
...
fn extension_of(path: &Path) -> String {
    path.extension()
        .and_then(|value| value.to_str())
        .map_or_else(|| "error".to_owned(), str::to_ascii_lowercase)
}
...
#[path = "../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/two/stragglers/tests.rs"]
mod tests;
