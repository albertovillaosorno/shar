...
pub(super) fn classification_from_package(
    ...
    let resource_index =
        tokens.iter().position(|token| *token == "resource")?;
    let tail = tokens.get(resource_index.saturating_add(1)..)?;
    if let Some(subcategory) = scene_resource_subcategory(&tokens, tail) {
        return Some((PackageCategory::UiResources, subcategory));
    }
    let scope = ui_resource_scope(tail)?;
    let role = ui_resource_role(tail);
    let detail = ui_resource_detail(&tokens, tail, role);
    Some((
...
fn ui_resource_detail(tokens: &[&str], tail: &[&str], role: &str) -> String {
    if !ui_resource_role_allows_detail(role) {
        return String::new();
    }
    if role == "art-assets" {
        return ui_resource_art_detail(tokens, tail);
    }
    tail.get(1..)
        .filter(|rest| !rest.is_empty())
        .map_or_else(String::new, |rest| format!("/{}", rest.join("-")))
}

/// Supports the `ui_resource_art_detail` operation within this deterministic
/// classification boundary.
fn ui_resource_art_detail(tokens: &[&str], tail: &[&str]) -> String {
...
fn scene_resource_subcategory(
    ...
    if tail.first().is_none_or(|head| *head != "pure3d") {
        return None;
    }
    let scene_tokens = tail.get(1..).unwrap_or(&[]);
    ...
    Some(format!(
        "ui-resources/frontend-scenes/{role}/{family}/{detail}"
    ))
}
...
fn scene_resource_role(scene_tokens: &[&str]) -> &'static str {
    ...
    {
        "source-metadata"
    } else if scene_tokens.iter().any(|token| token.starts_with("camset")) {
        "camera-sets"
    } else if scene_tokens.contains(&"camlight") {
        "camera-lighting"
    } else if scene_tokens.iter().any(|token| token.ends_with("hudmap")) {
        "hud-maps"
    } else if scene_tokens.contains(&"hudhole") {
        "hud-overlays"
    } else if scene_tokens.iter().any(|token| token.starts_with("gag")) {
        "gag-scenes"
    } else if scene_tokens.iter().any(|token| token.starts_with("glow")) {
        "interactive-glows"
    } else if scene_tokens.iter().any(scene_transition_token) {
        "screen-transitions"
    } else if scene_tokens.iter().any(scene_reward_token) {
        "reward-presentation"
    } else if scene_tokens.iter().any(scene_loading_token) {
        "loading-presentation"
    } else if scene_tokens.iter().any(scene_character_token) {
        "character-scenes"
    } else if scene_tokens.iter().any(scene_effect_token) {
        "effects"
    } else if scene_tokens.iter().any(scene_vehicle_token) {
        "vehicle-scenes"
    } else if scene_tokens.iter().any(scene_glyph_token) {
        "glyph-scenes"
    } else {
        "art-assets"
    }
...
fn ui_inventory_role(tail: &[&str]) -> Option<&'static str> {
    if ui_tail_has(tail, |token| {
        token.starts_with("card") || token == "cards" || token == "cardsfx"
    }) {
        Some("cards")
    } else if ui_tail_has(tail, |token| {
        token.starts_with("car") || token == "racecar" || token == "aicar"
    }) {
        Some("vehicles")
    } else if ui_tail_has(tail, |token| token.starts_with("font")) {
        Some("fonts")
    } else if ui_tail_has(tail, |token| {
        token.starts_with("but") || ui_control_token(token)
    }) {
        Some("controls")
    } else {
        None
...
fn ui_hud_role(tail: &[&str]) -> Option<&'static str> {
    if ui_tail_has(tail, |token| token.starts_with("frame")) {
        Some("frames")
    } else if ui_tail_has(tail, ui_hud_panel_token) {
        Some("hud-panels")
    } else if ui_tail_has(tail, ui_hud_map_token) {
        Some("hud-maps")
    } else if ui_tail_has(tail, ui_speaker_token) {
        Some("speaker-icons")
    } else if ui_tail_has(tail, ui_mission_token) {
        Some("mission-icons")
    } else if ui_tail_has(tail, ui_economy_token) {
        Some("economy-icons")
    } else if ui_tail_has(tail, ui_radar_token) {
        Some("radar-icons")
    } else if ui_tail_has(tail, ui_hit_and_run_token) {
...
fn ui_feedback_role(tail: &[&str]) -> Option<&'static str> {
    if ui_tail_has(tail, ui_audio_token) {
        Some("audio-icons")
    } else if ui_tail_has(tail, ui_communication_token) {
        Some("communication-icons")
    } else if ui_tail_has(tail, ui_branding_token) {
        Some("branding")
    } else if ui_tail_has(tail, ui_effect_token) {
        Some("effects")
    } else if ui_tail_has(tail, |token| token.starts_with("cam")) {
        Some("camera-sets")
    } else if ui_tail_has(tail, |token| token.starts_with("gag")) {
        Some("gag-icons")
    } else if ui_tail_has(tail, |token| token.starts_with("glow")) {
...
fn ui_presentation_role(tail: &[&str]) -> Option<&'static str> {
    if ui_tail_has(tail, ui_loading_token) {
        Some("loading")
    } else if ui_tail_has(tail, |token| token.starts_with("reward")) {
        Some("rewards")
    } else if ui_tail_has(tail, ui_hazard_token) {
        Some("hazards")
    } else if ui_tail_has(tail, ui_character_token) {
        Some("character-icons")
    } else if ui_tail_has(tail, ui_status_token) {
        Some("status-icons")
    } else if ui_tail_has(tail, ui_glyph_token) {
        Some("glyphs")
    } else {
        None
    }
...
/// classification boundary.
fn ui_tail_has(tail: &[&str], predicate: fn(&str) -> bool) -> bool {
    tail.iter().copied().any(predicate)
}
...
fn ui_hud_panel_token(token: &str) -> bool {
    token.starts_with("corner")
        || matches!(token, "greybar" | "greybgd" | "levelbar" | "redbgd")
}
...
fn ui_economy_token(token: &str) -> bool {
    matches!(token, "coins" | "dollar" | "money")
}
...
#[path = "../../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/two/units/ui_resources/tests.rs"]
mod tests;
