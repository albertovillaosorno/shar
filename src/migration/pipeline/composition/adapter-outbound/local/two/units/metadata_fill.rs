...
// Boundary-Contract:
// - Owns:
//   - Metadata fill outbound adapter.
// - Must-Not:
...
pub(in crate::adapters::driven::local) fn fill_minor_unit_metadata(
    ...
    for (line_index, line) in input.lines().enumerate() {
        ...
        let path = read_string_field(line, "path").ok_or_else(|| {
        ...
        let file_extension = read_string_field(line, "file_extension")
            .ok_or_else(|| {
                PipelineError::new(format!(
        ...
        let mut metadata =
            classify_minor_unit(extracted_root, &path, &file_extension)?;
        ...
        carry_manifest_provenance(&mut metadata, line);
        apply_manifest_evidence_classification(&mut metadata, line, &path);
        ...
        output.push_str(&render_filled_row(&path, &file_extension, &metadata));
        output.push('\n');
        rows = rows.saturating_add(1);
    }

    local_write_text(&manifest_path, &output, true)
...
fn render_filled_row(
    ...
    render_row(&[
        ("path", path),
        ("id", &metadata.id),
        ("obfuscated_route", &metadata.obfuscated_route),
        ...
        ("source_chunk_kind", &metadata.source_chunk_kind),
        ("source_chunk_ordinal", &metadata.source_chunk_ordinal),
        ("recovery_status", &metadata.recovery_status),
        ("derived_from", &metadata.derived_from),
        ("size_bytes", &metadata.size_bytes),
        ("unreal_import_relation", &metadata.unreal_import_relation),
        ("future_normalization", &metadata.future_normalization),
        ("component_links", &metadata.component_links),
        ("classification_notes", &metadata.classification_notes),
    ])
}
...
fn carry_manifest_provenance(metadata: &mut MinorUnitMetadata, line: &str) {
    metadata.obfuscated_route = read_string_field(line, "obfuscated_route")
        .unwrap_or_else(|| taxonomy::UNKNOWN.to_owned());
    metadata.source_chunk_kind = read_string_field(line, "source_chunk_kind")
        .filter(|value| value != taxonomy::UNKNOWN)
        .unwrap_or_else(|| taxonomy::NOT_APPLICABLE.to_owned());
    metadata.source_chunk_ordinal =
        read_string_field(line, "source_chunk_ordinal")
            .filter(|value| value != taxonomy::UNKNOWN)
            .unwrap_or_else(|| taxonomy::NOT_APPLICABLE.to_owned());
    metadata.recovery_status = read_string_field(line, "recovery_status")
        .filter(|value| value != taxonomy::UNKNOWN)
...
fn apply_manifest_evidence_classification(
    ...
    if metadata.kind == "mission-script" {
        refine_mission_script(metadata, path);
    } else if metadata.kind == "vehicle-tuning" {
        refine_vehicle_tuning(metadata, path);
    } else if metadata.type_ == "audio" && is_dialog_audio(path) {
        refine_dialog_audio(metadata, line, path);
    }
}
...
fn refine_mission_script(metadata: &mut MinorUnitMetadata, path: &str) {
...
fn refine_vehicle_tuning(metadata: &mut MinorUnitMetadata, path: &str) {
    let level = mission_level(path);
...
fn refine_dialog_audio(
    ...
    let route_group = read_string_field(line, "obfuscated_route")
        .map_or_else(|| "none".to_owned(), |route| route_parent_key(&route));
...
fn is_dialog_audio(path: &str) -> bool {
    path.split('/').any(is_dialog_container_segment)
}
...
fn is_dialog_container_segment(segment: &str) -> bool {
    matches!(segment, "dialog" | "dialogf" | "dialogg" | "dialogs")
}
...
fn has_level_mission_token(stem: &str) -> bool {
    stem.as_bytes().windows(4).any(|window| {
...
fn route_parent_key(route: &str) -> String {
    route
        .rsplit_once('/')
        .map_or_else(|| "none".to_owned(), |(parent, _leaf)| parent.to_owned())
}
...
pub(in crate::adapters::driven::local::two) fn compute_id(
    ...
    format!("{type_}-{}", uuid_shaped(fnv1a_128(&seed)))
}
...
/// Read string field.
pub(super) fn read_string_field(line: &str, field: &str) -> Option<String> {
...
#[path = "../../../../../../../../tests/migration/pipeline/unit/adapter-outbound/local/two/units/metadata_fill/tests.rs"]
mod tests;
