...
fn render_package(package: &MinorUnitPackage) -> String {
    let fields = [
        ("unit_ids", ids_json(package, None)),
        ("world_ids", ids_json(package, Some(MinorUnitRole::World))),
        (
            "texture_ids",
            ids_json(package, Some(MinorUnitRole::Texture)),
        ),
        (
            "material_ids",
            ids_json(package, Some(MinorUnitRole::Material)),
        ),
        ("model_ids", ids_json(package, Some(MinorUnitRole::Model))),
        (
            "physics_ids",
            ids_json(package, Some(MinorUnitRole::Physics)),
        ),
        (
            "animation_ids",
            ids_json(package, Some(MinorUnitRole::Animation)),
        ),
        ("scene_ids", ids_json(package, Some(MinorUnitRole::Scene))),
        (
            "locator_ids",
            ids_json(package, Some(MinorUnitRole::Locator)),
        ),
        ("camera_ids", ids_json(package, Some(MinorUnitRole::Camera))),
        ("light_ids", ids_json(package, Some(MinorUnitRole::Light))),
        (
            "particle_ids",
            ids_json(package, Some(MinorUnitRole::Particle)),
        ),
        (
            "controller_ids",
            ids_json(package, Some(MinorUnitRole::Controller)),
        ),
        ("audio_ids", ids_json(package, Some(MinorUnitRole::Audio))),
        ("movie_ids", ids_json(package, Some(MinorUnitRole::Movie))),
        ("script_ids", ids_json(package, Some(MinorUnitRole::Script))),
        ("text_ids", ids_json(package, Some(MinorUnitRole::Text))),
        ("ui_ids", ids_json(package, Some(MinorUnitRole::Ui))),
        (
            "metadata_ids",
            ids_json(package, Some(MinorUnitRole::Metadata)),
        ),
        ("error_ids", ids_json(package, Some(MinorUnitRole::Error))),
        (
            "source_unit_ids",
            string_array_json(
                &package
        ...
        ("text_key_ids", text_key_ids_json(package)),
        ("members", members_json(package)),
        ("text_keys", text_keys_json(package)),
    ];
    let mut row = format!(
        concat!(
            "{{\"package_id\":\"{}\",",
...
fn ids_json(package: &MinorUnitPackage, role: Option<MinorUnitRole>) -> String {
    let ids = package
        .members
        .iter()
        .filter(|member| {
...
fn text_keys_json(package: &MinorUnitPackage) -> String {
    let mut output = String::from("[");
    for (index, key) in package.text_keys.iter().enumerate() {
        if index > 0 {
            output.push(',');
        }
        let key_json = format!(
