...
pub(super) fn classify_minor_unit(
    ...
    if meta.type_ == UNKNOWN {
        classify_legacy_extension(&mut meta, &route, &ext);
    }
    finalize_metadata(&mut meta, extracted_root, manifest_path, &ext, &route)?;
    Ok(meta)
}
...
impl RouteSignature {
    ...
    fn has(&self, feature: RouteFeature) -> bool {
...
fn apply_classification(
    ...
    classification.type_.clone_into(&mut metadata.type_);
    classification.subtype.clone_into(&mut metadata.subtype);
    classification.kind.clone_into(&mut metadata.kind);
    classification.function.clone_into(&mut metadata.function);
    classification.origin.clone_into(&mut metadata.origin);
    classification
        .unreal_import_relation
...
/// Read prefix.
fn read_prefix(path: &Path, limit: usize) -> PipelineOutcome<Vec<u8>> {
...
fn finalize_metadata(
    ...
    if meta.schema == UNKNOWN {
        schema_hint(ext, route).clone_into(&mut meta.schema);
    }
    ...
    route.container().clone_into(&mut meta.source_container);
    derived_from(manifest_path).clone_into(&mut meta.derived_from);
    meta.size_bytes = size_value(extracted_root, manifest_path)?;
    meta.component_links = if route.has(RouteFeature::P3dComponent) {
        "component-sibling-set".to_owned()
    } else {
        "none".to_owned()
    };
    route.note().clone_into(&mut meta.classification_notes);
...
/// Schema hint.
fn schema_hint(ext: &str, route: &RouteSignature) -> &'static str {
    if route.has(RouteFeature::P3dComponent) || route.leaf == "components.jsonl"
    {
...
fn size_value(
    ...
    let source_disk_path = manifest_path
        ...
        .or_else(|| {
            manifest_path
                .strip_prefix("game/")
                .map(|relative| std::path::PathBuf::from("game").join(relative))
        });
