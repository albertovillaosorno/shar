...
fn edit_row(line: &str) -> String {
    let type_ = read_string_field(line, "type").unwrap_or_default();
    let kind = read_string_field(line, "kind").unwrap_or_default();
    let origin = read_string_field(line, "origin").unwrap_or_default();
    let future =
        read_string_field(line, "future_normalization").unwrap_or_default();
    let path = read_string_field(line, "path").unwrap_or_default();
    ...
    {
        return rewrite_p3d_component_row(line, &path);
    }
...
fn rewrite_p3d_component_row(line: &str, path: &str) -> String {
    ...
    let refreshed_id = compute_id(
        &read_string_field(line, "obfuscated_route").unwrap_or_default(),
        bucket.type_,
        &read_string_field(line, "recovery_status").unwrap_or_default(),
    );
    replace_many_owned(line, &[
        ("id", refreshed_id),
        ("type", bucket.type_.to_owned()),
        ("subtype", subtype),
        ("kind", bucket.kind.to_owned()),
        ("function", function),
        ("unreal_import_relation", bucket.relation.to_owned()),
        ("future_normalization", bucket.future.to_owned()),
...
fn component_segment(path: &str) -> &str {
    ...
    if let Some(index) = parts.iter().position(|part| *part == "components")
...
const fn error_bucket(reason: &'static str) -> ComponentBucket {
    bucket("error", "error", "error", "error", reason)
}
...
/// Replace many owned.
fn replace_many_owned(line: &str, replacements: &[(&str, String)]) -> String {
    let mut output = line.to_owned();
    for (field, value) in replacements {
        output = replace_field(&output, field, value);
    }
    output
}

/// Replace field.
fn replace_field(line: &str, field: &str, value: &str) -> String {
    let needle = format!("\"{field}\":\"");
    let Some(start) = line.find(&needle) else {
        return line.to_owned();
    };
    let value_start = start.saturating_add(needle.len());
    let Some(value_end_offset) =
        line.get(value_start..).unwrap_or_default().find('"')
    else {
        return line.to_owned();
    };
    let value_end = value_start.saturating_add(value_end_offset);
    let mut output = String::new();
    output.push_str(line.get(..value_start).unwrap_or_default());
    output.push_str(value);
    output.push_str(line.get(value_end..).unwrap_or_default());
    output
}
