    ...
    let value = json!({
        ...
        "boundary": {
            ...
            "sidecars": [
                ...
                "exact FBX material slots with shader and texture evidence"
            ],
            "excluded": [
        ...
        "counts": {
            ...
            "material_slots": records
                .iter()
                .map(|record| record.material_slots.len())
                .sum::<usize>(),
...
/// Render one exact vehicle FBX material slot and its source evidence.
fn material_slot_value(record: &MaterialSlotRecord) -> Value {
    let semantics = record.semantics;
    let texture = record.texture_path.as_ref().map(|path| {
        json!({
            "path": path,
...
fn part_semantics(part: &PartRecord) -> Vec<&'static str> {
    ...
    if part.semantics.is_transparent() {
        semantics.push("transparent");
    }
    if part.semantics.is_glass() {
        semantics.push("glass");
    }
    if part.semantics.is_mirror() {
        semantics.push("mirror");
    }
    if part.semantics.is_reflective() {
        semantics.push("reflective");
    }
    if part.semantics.is_light_emitter() {
        semantics.push("light-emitter");
    }
    if part.semantics.is_visual_effect() {
        semantics.push("vfx");
    }
    semantics
}
