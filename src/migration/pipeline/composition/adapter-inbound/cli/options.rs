...
impl ParsedArguments {
    ...
    pub(super) fn run_label(&self) -> Option<String> {
        self.run.label.clone()
    }

    /// Resolve the diagnostic log for one acquired run identity.
    pub(super) fn log_file_for_run(&self, run_id: &str) -> Option<PathBuf> {
        self.run.log_file_for_run(
    ...
    while let Some(argument) = arguments.get(index) {
        ...
        if parse_options && argument == "--verbosity" {
            ...
            if value.starts_with('-') {
                return Err(format!(
                    "--verbosity requires a value before {value}"
                ));
            }
            select_verbosity(
        ...
        if parse_options {
            let parsed_log_option =
                parse_log_option(arguments, index, argument)?;
        ...
        if parse_options
            && let Some(consumed) =
                parse_run_option(arguments, index, argument, &mut parsed.run)?
        {
...
#[cfg(test)]
// jig-ignore-next-line: exact syntax is indivisible
#[path = "../../../../../../tests/migration/pipeline/unit/adapter-inbound/cli/options_tests.rs"]
mod tests;
