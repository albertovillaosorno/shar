...
fn dispatch_known_command(
    ...
    if command == "fbx-export-catalog" {
        return run_complete_fbx_catalog(&parsed.positionals);
    }
...
/// Runs the exhaustive package-level FBX catalog publication.
fn run_complete_fbx_catalog(arguments: &[String]) -> CommandOutcome {
    if let Some(outcome) = reject_extra_positionals(arguments, 3) {
        return outcome;
    }
    let Some(index_path) = arguments.first() else {
