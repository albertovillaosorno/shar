...
# Path:
#   - mcp/tests/test_project_configuration.py
#
# Copyright:
...
def test_unreal_project_has_one_canonical_descriptor_root() -> None:
    ...
    canonical_root = _REPOSITORY_ROOT / "uproject"
    obsolete_root = _REPOSITORY_ROOT / "shar-uproject"
...
def test_unreal_project_enables_inbound_server_and_all_toolsets() -> None:
    project_path = _REPOSITORY_ROOT / "uproject/shar.uproject"
...
def test_project_plugins_remain_local_and_translator_is_not_a_server() -> None:
    ...
    translator_readme = (_REPOSITORY_ROOT / "mcp/README.md").read_text(
        encoding="utf-8"
    )

    ignore_lines = ignore_text.splitlines()
    assert "uproject/Plugins/" in ignore_lines
...
def test_translator_pytest_configuration_owns_import_discovery() -> None:
    ...
    config_lines = (
        (_REPOSITORY_ROOT / "mcp/pytest.ini")
...
def test_translator_version_matches_package_metadata() -> None:
    ...
    pyproject = require_json_object(
        cast(
            "object",
            tomllib.loads(
                (_REPOSITORY_ROOT / "mcp/pyproject.toml").read_text(
                    encoding="utf-8"
                )
            ),
        ),
        context="mcp/pyproject.toml",
    )
    project = require_json_object(
        pyproject.get("project"),
        context="mcp/pyproject.toml.project",
    )
...
def test_native_mcp_server_autostarts_with_tool_search() -> None:
    settings_path = (
        _REPOSITORY_ROOT
        / "uproject/Config/DefaultEditorPerProjectUserSettings.ini"
    )
...
def test_translator_uses_flat_source_layout_and_stable_entrypoint() -> None:
    ...
    package_root = _REPOSITORY_ROOT / "mcp"
...
def test_game_feature_data_primary_asset_type_is_always_cooked() -> None:
    ...
    settings_path = _REPOSITORY_ROOT / "uproject/Config/DefaultGame.ini"
