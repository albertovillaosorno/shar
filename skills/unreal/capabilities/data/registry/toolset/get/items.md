...
Returns cached item data. Items must be loaded in the registry cache to be
returned.
...
Use this tool to retrieve a bounded set of named Data Registry items before SHAR
...
- Item names and row payloads depend on the registry row struct.
...
A map from item name to its struct data. Names that were not found in the cache
