...
Use this tool to create one disposable procedural graph asset in a bounded
SHAR validation folder.
...
```json
{
  "name": "PCG_MCP_Main_c297c180",
  "path": "/Game/SHAR_MCP_Validation_PCG_c297c180"
}
```
...
Asset existence changed from false to true. `GetGraphStructure` returned the
exact graph name with default input and output nodes, and `GetGraphSchema`
returned the default pins.
...
Name for the new PCG graph asset (e.g., "PCG_ForestScatter")
...
Content folder path where the asset should be created. Defaults to "/Game/PCG"
