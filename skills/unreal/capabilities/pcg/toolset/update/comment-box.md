...
Updates an existing comment box with new nodes and value. Note: If the bounding
box of the nodes include other nodes, they will be included in the comment.
...
Use this tool to change the text, color, or membership of one PCG editor
comment box.
...
```json
{
  ...
  "comment": "MCP Comment Updated c297c180",
  "commentId": "C8E9EB8A4C84053D86CE88B89860CE47",
  "graph": {
...
A Slate snapshot changed from the original unique comment text to the updated
text, with the old text absent.
...
The new comment to put on the comment box. If empty, will keep the same text as
it was. Default is empty.
...
The list of nodes to include in the comment box. If empty, the box will keep
