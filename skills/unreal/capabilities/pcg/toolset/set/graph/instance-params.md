...
Sets the graph instance params of a specific actor, actor MUST have a graph
instance
...
Use this tool to override exact exposed parameters on one PCG graph instance.
...
`GetGraphInstanceParams` changed the exposed `densityScale` override from its
initial value to the requested instance value.
...
JSON string representing JsonObject for the params to set. MUST be in format:
...
The actor to set graph instance params for
