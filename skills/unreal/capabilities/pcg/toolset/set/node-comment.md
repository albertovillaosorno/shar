...
Use this tool to attach concise SHAR intent or provenance text to one PCG
node.
...
```json
{
  ...
  "nodeComment": "Offsets SHAR validation points."
}
```
...
`GetNodeInfo` changed the target node comment from empty to `Offsets SHAR
validation points.`.
...
The node to set the comment on.
