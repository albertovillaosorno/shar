# File media source payload exists
...
```text
SharImportEditor.SharImportToolset.FileMediaSourcePayloadExists
```
...
`assetPath` has no prose; confirm its meaning with `describe`.
