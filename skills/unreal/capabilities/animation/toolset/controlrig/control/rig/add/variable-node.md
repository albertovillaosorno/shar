...
Use this tool to add a getter or setter node for one existing Control Rig
member variable.
...
`list_nodes` added the graph-local `VariableNode` for the existing
`GraphValue` member variable.
...
True for getter node, False for setter node.
