...
Use this tool to connect one compatible RigVM output pin to one exact input
pin.
...
```json
{
  ...
  "source_pin": {
    "refPath": "/Game/SHAR_MCP_Validation_ControlRig/CR_MCP.CR_MCP:RigVMModel MCPGraph.MCPAddA.Result"
  },
  "target_pin": {
    "refPath": "/Game/SHAR_MCP_Validation_ControlRig/CR_MCP.CR_MCP:RigVMModel MCPGraph.MCPAddB.A"
  }
}
```
...
`get_connected_pins` changed from empty to the exact target input `MCPAddB.A`
for source `MCPAddA.Result`.
