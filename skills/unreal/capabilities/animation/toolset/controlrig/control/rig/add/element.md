...
Use this tool to add one generic bone or null element when the specific helper
is not required.
...
`get_elements` added `MCP_ElementNull` with type `Null` to the complete
hierarchy inventory.
...
- The generic helper accepted only `Bone` or `Null`; use `add_control` for
  controls. A Curve request was rejected before mutation.
...
Type of element to create: "bone" or "null".
