...
Use this tool to delete one exact disposable or obsolete RigVM node.
...
```json
{
  ...
  "node": {
    "refPath": "/Game/SHAR_MCP_Validation_ControlRig/CR_MCP.CR_MCP:RigVMModel MCPGraph.MCPAddB"
  }
}
```
...
`list_nodes` contained `MCPAddB` before deletion and omitted it afterward
while preserving the other three nodes.
