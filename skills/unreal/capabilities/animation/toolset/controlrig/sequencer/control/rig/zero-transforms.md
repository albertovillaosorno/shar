...
Use this tool to reset the selected Control Rig transform value to its
authored default.
...
After setting the selected PositionControl to 91/92/93, the call returned true
and the dedicated getter returned 0/0/0 at frame 24.
...
- With `selection_only: true`, behavior depends on active editor selection.
  Capture the selected-control list before invoking.
...
If True, include channels in addition to parent controls.
...
If True, only reset controls on rigs with selected controls. If False, reset
all.
