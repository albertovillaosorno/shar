...
Use this tool to set one Boolean Control Rig value at an exact SHAR sequence
frame.
...
The dedicated getter changed from false to true after the call, and a cleanup
call restored false.
...
- This validation used `set_key: false`; use explicit keying tools when a
  durable sequence key is required.
