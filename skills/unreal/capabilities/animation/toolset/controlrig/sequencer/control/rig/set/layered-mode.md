...
Use this tool to switch one existing SHAR Control Rig track between absolute
and layered evaluation.
...
The layered-state getter changed from false to true after the call. Cleanup
restored the track to non-layered mode.
...
- This changes an existing track. The earlier `find_or_create_track`
  `is_layered` argument remains creation-time-only and is a separate contract.
...
True for layered, False for absolute.
