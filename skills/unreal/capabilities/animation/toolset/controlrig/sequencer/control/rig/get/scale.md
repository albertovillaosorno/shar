...
Use this tool to verify a scale-only Control Rig channel at an exact Sequencer
frame before SHAR deformation or visibility decisions.
...
Two disposable cycles keyed ScaleControl to `(1.5, 2.0, 2.5)` at frame 24 and
returned matching x, y, and z fields.
...
- Validate the SCALE type through `get_controls_info`.
- This value is distinct from an actor or component scale.
- Floating-point values should be compared with tolerance.
