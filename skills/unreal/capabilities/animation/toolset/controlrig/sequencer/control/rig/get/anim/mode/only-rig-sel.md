...
Use this tool to determine whether Animation Mode restricts viewport selection
to Control Rig controls.
...
Two cycles returned `false`, so viewport selection was not restricted to rig
controls.
...
- This is global editor selection policy.
- A false value does not guarantee every actor is selectable.
- Other editor modes and locks still apply.
- Re-read after selection-policy changes.
