...
Use this tool to request a mirrored pose for the current bounded Control Rig
selection.
...
The call returned true for the selected unpaired `PositionControl`; selection
and its zero transform remained unchanged, proving the bounded no-op case.
...
- This fixture had no mirrored naming pair, so the verified behavior is a
  successful bounded no-op. It does not prove a project-specific left/right
  naming convention.
