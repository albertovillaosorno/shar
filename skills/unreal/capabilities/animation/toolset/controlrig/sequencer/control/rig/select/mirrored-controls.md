...
Use this tool to select mirrored counterparts for the current bounded Control
Rig selection.
...
The call returned true for the selected unpaired `PositionControl`; the
selected-control reader remained exactly `PositionControl`.
...
- This fixture had no mirrored naming pair, so the verified behavior is an
  unchanged selection. Test paired controls separately before relying on
  automatic counterpart discovery.
