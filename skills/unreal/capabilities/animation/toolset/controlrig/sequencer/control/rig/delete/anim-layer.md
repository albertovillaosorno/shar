...
Use this tool to remove one exact disposable Control Rig animation layer by
its current index.
...
A disposable duplicate layer was created, its returned index was deleted, and
`get_anim_layers` no longer contained that duplicate.
