...
Use this tool to list Outliner nodes locked against editing in the current
Sequencer session.
...
- Keep node locks distinct from whole-sequence and playback-range locks.
...
- Empty is valid.
- Outliner node locks differ from sequence-level locks.
- Returned proxies are session-local and can become stale.
