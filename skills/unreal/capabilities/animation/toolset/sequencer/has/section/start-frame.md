...
Use this tool to determine whether a MovieScene section has a bounded start
before reading its start frame.
...
Both camera cycles returned `true` for the Camera Cuts section, whose start
frame was 0.
...
- Some section types can be open at the start.
- True does not provide the frame value.
- Check the end-bound read independently.
