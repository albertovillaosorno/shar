...
Use this tool to bake a reviewed SHAR actor binding into explicit transform
animation before downstream sequence editing or export.
...
```json
{
  "bindings": [
    {
      "bindingId": "6BD24629-4C8B-7AA4-052C-BB845456FAB4",
      "sequence": {
...
`get_tracks_on_binding` changed from 0 tracks to 1 track. The new track was an
exact `MovieScene3DTransformTrack` nested under the disposable sequence.
...
- Baking can generate dense transform data across the playback range and
  should be limited to explicitly reviewed bindings.
...
List of SequencerBindingProxy objects to bake.
