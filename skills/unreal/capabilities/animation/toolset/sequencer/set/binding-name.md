...
Use this tool to set binding name within a reviewed SHAR Level Sequence while
preserving exact object identities and an explicit inverse.
...
In the disposable SHAR sequence, `get_binding_name` changed from Actor to MCP
Batch Actor after the call. The result was read independently from the
mutation response before cleanup.
