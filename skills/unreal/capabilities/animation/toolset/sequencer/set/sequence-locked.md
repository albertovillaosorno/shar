...
Use this tool to set sequence locked during a bounded SHAR mission, dialogue,
...
`get_sequence_lock_state` changed from `False` to `True` after the call on the
focused disposable SHAR sequence. The result was read independently from the
mutation response.
...
- Sequence lock is focused-editor state; capture and restore the previous
  value before closing the sequence.
