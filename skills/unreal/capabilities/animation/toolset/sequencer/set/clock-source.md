...
Use this tool to set clock source for a reviewed SHAR mission, dialogue,
...
In the disposable SHAR sequence, `get_clock_source` changed from
<UpdateClockSource.PLATFORM: 1> to <UpdateClockSource.AUDIO: 2> after the
call. The result was read independently from the mutation response before
cleanup.
...
Clock source string (e.g. 'TICK', 'PLATFORM', 'AUDIO', 'TIMECODE', 'CUSTOM').
