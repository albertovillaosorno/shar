...
Use this tool to choose whether a SHAR section restores or keeps evaluated
...
The completion-mode getter changed from project default to restore state;
cleanup restored the accepted `ProjectDefault` token.
...
- The getter serializes enum values as formatted strings such as
  `<MovieSceneCompletionMode.PROJECT_DEFAULT: 2>`; setters accept
  `ProjectDefault`, `KeepState`, or `RestoreState`, not the formatted getter
  text.
