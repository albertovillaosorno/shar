...
Use this tool to set view range for a reviewed SHAR mission, dialogue, camera,
...
`get_view_range` changed from `{"start": -0.5, "end": 5.5}` to `{"start":
-1.5, "end": 8.5}` after the call. The returned values were parsed as seconds
...
The visible range end in seconds.
...
The visible range start in seconds.
