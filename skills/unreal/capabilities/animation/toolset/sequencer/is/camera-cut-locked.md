...
Use this tool to check whether Sequencer camera cuts are currently locked to the
editor viewport.
...
- Use this for viewport state, not sequence asset structure.
...
Two independent disposable sequence cycles returned `false` for an empty
sequence with no camera-cut track.
...
- False can mean no lock or no applicable camera cut.
- The state is editor-session specific.
