...
Use this tool to check whether the active sequence and its descendants are
...
- Compare with `get_sequence_lock_state` when verifying wrapper parity.
...
After locking the disposable sequence, two cycles returned `true`; the result
matched `get_sequence_lock_state`.
...
- It reads active editor state, including descendant-lock semantics.
- Unlock before cleanup or any authorized mutation.
