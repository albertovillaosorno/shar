...
Use this tool to read the tracks currently selected in the active Sequencer
editor.
...
- Read immediately after selecting tracks, before selecting another item
  category.
...
Two cycles returned the selected `Camera Cuts` master track followed by the
actor `Transform` track.
...
- Selecting another category can replace or clear track selection.
- Returned track refs become stale after structural edits.
