...
Use this tool to identify the MovieScene update clock that drives sequence
evaluation.
...
- Preserve the returned enum identity rather than parsing only the numeric
  suffix.
...
After selecting AUDIO, two fixture cycles returned `<UpdateClockSource.AUDIO:
2>` exactly.
...
- Do not compare only the integer value.
- Clock source affects runtime evaluation behavior.
- Invalid sequence refs fail during translation.
