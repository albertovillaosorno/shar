...
Use this tool to read the active LevelSequence lock state through the alternate
Sequencer lock-status endpoint.
...
- Compare with `is_sequence_locked` to detect interface divergence.
...
After locking the fixture, two cycles returned `true`, exactly matching
`is_sequence_locked`.
...
- It currently overlaps with `is_sequence_locked`.
- Treat divergence between the two endpoints as a verification failure.
