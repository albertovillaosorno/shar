...
Use this tool to read how one section restores or retains state after
evaluation.
...
- Preserve the returned enum identity for comparison.
...
Two cycles returned `<MovieSceneCompletionMode.PROJECT_DEFAULT: 2>` for the
Camera Cuts section.
...
- PROJECT_DEFAULT delegates behavior to project or sequence policy.
- Do not infer the final restore behavior from the name alone.
