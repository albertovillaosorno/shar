...
Use this tool to read the Sequencer working range in seconds independently of
playback and visible view ranges.
...
- Parse the returned JSON string as seconds.
...
After setting -2.0 through 10.0 seconds, two cycles returned `{"start": -2.0,
"end": 10.0}`.
...
- Work range is distinct from view and playback ranges.
- Values are seconds and may extend before frame zero.
- Editor changes can alter this state.
