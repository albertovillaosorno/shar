...
Use this tool to remove all bindings while authoring reviewed SHAR mission,
...
`get_bound_objects` changed from 2 actors to an empty list while the binding
identity remained available for later sequence operations.
...
- This clears actor assignments from one binding; it does not remove the
  MovieScene binding object itself.
