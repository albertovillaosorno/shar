...
Use this tool to add one exact sequence-level track to a reviewed SHAR Level
Sequence when the track type is already known.
...
```json
{
  ...
  "track_type": {
    "refPath": "/Script/MovieSceneTracks.MovieSceneEventTrack"
  }
}
```
...
`get_tracks_on_sequence` changed from 0 tracks to 1 track. The returned nested
reference was the exact `MovieSceneEventTrack_0` added to the disposable
sequence.
...
- Track class compatibility is evaluated by Unreal; use an exact native class
  path and the returned nested track reference.
