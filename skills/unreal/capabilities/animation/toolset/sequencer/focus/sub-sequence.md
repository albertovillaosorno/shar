...
Use get_sections() on a sub-track to find the sub-section, then pass it here to
focus the sub-sequence it references.
...
Use this tool to focus Sequencer on the sub-sequence referenced by one exact
section.
...
{
  "sub_section": {
    "refPath": "/Game/SHAR_MCP_Validation_Round50_260718/LS_MCP_Round50.LS_MCP_Round50:MovieScene_0.MovieSceneSubTrack_0.MovieSceneSubSection_0"
  }
}
```
...
The call returned true; `get_focused_sequence` returned the referenced
sub-sequence and hierarchy readers showed the nested focus.
