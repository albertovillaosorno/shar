...
Use this tool to author an exact Boolean property key in a reviewed SHAR
...
```json
{
  ...
  "frame": 36,
  "section": {
    "refPath": "/Game/SHAR_MCP_Validation_KF_23991520/LS_MCP_KF_23991520.LS_MCP_KF_23991520:MovieScene_0.MovieSceneBoolTrack_1.MovieSceneBoolSection_0"
  },
  "value": true
}
```
...
`get_keys` changed from `[]` to one Boolean key at frame 36 with value `true`.
The disposable `bHidden` property section exposed its channel name as `None`,
which was discovered rather than predicted.
...
- The tested Boolean property track reported the channel name `None`; treat
  that as a valid discovered name, not a missing argument.
