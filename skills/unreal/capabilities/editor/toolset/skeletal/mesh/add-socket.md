...
Sockets are named attachment points used to attach weapons, accessories, or
effects at a consistent position relative to a bone.
...
Use this tool to add one named skeletal-mesh socket to an exact reviewed bone
for SHAR attachment authoring.
...
`get_socket_names` returned only `MCP_Socket`, and `get_socket_bone` returned
`Bone01` for the returned socket reference.
...
The name of the bone to attach the socket to.
...
The name to give the new socket.
