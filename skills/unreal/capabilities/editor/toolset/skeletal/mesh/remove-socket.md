...
Use this tool to remove one exact task-owned skeletal-mesh socket after
...
```json
{
  ...
  "socket_name": "MCP_Socket_Renamed"
}
```
...
The call returned true and `get_socket_names` changed from the renamed
task-owned socket to an empty list.
...
The name of the socket to remove.
