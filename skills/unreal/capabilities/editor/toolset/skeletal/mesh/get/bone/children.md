...
Use this tool to inspect one bone's direct children during SHAR hierarchy and
rig compatibility review.
...
- Expect direct children only, not the complete descendant subtree.
...
Repeated reads returned `body` under `Root`, `box` under `body`, and an empty
array for leaf bone `box`. A missing bone raised a bone-not-found error.
<!-- END MANUAL FIELD: project-verification -->

### Known project caveats

<!-- BEGIN MANUAL FIELD: known-caveats -->
- The result contains direct children only.
- An empty array is valid for a leaf bone.
- Order follows the reference-skeleton hierarchy.
- Reimport can invalidate names and relationships.
