...
Use this tool to establish valid skeletal-mesh LOD indices before geometry or
section queries.
...
Two calls returned `1`. Independent section and vertex queries succeeded for LOD
`0`; indices `-1`, `1`, and `999` raised out-of-range errors stating that the
mesh has one LOD.
...
- Imported or generated LOD changes can alter the count.
- Always refresh the count before using cached LOD indices.
