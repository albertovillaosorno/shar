...
next LOD activates. Thresholds must be in strictly descending order (LOD 0 has
the largest threshold), and there must be exactly one threshold per LOD.
...
Use this tool to assign one screen-size threshold per current static-mesh LOD.
...
`get_lod_thresholds` returned one threshold for each of the three current LODs
after the setter completed successfully.
...
Screen-size threshold values, one per LOD, in descending order.
