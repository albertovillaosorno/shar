...
Clears all parameter overrides on a material instance, reverting to parent
defaults.
...
Use this tool to remove material-instance overrides before returning a SHAR
material variant to inherited defaults.
...
After the call, scalar and vector getters returned parent defaults and texture
returned `DefaultTexture`; the static switch remained true and is recorded as
a limitation.
...
- The tested Unreal 5.8 call did not clear the static-switch value:
  `MCPSwitch` remained true while scalar, vector, and texture values reverted.
  Re-read every parameter class after cleanup.
