...
Warning: Each new Material increases shader compile times. Prefer creating a
MaterialInstance from an existing Material where possible.
...
Use this tool to create one bounded SHAR material asset before authoring its
graph.
...
```json
{
  "asset_name": "M_MCP_07b38dea",
  "folder_path": "/Game/SHAR_MCP_Validation_Material_07b38dea"
}
```
...
Asset existence changed from false to true. The returned material reference
matched the requested package, and `get_expressions` independently returned an
empty graph.
...
- Creation returns an unsaved editor asset; verify exact package identity and
  nested state before continuing.
...
The content-browser path to the folder for the new asset.
