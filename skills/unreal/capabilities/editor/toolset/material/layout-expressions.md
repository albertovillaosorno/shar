...
Automatically arranges all expression nodes in a Material or MaterialFunction
graph.
...
Use this tool to apply the Material Editor layout command to a reviewed SHAR
material graph.
...
The call returned null without error; `get_expressions` still returned the
four exact parameter nodes, whose reflected X positions remained 0, 300, 600,
and 900.
...
- The operation returns null and may preserve already orderly coordinates;
  read expression positions and graph membership instead of expecting a
  movement count.
