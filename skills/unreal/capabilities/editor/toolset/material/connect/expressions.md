...
Connects an expression node's output pin to another expression node's input
pin.
...
Use this tool to connect one exact material-expression output to a named input
...
{
  "from_expression": {
    "refPath": "/Game/SHAR_MCP_Validation_Material_07b38dea/M_MCP_07b38dea.M_MCP_07b38dea:MaterialExpressionConstant3Vector_0"
  },
  "from_output_name": "",
  "to_expression": {
...
`get_expression_inputs` changed inputs A and B from expression `None` to the
exact two Constant3Vector references connected to the Add node.
...
The input pin name to connect to. Use get_expression_input_names to discover
valid names.
