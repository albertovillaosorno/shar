...
The parameter expressions themselves are not deleted - only their group
assignment is cleared.

Note: triggers an internal recompile; no separate call to recompile/update is
needed.
...
Use this tool to remove one material parameter group and return its parameters
...
- Capture `list_parameter_groups`, `list_parameters`, and reflected expression
  group values before mutation.
...
`list_parameter_groups` removed group B while all four parameters remained
present. ObjectTools showed the texture and switch parameters were reassigned
to group `None`.
...
- Deleting a group does not delete its parameters. In the reproduced
  lifecycle, texture and switch parameters moved to group `None`.
