...
Use this tool to rename one material parameter group while preserving its
...
```json
{
  ...
  "new_name": "MCP_Group_C",
  "old_name": "MCP_Group_A"
}
```
...
`list_parameter_groups` changed from groups A and B to groups B and C.
ObjectTools independently showed both scalar and vector parameters moved from
A to C.
...
- The operation updates every parameter in the old group; re-read both the
  group inventory and each affected expression.
