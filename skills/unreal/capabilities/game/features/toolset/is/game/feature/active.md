...
Checks whether a Game Feature Plugin is active. Raises an error if the
subsystem is unavailable or the plugin is not found. Use GetGameFeatureState if
you need the current state when the plugin is not active.
...
Use this tool to confirm whether one exact Game Feature is currently active.
...
The reader returned false for the discovered feature in `Unknown` state, then
true after activation and false again after deactivation.
...
True if the Game Feature Plugin is active.
