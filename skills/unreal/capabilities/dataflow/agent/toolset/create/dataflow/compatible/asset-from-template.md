# Create dataflow compatible asset from template
...
Creates a new Dataflow-compatible asset and initialises its embedded Dataflow
graph from a registered template in one step.
...
Asset class name with or without the "U"/"A" prefix
...
empty string to create with an empty embedded graph (equivalent to
