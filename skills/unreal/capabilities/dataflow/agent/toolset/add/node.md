...
```json
{
  ...
  "jsonParams": "{}",
  "nodeName": "Add_c297c180",
  "typeName": "FDataflowMathAddNode",
  "x": 100,
  "y": 100
}
```
...
`GetGraphStructure` added exact Add and Multiply math nodes with stable names,
GUIDs, positions, and pin schemas.
...
The Dataflow asset to add the node to
...
Optional JSON object of property overrides (e.g., {"Value": 3.14})
...
Unique name for the node within this graph
...
X position in the graph editor (default 0, typical node width ~200)
...
Y position in the graph editor (default 0, typical node height ~100)
...
The newly created UDataflowEdNode, or nullptr on failure
