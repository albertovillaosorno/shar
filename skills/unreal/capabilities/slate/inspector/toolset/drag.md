...
Use this tool to dock or reposition one exact Slate tab between reviewed
targets.
...
Dragging the separate Game Feature Data tab returned true. Top-level windows
changed from two to one, and a deep snapshot found the same tab and editor
header docked in the main window.
...
- Docking changes window topology and can invalidate both source and
  destination refs immediately.
...
Slate widget ref for the drop target.

### `modifiers`

- Required: **no**
- Type: `object`
- Default: `{"bShift":false,"bCtrl":false,"bAlt":false,"bCmd":false}`
- Purpose:
...
Slate widget ref for the drag source.
