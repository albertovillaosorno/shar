...
Removes the body for the given bone along with any constraints that reference
it. Raises a script error if PhysicsAsset is null or no body exists for
BoneName.
...
Use this tool to remove one exact physics body from a Physics Asset.
...
`GetBodyNames` returned to only `Bone01` after removing the exact `Bone02`
body.
...
The name of the bone whose body to remove.
