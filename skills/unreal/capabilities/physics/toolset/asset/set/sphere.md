...
Adds or replaces a sphere collision primitive on a body. If any shape with the
...
Use this tool to create or update one named sphere shape on an existing
physics body.
...
`GetBodyShapes` added named sphere `MCP_Sphere` at `(1, 2, 3)` with radius
`8`.
...
Center of the sphere in bone-local space (cm).
...
Radius of the sphere (cm). Must be greater than zero.
