...
- Evidence status: Partially verified — The `RADCORE CEMENT LIBRARY` identity,
  aligned archive regions, name and directory tables, entry preflight, path
  safety, and extraction behavior are verified in repository code and tests;
  compression variants, platform differences, version history, and current
...
- Subject class: Proprietary aligned archive container.

## Covered Material

The RCF archive family associated with RadCore and File Cementer, including
header identification, aligned payload regions, directory and name tables, entry
metadata, and archive extraction behavior.
...
Historical local evidence identifies File Cementer, Cement libraries, and
RadCore notices. The complete version history, compression variants, platform
differences, and current ownership chain remain unresolved.
...
- Do not infer ownership, authorization, or redistribution rights from
  successful decoding.
- Fail closed on path traversal, overlapping ranges, malformed names, and
  unsupported variants.
- Do not treat archive entry names or extracted contents as repository-owned
  material.
