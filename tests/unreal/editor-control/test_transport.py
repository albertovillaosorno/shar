...
def test_transport_rejects_unbounded_unique_pagination(
    ...
    monkeypatch.setattr(
        "mcp.adapter_outbound.streamable_http._MAX_TOOL_LIST_PAGES",
        2,
    )
...
def test_transport_rejects_excessive_paginated_tool_count(
    ...
    monkeypatch.setattr(
        "mcp.adapter_outbound.streamable_http._MAX_TOOL_NAMES",
        2,
    )
    with FakeUnrealServer() as server:
        transport = StreamableHttpTransport(
...
def test_transport_rejects_excessive_paginated_tool_name_bytes(
    ...
    monkeypatch.setattr(
        "mcp.adapter_outbound.streamable_http._MAX_TOOL_NAME_BYTES",
        first_page_bytes,
    )
    with FakeUnrealServer() as server:
        transport = StreamableHttpTransport(
...
def test_transport_rejects_excessive_single_cursor_bytes(
    ...
    monkeypatch.setattr(
        "mcp.adapter_outbound.streamable_http._MAX_SINGLE_CURSOR_BYTES",
        len("page-2") - 1,
        raising=False,
    )
    with FakeUnrealServer(advance_cursor=True) as server:
...
def test_transport_rejects_excessive_pagination_cursor_bytes(
    ...
    monkeypatch.setattr(
        "mcp.adapter_outbound.streamable_http._MAX_PAGINATION_CURSOR_BYTES",
        len("page-2"),
    )
    with FakeUnrealServer(advance_cursor=True) as server:
        transport = StreamableHttpTransport(
