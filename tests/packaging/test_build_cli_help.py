...
class BuildCliHelpTests(unittest.TestCase):
    ...
    def test_arch_revalidation_rejects_saved_selection_drift(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="shar-arch-drift-") as raw:
            ...
            selected = [module._TARGETS_BY_ID["linux-x64"]]
            module._write_selection(output, selected)
            real_payload = module._payload

            def drift(selection: list[object]) -> dict[str, object]:
                expected = real_payload(selection)
                output.write_text(
                    module.json.dumps({**expected, "drift": True}) + "\n",
                    encoding="utf-8",
                )
                return expected

            with (
                mock.patch.object(module, "_payload", side_effect=drift),
                self.assertRaisesRegex(
                    SystemExit,
                    "saved selection changed during revalidation",
                ),
            ):
                module._revalidate_selection(output)
