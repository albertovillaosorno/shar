...
// Boundary-Contract:
// - Owns:
//   - Semantic body texture test module.
// - Must-Not:
...
-> Result<(), String> {
    ...
    let first = plan_body_texture(&character, &source, &recipe)
        .map_err(|error| format!("first plan failed: {error:?}"))?;
    let second = plan_body_texture(&character, &source, &recipe)
    ...
    validate_uv_only_change(&character, first)
}
...
fn validate_plan_shape(plan: &BodyTexturePlan) -> Result<(), String> {
    ...
    let regions = plan
        ...
        .collect::<BTreeSet<_>>();
    if regions != BodyRegion::ALL.into_iter().collect() {
        return Err(format!("unexpected semantic regions: {regions:?}"));
    }
    if plan.charts.len() != 5 {
        return Err(format!("expected five charts, got {}", plan.charts.len()));
    }
    for (index, first) in plan.charts.iter().enumerate() {
        for second in plan.charts.iter().skip(index + 1) {
            if overlaps(first.cell, second.cell) {
                return Err(format!(
        ...
        let escaped = first.content.x < first.cell.x
            || first.content.y < first.cell.y
            || first.content.right() > first.cell.right()
            || first.content.bottom() > first.cell.bottom();
        if escaped {
            return Err(format!(
...
fn validate_atlas_colors(plan: &BodyTexturePlan) -> Result<(), String> {
    let colors = plan.atlas.pixels().iter().copied().collect::<BTreeSet<_>>();
...
fn validate_uv_only_change(
    ...
    let original_uvs = character.parts[0].mesh.groups[0].uvs.clone();
    let remapped_uvs = &plan.remapped_character.parts[0].mesh.groups[0].uvs;
    if original_uvs == *remapped_uvs {
        return Err("semantic planning did not remap UVs".to_owned());
    }
    if remapped_uvs
        .iter()
        .any(|uv| !(0. ..=1.).contains(&uv[0]) || !(0. ..=1.).contains(&uv[1]))
    {
        return Err("remapped UV escaped the atlas".to_owned());
    }
    let mut normalized = plan.remapped_character;
    normalized.parts[0].mesh.groups[0].uvs = original_uvs;
...
fn tiled_sampling_wraps_before_v_up_texel_selection() -> Result<(), String> {
    ...
    let image = RgbaImage::new(2, 2, vec![
...
fn classifies_legacy_pelvis_support_joint_as_lower_body() {
    assert_eq!(BoneFamily::from_bone_id("Ass_Joint"), BoneFamily::LowerBody,);
}
...
fn semantic_atlas_allows_an_empty_hair_lane() -> Result<(), String> {
    ...
    let _previous = recipe
        .color_overrides
        .insert(BODY_COLORS[1], BodyRegion::Skin);
