...
fn write_file_entry(
    ...
    if !copy_fixture_bytes(archive, position, &2_u16.to_le_bytes()) {
    ...
    if !copy_fixture_bytes(archive, position.saturating_add(2), &encoded_name) {
...
fn empty_archive_with(marker: &[u8]) -> Vec<u8> {
    ...
    assert!(
        copy_fixture_bytes(&mut archive, 0, MAGIC),
        "fixture magic range must fit"
    );
    assert!(
        copy_fixture_bytes(&mut archive, 4, &5_u32.to_le_bytes(),),
        "fixture version range must fit"
    );
    assert!(
        copy_fixture_bytes(&mut archive, 0x0c, &0x0200_0000_u32.to_le_bytes(),),
    ...
    assert!(
        copy_fixture_bytes(&mut archive, META_OFFSET, marker),
...
fn truncated_magic_reports_truncation() -> Result<(), String> {
    match parse(b"LSP") {
        ...
        other => Err(format!(
            "truncated magic must report truncation, got {other:?}"
        )),
    }
}
...
fn wrong_magic_reports_the_observed_bytes() -> Result<(), String> {
    match parse(b"NOPE") {
        Err(LmlmError::BadMagic { observed }) if observed == *b"NOPE" => Ok(()),
...
fn rejects_nonzero_trailing_archive_padding() -> Result<(), String> {
    ...
    if !copy_fixture_bytes(&mut archive, trailing_offset, &[1]) {
...
fn rejects_payloads_inside_the_entry_table() -> Result<(), String> {
    ...
    write_file_entry(&mut archive, TEST_ENTRY, "table.bin", 0x800, 1)?;
    match parse(&archive) {
        ...
        other => Err(format!(
            "payload inside entry table must fail, got {other:?}"
        )),
    }
}
...
fn accepts_final_payload_immediately_after_metadata() -> Result<(), String> {
    ...
    if !copy_fixture_bytes(&mut archive, payload_offset, &[1]) {
...
fn rejects_unsupported_file_record_control() -> Result<(), String> {
    ...
    match parse(&archive) {
        Err(LmlmError::UnsupportedFileRecordControl { offset, value: 2 })
...
fn rejects_nonzero_file_record_control_padding() -> Result<(), String> {
    ...
    let padding_offset = FIRST_ENTRY
        ...
        .saturating_add(1);
    if !copy_fixture_bytes(&mut archive, padding_offset, &[1]) {
        return Err("fixture file transition padding must fit".to_owned());
    }
    match parse(&archive) {
        Err(LmlmError::NonZeroFileRecordPadding { offset, value: 1 })
...
fn accepts_directory_control_for_immediate_subdirectory() -> Result<(), String>
{
    ...
    write_directory_entry(&mut archive, TEST_ENTRY, "parent", 1, true)?;
...
fn rejects_unsupported_directory_record_control() -> Result<(), String> {
    ...
    write_directory_entry(&mut archive, TEST_ENTRY, "empty", 0, false)?;
...
fn rejects_nonzero_directory_record_padding() -> Result<(), String> {
    ...
    write_directory_entry(&mut archive, TEST_ENTRY, "empty", 0, false)?;
    let padding_offset = TEST_ENTRY.saturating_add(BLOCK).saturating_add(0x0f);
    if !copy_fixture_bytes(&mut archive, padding_offset, &[1]) {
...
fn rejects_directory_control_missing_subdirectory() -> Result<(), String> {
    ...
    write_directory_entry(&mut archive, TEST_ENTRY, "parent", 1, false)?;
    write_directory_entry(
        &mut archive,
        TEST_ENTRY.saturating_add(BLOCK.saturating_mul(2)),
        "child",
...
fn rejects_nonzero_reserved_metadata_bytes() -> Result<(), String> {
    for relative_offset in [0, 0x0b] {
        ...
        if !copy_fixture_bytes(&mut archive, metadata_offset, &[1]) {
...
fn rejects_nonzero_metadata_padding() -> Result<(), String> {
    ...
    write_file_entry(&mut archive, TEST_ENTRY, "metadata.bin", 0x3200, 1)?;
    let padding_offset = TEST_ENTRY.saturating_add(BLOCK).saturating_add(0x1c);
    if !copy_fixture_bytes(&mut archive, padding_offset, &[1]) {
...
fn rejects_nonzero_name_padding() -> Result<(), String> {
    ...
    write_file_entry(&mut archive, TEST_ENTRY, "pad.bin", 0x3200, 1)?;
...
fn rejects_excessive_directory_nesting() -> Result<(), String> {
    ...
    for index in 0_u8..65_u8 {
        write_directory_entry(&mut archive, position, "d", 1, index < 64)?;
        position = position.saturating_add(BLOCK.saturating_mul(2));
    }
    write_file_entry(&mut archive, position, "leaf.bin", 0x20200, 1)?;
    match parse(&archive) {
        Err(LmlmError::ExcessiveDirectoryDepth { depth: 65, .. }) => Ok(()),
        other => Err(format!(
            "excessive directory nesting must fail, got {other:?}"
        )),
    }
}
...
fn rejects_overlong_portable_paths() -> Result<(), String> {
    ...
    write_directory_entry(&mut archive, TEST_ENTRY, &directory, 1, false)?;
    write_file_entry(
        &mut archive,
        TEST_ENTRY.saturating_add(BLOCK.saturating_mul(2)),
        &file,
...
-> Result<(), String> {
    ...
    match parse(&archive) {
        Ok(entries) if entries.iter().any(|entry| entry.path == name) => Ok(()),
        other => Err(format!(
...
fn rejects_portable_path_collisions() -> Result<(), String> {
    ...
    write_file_entry(&mut archive, TEST_ENTRY, "Shared.bin", 0x3200, 0x200)?;
    write_file_entry(
        &mut archive,
...
fn rejects_unicode_case_equivalent_path_collisions() -> Result<(), String> {
    ...
    write_file_entry(&mut archive, TEST_ENTRY, "Σ.bin", 0x3200, 0x200)?;
    write_file_entry(
        &mut archive,
        TEST_ENTRY.saturating_add(BLOCK.saturating_mul(3)),
        "ς.bin",
...
fn rejects_overlapping_payload_ranges() -> Result<(), String> {
    ...
    write_file_entry(&mut archive, TEST_ENTRY, "first.bin", 0x3200, 0x300)?;
    write_file_entry(
        &mut archive,
        TEST_ENTRY.saturating_add(BLOCK.saturating_mul(3)),
        "second.bin",
    ...
    match parse(&archive) {
        ...
        other => Err(format!(
            "overlapping payload ranges must fail, got {other:?}"
        )),
    }
}
...
fn rejects_payload_past_archive_end() -> Result<(), String> {
    ...
    match parse(&archive) {
        Err(LmlmError::InvalidEntryRange { .. }) => Ok(()),
